...
void __wbt_done(struct rq_wb *rwb, enum wbt_flags wb_acct)
{
	...
	if (test_bit(WB_no_wbt_wait, &rwb->bdi->wb.state)) {
		rwb_wake_all(rwb);
		return;
	}
...
static void wb_timer_fn(unsigned long data)
{
	...
	case LAT_UNKNOWN_WRITES:
		 ...
		 * Allow step to go negative, to decrease write perf.
		 */
		scale_down(rwb, true);
		break;
	case LAT_UNKNOWN:
...
{
	if (current_is_kswapd() || test_bit(WB_no_wbt_wait, &rwb->bdi->wb.state))
		return false;

	/*
	 * If not a WRITE (or a discard), do nothing
	 */
	if (!(rw & REQ_WRITE) || (rw & REQ_DISCARD) || (rw & REQ_SYNC) || (rw & REQ_META) || (rw & REQ_NOMERGE))
		return false;
...
void wbt_set_queue_depth(struct rq_wb *rwb, unsigned int depth)
{
	if (rwb) {
		/*
		   Dm dev creat set q->nr_requests defaults value is latter, so Dm dev
		   can not throttle. This to set queue_depth defaults value.
		 */
		if (depth == 0)
			rwb->queue_depth = BLKDEV_MAX_RQ;
		else
