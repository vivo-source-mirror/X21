...
#define MSM_POSTPROCESSING_NAME	"msm_postproc"
#define MSM_DUMMY_NAME	"msm_dummy"
...
{
	int rc = 0;
	/* Jeff commented out: struct v4l2_event event; */
	struct msm_video_device *pvdev = video_drvdata(filep);
...
/* Jeff added for VIVO -- begin */
	parm->parm.capture.capturemode = event_data->session_id;
	parm->parm.capture.extendedmode = event_data->stream_id;
	/* Jeff added for VIVO -- end */
	sp->stream_created = true;

	return rc;
/*  SL comment below to remove warning
error:
...
static int camera_v4l2_open(struct file *filep)
{
#if 0
	struct msm_video_device *pvdev = video_drvdata(filep);
	printk(KERN_ERR "Jeff: %s called, open_cnt = %d\n", 
	...
	msm_camera_open_check(&pvdev->vdev->dev);
	return rc;
...
static inline int __camera_v4l2_close_internal(struct file *filep,
	enum msm_camera_close_t type)
{
	struct msm_video_device *pvdev = video_drvdata(filep);
	struct camera_v4l2_private *sp = fh_to_private(filep->private_data);
	...
	msm_camera_close_check(&pvdev->vdev->dev, type);
...
static int camera_v4l2_close(struct file *filep)
{
	...
	return -EIO;
#else
  return __camera_v4l2_close_internal(filep, MSM_CAMERA_CLOSE_SENSOR);
#endif
}
...
static int camera_v4l2_cpp_close(struct file *filep)
{
	return __camera_v4l2_close_internal(filep, MSM_CAMERA_CLOSE_CPP);
}

struct v4l2_file_operations camera_v4l2_cpp_fops = {
	.release = camera_v4l2_cpp_close,
};
	...
	enum camera_init_v4l2_t type)
{
	...
	/* Jeff added - begin */
	switch (type) {
	case CAMERA_V4L2_SENSOR: {
		vdev_name = "msm-sensor";
		pvdev->vdev->fops = &camera_v4l2_fops;
#if defined(CONFIG_MEDIA_CONTROLLER)
		mdev_model = MSM_CAMERA_NAME;
#endif
	}
		break;

	case CAMERA_V4L2_PPROC: {
		vdev_name = "msm-pproc";
		camera_v4l2_cpp_fops = camera_v4l2_fops;
		camera_v4l2_cpp_fops.release = camera_v4l2_cpp_close;
		pvdev->vdev->fops = &camera_v4l2_cpp_fops;
#if defined(CONFIG_MEDIA_CONTROLLER)
		mdev_model = MSM_POSTPROCESSING_NAME;
#endif
	}
		break;

	case CAMERA_V4L2_DUMMY: {
		vdev_name = "msm-dummy";
		camera_v4l2_cpp_fops = camera_v4l2_fops;
		camera_v4l2_cpp_fops.release = camera_v4l2_cpp_close;
		pvdev->vdev->fops = &camera_v4l2_cpp_fops;
#if defined(CONFIG_MEDIA_CONTROLLER)
		mdev_model = MSM_DUMMY_NAME;
#endif
	}
		break;
	...
	strlcpy(v4l2_dev->mdev->model, mdev_model, sizeof(v4l2_dev->mdev->model));
	...
	strlcpy(pvdev->vdev->name, vdev_name, sizeof(pvdev->vdev->name));
	pvdev->vdev->release  = video_device_release;
#if 0
	if (type == CAMERA_V4L2_SENSOR) {
		pvdev->vdev->fops = &camera_v4l2_fops;
	} else if (type == CAMERA_V4L2_PPROC) {
		camera_v4l2_cpp_fops = camera_v4l2_fops;
		camera_v4l2_cpp_fops.release = camera_v4l2_cpp_close;
		pvdev->vdev->fops = &camera_v4l2_cpp_fops;
	}
