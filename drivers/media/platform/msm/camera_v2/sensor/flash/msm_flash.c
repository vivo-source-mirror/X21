...
static struct msm_flash_ctrl_t *g_fctrl[MAX_FLASHS];  /*add by vivo wenjun for ext led_flash node 2017.02.27*/
struct ir_flash_work {
	struct delayed_work ir_delayed_work;
	struct msm_flash_ctrl_t *ir_fctrl;
};

struct ir_flash_work ir_flash_wq;
static struct v4l2_file_operations msm_flash_v4l2_subdev_fops;
/*static struct led_trigger *torch_trigger;  modify by vivo wenjun*/
...
#ifdef QCOM_ORIGIN /*modify by vivo wenjun*/
						...
						struct kobj_attribute *attr, char *buf)
{
   ...
   if (0 == strcmp(attr->attr.name, "EXT_REAR_FLASH_LED"))
		flash_ctrl = g_fctrl[FLASH_BACK];
	else if (0 == strcmp(attr->attr.name, "EXT_FRONT_FLASH_LED"))
		flash_ctrl = g_fctrl[FLASH_FRONT];

	if (!flash_ctrl) {
		pr_err("%s:%d failed flash_ctrl is NULL\n", __func__, __LINE__);
		return -EINVAL;
	}

   if (1 == flash_ctrl->camera_flash_mode)
	return 11;
   else
	return 10;
/*
	for (i = 0; i < flash_ctrl->torch_num_sources; i++){
		if (flash_ctrl->torch_trigger[i])
			led_trigger_event(flash_ctrl->torch_trigger[i], 255);
						...
						struct kobj_attribute *attr, const char *buf, size_t size)
{
   ...
   if (0 == strcmp(attr->attr.name, "EXT_REAR_FLASH_LED"))
	...
	/*add by vivo wenjun for lcd-flash*/
	/*if (1 == flash_ctrl->lcd_flash) {
		pr_err("wenjun+++ lcd-flash mode , do nothing\n");
		return 1;
	}*/
	/*add by vivo wenjun for lcd-flash*/
	...
	pr_err("set flash_current = (%d) ; name %s ; flash_mode = %d\n", flash_current, attr->attr.name, flash_mode);

	if (flash_ctrl->torch_num_sources != 0)
		cur = flash_current / flash_ctrl->torch_num_sources ;    /*The current vali is from application*/

	for (i = 0; i < flash_ctrl->torch_num_sources; i++) {
		if (flash_ctrl->torch_trigger[i])
			led_trigger_event(flash_ctrl->torch_trigger[i], cur);
		CDBG("led_trigger_event: torch_trigger[%d] = %d \n", i, cur);
		msleep(1);
	}
	if (flash_ctrl->switch_trigger)
		led_trigger_event(flash_ctrl->switch_trigger, flash_mode);
	return size;
}
...
__ATTR(EXT_FRONT_FLASH_LED, 0644, ext_flash_led_show, ext_flash_led_store);
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	/*add by vivo wenjun for lcd-flash*/
	if (1 == flash_ctrl->lcd_flash) {
		pr_err("lcd-flash mode , do nothing\n");
		return 0;
	}
	/*add by vivo wenjun for lcd-flash*/
	...
	if (flash_ctrl->switch_trigger)
		led_trigger_event(flash_ctrl->switch_trigger, 0);
	flash_ctrl->camera_flash_mode = 0;
	return 0;
}
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	flash_ctrl->startTime = ktime_get();
	flash_ctrl->led_count = 0;
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if (flash_ctrl->switch_trigger)
		led_trigger_event(flash_ctrl->switch_trigger, 1);
	flash_ctrl->camera_flash_mode = 1;
	return 0;
}

static int32_t msm__ir_flash_enable(
	struct msm_flash_ctrl_t *flash_ctrl,
	struct msm_flash_cfg_data_t *flash_data)
{
	int32_t ir_enable = flash_data->ir_flash_enable;
	int i = 0;

	if (flash_ctrl->irq_num > 0 && ir_enable == 1 && flash_ctrl->ir_irq_enable == 0) {
		enable_irq(flash_ctrl->irq_num);
		flash_ctrl->ir_irq_enable = 1;
	} else if (flash_ctrl->ir_irq_enable == 1) {
		disable_irq(flash_ctrl->irq_num);
		flash_ctrl->ir_irq_enable = 0;
	}		
	for (i = 0; i < flash_ctrl->torch_num_sources; i++)
		if (flash_ctrl->torch_trigger[i])
			led_trigger_event(flash_ctrl->torch_trigger[i], 0);
	...
	flash_ctrl->startTime = ktime_get();
	flash_ctrl->led_count = 0;
	flash_ctrl->first_led_on = 0;
	pr_err("enable irq %d flash_ctrl->irq_num %d\n", ir_enable, flash_ctrl->irq_num);
	return 0;
}
...
{
	int32_t rc = 0;
/* modiy by vivo wenjun to solve front moonlight and facewake conflict */
   if (1 == flash_ctrl->camera_flash_mode) {
	...
	void __user *argp)
{
	...
	case CFG_FLASH_HIGH:
		...
		break;
	case CFG_IR_FLASH_ENABLE:
		rc = flash_ctrl->func_tbl->camera_ir_flash_enable(
			flash_ctrl, flash_data);
		break;
	default:
		rc = -EFAULT;
...
static irqreturn_t ir_threaded_isr(int irq, void *dev_id)
{
	schedule_delayed_work(&ir_flash_wq.ir_delayed_work, msecs_to_jiffies(0));
	return IRQ_HANDLED;
}
static void ir_delayed_func(struct work_struct *work)
{
	struct delayed_work *ir_flash_dw = container_of(work, struct delayed_work, work);
	struct ir_flash_work *ir_flash_work = container_of(ir_flash_dw, struct ir_flash_work, ir_delayed_work);
	struct msm_flash_ctrl_t *flash_ctrl = ir_flash_work->ir_fctrl;
	int32_t curr = 1200;
	int32_t max_current = 0;
	int32_t i = 0;
	u64 deltaTime = ktime_us_delta(ktime_get(), flash_ctrl->startTime);
	
	
	if (flash_ctrl->first_led_on == 0 && deltaTime > 100000) {
		flash_ctrl->first_led_on++;
	}
	
	if ((deltaTime > 380000 && flash_ctrl->led_count < 13) || (flash_ctrl->first_led_on == 2)) {

		flash_ctrl->startTime = ktime_get();
		flash_ctrl->led_count++;
	/* Turn on flash triggers */
		for (i = 0; i < flash_ctrl->flash_num_sources; i++) {
			if (flash_ctrl->flash_trigger[i]) {
				max_current = flash_ctrl->flash_max_current[i];
...
static int32_t msm_flash_init_ir_irq(struct device_node *of_node,
	struct msm_flash_ctrl_t *fctrl)
{

	int ret = 0;

	ret = of_get_named_gpio_flags(of_node, "ir-irq-gpio", 0, NULL);
	if (ret <= 0) {
		pr_err("can not find property irq-gpio, error %d\n", ret);
		return ret;
	}
	fctrl->ir_irq_gpio = ret;

	ret = gpio_request_one(fctrl->ir_irq_gpio, 2, "ir-irq-gpio");
	if (ret) {
		pr_err("gpio %d request error %d\n", fctrl->ir_irq_gpio, ret);
		return ret;
	}

	ret = gpio_direction_input(fctrl->ir_irq_gpio);
	if (ret < 0) {
		pr_err("set gpio %d  input error %d\n", fctrl->ir_irq_gpio, ret);
		goto ir_irq_gpio_err;
	}

	ret = gpio_to_irq(fctrl->ir_irq_gpio);
	if (ret < 0) {
		pr_err("unable to get irq num for gpio %d, error %d\n", fctrl->ir_irq_gpio, ret);
		goto ir_irq_gpio_err;
	}
	fctrl->irq_num = ret;

	ret = request_threaded_irq(fctrl->irq_num, NULL, ir_threaded_isr,
				IRQF_TRIGGER_RISING | IRQF_ONESHOT, "ir_irq", fctrl);
	if (ret < 0) {
		pr_err("cannot request threaded for  irq  error %d\n", ret);
		goto ir_irq_err;
	}
	INIT_DELAYED_WORK(&ir_flash_wq.ir_delayed_work, ir_delayed_func);

	ir_flash_wq.ir_fctrl = fctrl;
	disable_irq(fctrl->irq_num);
	fctrl->ir_irq_enable = 0;
	return ret;

ir_irq_err:
	free_irq(fctrl->irq_num, fctrl);

ir_irq_gpio_err:
	gpio_free(fctrl->ir_irq_gpio);
	return ret;
}
	...
	struct msm_flash_ctrl_t *fctrl)
{
	...
	/*add by vivo wenjun for lcd-flash*/
	fctrl->lcd_flash = 0;
	rc = of_property_read_u32(of_node, "lcd-flash", &fctrl->lcd_flash);
	if (rc < 0) {
		pr_err("failed get lcd-flash rc %d ++++++++\n", rc);
	}
	/*add by vivo wenjun for lcd-flash*/
	...
	msm_flash_init_ir_irq(of_node, fctrl);
	...
	struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_FLASH_CFG32:
		...
		case CFG_FLASH_INIT:
			...
			break;
		case CFG_IR_FLASH_ENABLE:
			flash_data.ir_flash_enable = u32->ir_flash_enable;
			break;
		default:
			break;
		}
...
#endif
/*add by vivo wenjun for ext led_flash node 2017.02.27*/
	switch (flash_ctrl->pdev->id) {
	case FLASH_BACK:
...
/*add end++++++++++++++*/
	CDBG("probe success g_fctrl[%d] %pK\n", flash_ctrl->pdev->id, g_fctrl[flash_ctrl->pdev->id]);
...
{
	int32_t rc = 0;
	rc = ext_flash_led_init();  /*add by vivo wenjun for ext led_flash node*/
...
static struct msm_flash_table msm_pmic_flash_table = {
	...
	.func_tbl = {
		...
		.camera_ir_flash_enable = msm__ir_flash_enable,
	},
};
