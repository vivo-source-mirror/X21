...
ERROR:
	...
	/**
	  * eeprom_parse_memory_map for OTP type of hynix  - Parse mem map
	  * @e_ctrl:	ctrl structure
	  * @eeprom_map_array: eeprom map
	  *
	  * Returns success or failure
	  */
	static int eeprom_parse_memory_map_Hynix(struct msm_eeprom_ctrl_t *e_ctrl,
		struct msm_eeprom_memory_map_array *eeprom_map_array)
	{
		int rc =  0, i, j, k;
		uint8_t *memptr;
...
static int msm_eeprom_power_up(struct msm_eeprom_ctrl_t *e_ctrl,
	...
	int32_t rc = 0;
	/*luzhi-add to fix vreg is 0 cause eeprom power up failed issue*/
	if (power_info->num_vreg > 0) {
	...
	void __user *argp)
{
	...
	isHynixType = cdata->cfg.eeprom_info.isHynixMode;
    ...
    if (isHynixType == 1)
		rc = eeprom_parse_memory_map_Hynix(e_ctrl, memory_map_arr);
	else
	...
	void __user *argp)
{
	...
	case CFG_EEPROM_INIT:
		...
		if (e_ctrl->cal_data.num_data == 0) {
			...
			pr_err("%s:%d Already read eeprom\n",
				__func__, __LINE__);
		}
		break;
	default:
	...
	void __user *argp)
{
	...
	if (mem_map_array == NULL) {
		...
		isHynixType = cdata32->cfg.eeprom_info.isHynixMode;
	...
	if (rc < 0) {
      ...
      if (isHynixType == 1)  /*if sensor is type of Hynix add by vivo dongzhixiang*/
		rc = eeprom_parse_memory_map_Hynix(e_ctrl, mem_map_array);   
	  else
		rc = eeprom_parse_memory_map(e_ctrl, mem_map_array);
	if (rc < 0) {
		pr_err("%s:%d memory map parse failed\n",
			__func__, __LINE__);
		prc = msm_camera_power_down(power_info,
			e_ctrl->eeprom_device_type, &e_ctrl->i2c_client);
		if (prc < 0)
			pr_err("%s:%d Power down failed rc %d\n",
				__func__, __LINE__, prc);
		goto free_mem;
	}
