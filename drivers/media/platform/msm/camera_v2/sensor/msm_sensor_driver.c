			...
			(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->bypass_video_node_creation == 0) {
		rc = camera_init_v4l2(&client->dev, &session_id, CAMERA_V4L2_SENSOR);
		if (rc < 0) {
			...
			(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->bypass_video_node_creation == 0) {
		rc = camera_init_v4l2(&s_ctrl->pdev->dev, &session_id, CAMERA_V4L2_SENSOR);
		if (rc < 0) {
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (s_ctrl->is_probe_succeed == 1) {
		...
		if ((slave_info->sensor_id_info.sensor_id ==
			s_ctrl->sensordata->cam_slave_info->
				...
				(slave_info->sensor_id_info.sensor_id2 != 0 &&
				slave_info->sensor_id_info.sensor_id2 ==
			s_ctrl->sensordata->cam_slave_info->
				sensor_id_info.sensor_id2)) && 
			...
			msm_sensor_fill_sensor_info(s_ctrl,
				...
				pr_err("slot %d match this sensor failed\n",
				slave_info->camera_id);
				rc = -EINVAL;		/*modify by vivo wenjun for QC bug  2017.03.04*/
			}
		goto free_slave_info;
	}
	...
	camera_info->sensor_id2_reg_addr =
		slave_info->sensor_id_info.sensor_id2_reg_addr;
