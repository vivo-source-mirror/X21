...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (msm_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
		pr_err("%s chip id %x does not match %x\n",
				__func__, chipid, slave_info->sensor_id);
		if (slave_info->sensor_id2_reg_addr != 0) {
			rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
				sensor_i2c_client, slave_info->sensor_id2_reg_addr,
				&chipid, MSM_CAMERA_I2C_WORD_DATA);
			pr_err("%s chip id2 %x ; expected id2 %x\n",
					__func__, chipid, slave_info->sensor_id2);
			if (msm_sensor_id_by_mask(s_ctrl, chipid) == slave_info->sensor_id2)
				return rc;
		}
		return -ENODEV;
	}
	...
	void __user *argp)
{
	...
	case CFG_WRITE_I2C_ARRAY_ASYNC: {
		...
		/*add by luzhi for i2c frequency dynamic switching */
		s_ctrl->sensor_i2c_client->cci_client->i2c_freq_mode = conf_array32.i2c_freq_mode;
		pr_debug("%s i2c_freq_mode=%d\n", __func__, s_ctrl->sensor_i2c_client->cci_client->i2c_freq_mode);
