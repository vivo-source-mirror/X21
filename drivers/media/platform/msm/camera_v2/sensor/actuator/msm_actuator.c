...
static struct msm_actuator msm_midvcm_actuator_table;/*add for mid vcm type, vivo liuchangyou 20170517*/
...
static struct msm_actuator *actuators[] = {
	...
	&msm_midvcm_actuator_table,/*add for mid vcm type, vivo liuchangyou 20170517*/
};
	...
	struct msm_actuator_move_params_t *move_params)
{
	...
	if (a_ctrl->curr_step_pos != 0) {
		...
		/*luzhi add for sw virtual*/
		if (a_ctrl->is_virtual_af == 0) {
	...
	int16_t next_lens_position, uint32_t hw_params, uint16_t delay)
{
	...
	/*luzhi add for sw virtual af*/
	if (a_ctrl->is_virtual_af) {
		printk("alexadd %s now use a virtual af device and no need to write register\n", __func__);
		return;
	}
	...
	struct msm_actuator_move_params_t *move_params)
{
	...
	if (dest_step_pos == a_ctrl->curr_step_pos){
          //liuchangyou add, return current dac if position is not change
          move_params->curr_lens_pos = a_ctrl->step_position_table[a_ctrl->curr_step_pos];
          return rc;
        }
	...
	if (0 == reg_setting.size) {
		pr_err("the reg_setting.size is null!!!\n");
		return rc;
	}
	/*add end*/
	/*luzhi add for sw virtual af*/
	...
	void __user *argp)
{
	...
	case CFG_ACTUATOR_INIT:
		/*luzhi add for sw virtual af*/
		a_ctrl->is_virtual_af = cdata->is_virtual_af;
		printk("CFG_ACTUATOR_INIT a_ctrl->is_virtual_af=%d\n", (int)cdata->is_virtual_af);
	...
	struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_ACTUATOR_CFG32:
		...
		default:
			actuator_data.cfgtype = u32->cfgtype;
			/*luzhi-add for 2PD front virtual af*/
			actuator_data.is_virtual_af = u32->is_virtual_af;
			parg = &actuator_data;
			break;
		}
		break;
...
/*add for mid vcm type, vivo liuchangyou 20170517*/
