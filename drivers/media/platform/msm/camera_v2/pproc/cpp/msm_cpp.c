...
static int msm_cpp_check_priority_wait(struct cpp_device *cpp_dev, 
	enum msm_cpp_stream_t stream_type, int timeout, struct v4l2_fh *vfh);
...
static void msm_cpp_enqueue(struct msm_device_queue *queue,
...
static int cpp_init_hardware(struct cpp_device *cpp_dev)
{
	...
	pr_err("stream_cnt:%d\n", atomic_read(&cpp_dev->stream_cnt));
	atomic_set(&cpp_dev->stream_cnt, 0);
...
static void cpp_release_hardware(struct cpp_device *cpp_dev)
{
	...
	if (atomic_read(&cpp_dev->stream_cnt) > 0) {
...
static int cpp_open_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	vfh = &fh->vfh;
	vfh->prio = V4L2_PRIORITY_UNSET;

	if (atomic_read(&cpp_dev->cpp_open_cnt) == MAX_ACTIVE_CPP_INSTANCE) {
	...
	CPP_DBG("open %s %d %p\n", __func__, i, &fh->vfh);
	atomic_add(1, &cpp_dev->cpp_open_cnt);

	msm_cpp_vbif_register_error_handler(cpp_dev,
		VBIF_CLIENT_CPP, cpp_vbif_error_handler);

	if (atomic_read(&cpp_dev->cpp_open_cnt) == 1) {
		rc = cpp_init_hardware(cpp_dev);
		if (rc < 0) {
			atomic_sub(1, &cpp_dev->cpp_open_cnt);
...
static int cpp_close_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	/* Jeff added for VIVO - Begin */
	struct v4l2_fh *vfh = NULL;
	/* Jeff added for VIVO - End */
	if (!sd) {
	...
	if (atomic_read(&cpp_dev->cpp_open_cnt) == 0) {
		mutex_unlock(&cpp_dev->mutex);
		return 0;
	}
	...
	for (i = 0; i < MAX_ACTIVE_CPP_INSTANCE; i++) {
		if ((cpp_dev->cpp_subscribe_list[i].active == 1) &&
				/* Jeff added for VIVO - Begin */
				(cpp_dev->cpp_subscribe_list[i].vfh) == vfh) {
				/* Jeff added for VIVO - End */
			cpp_dev->cpp_subscribe_list[i].active = 0;
			cpp_dev->cpp_subscribe_list[i].vfh = NULL;
			break;
	...
	if (atomic_read(&cpp_dev->cpp_open_cnt) == 0) {
		...
		atomic_set(&cpp_dev->stream_cnt, 0);
		atomic_set(&cpp_dev->offline_pproc_cnt, 0);
		atomic_set(&cpp_dev->regular_pproc_cnt, 0);
	...
	msm_cpp_vbif_register_error_handler(cpp_dev,
		...
		/* Jeff added for VIVO - Begin */
		if (vfh->prio == V4L2_PRIORITY_INTERACTIVE) {
			/* FIXME: safe for now */
			if (atomic_read(&cpp_dev->offline_pproc_cnt) > 0)
				atomic_sub(1, &cpp_dev->offline_pproc_cnt);

			if (atomic_read(&cpp_dev->stream_cnt) > 0 && 
					atomic_read(&cpp_dev->offline_pproc_cnt) == 0 &&
					atomic_read(&cpp_dev->regular_pproc_cnt) > 0)
				complete(&cpp_dev->wait_complete);
		} else if (vfh->prio == V4L2_PRIORITY_BACKGROUND) {
			/* FIXME: safe for now */
			if (atomic_read(&cpp_dev->regular_pproc_cnt) > 0)
				atomic_sub(1, &cpp_dev->regular_pproc_cnt);
		}
		/* Jeff added for VIVO - End */
	}
	mutex_unlock(&cpp_dev->mutex);
	...
	uint8_t put_buf)
{
	...
	if (frame_qcmd) {
		...
		msm_cpp_enqueue(&cpp_dev->eventData_q, &event_qcmd->list_eventdata);
	...
	struct msm_queue_cmd *frame_qcmd)
{
	...
	if (cpp_dev->processing_q.len < MAX_CPP_PROCESSING_FRAME) {
		...
		spin_lock_irqsave(&cpp_timer.data.processed_frame_lock, flags);
		msm_cpp_enqueue(&cpp_dev->processing_q,
...
static int msm_cpp_cfg_frame(struct cpp_device *cpp_dev,
	struct msm_cpp_frame_info_t *new_frame, struct v4l2_fh *vfh)
{
	int32_t rc = 0;
	struct msm_queue_cmd *frame_qcmd = NULL;
	uint32_t *cpp_frame_msg;
	...
	/* Jeff added for VIVO - begin */
    mutex_unlock(&cpp_dev->mutex);
	rc = msm_cpp_check_priority_wait(cpp_dev, new_frame->stream_type,
					MSM_CPP_CHECK_PRIORITY_TIMEOUT, vfh); 
    mutex_lock(&cpp_dev->mutex);
...
frame_msg_err:
	kfree(cpp_frame_msg);
	kfree(new_frame);
	printk(KERN_CRIT "%s frame_msg_err, rc = %d\n", __func__, rc);
	return rc;
}
	...
	enum msm_cpp_stream_t stream_type, int timeout, struct v4l2_fh *vfh)
{
	...
	mutex_lock(&cpp_dev->mutex);
	offline_cnt = atomic_read(&cpp_dev->offline_pproc_cnt);
	switch (stream_type) {
	case MSM_CPP_STREAM_REGULAR_PROC: {
		if (offline_cnt > 0) {
			if (offline_cnt == 1)
				init_completion(&cpp_dev->wait_complete);

			/* unlock before sleep and wait */
			mutex_unlock(&cpp_dev->mutex);

			if (wait_for_completion_timeout(&cpp_dev->wait_complete,
				msecs_to_jiffies(timeout)) == 0) {
				printk(KERN_CRIT "REGULAR_PROC timed out\n");
				return -EBUSY;
			}
		mutex_lock(&cpp_dev->mutex);
		}
	...
	case MSM_CPP_STREAM_OFFLINE_PROC:
		vfh->prio = V4L2_PRIORITY_INTERACTIVE;
	break;
...
static int msm_cpp_cfg(struct cpp_device *cpp_dev,
	struct msm_camera_v4l2_ioctl_t *ioctl_ptr, struct v4l2_fh *vfh)
{
	...
	if (!frame) {
		...
		printk(KERN_CRIT "%s call msm_cpp_cfg_frame, type = %d\n", __func__, k_frame_info.stream_type);
		rc = msm_cpp_cfg_frame(cpp_dev, frame, vfh);
		if (rc >= 0) {
...
#ifdef CONFIG_COMPAT
static int msm_cpp_copy_from_ioctl_ptr(struct cpp_device *cpp_dev,
	void *dst_ptr, struct msm_camera_v4l2_ioctl_t *ioctl_ptr)
{
	int ret;
			...
			unsigned int cmd, void *arg)
{
	...
	int rc = 0;
	struct msm_cpp_subdev_data_t *subdev_data =
		(struct msm_cpp_subdev_data_t *)arg;

	if (sd == NULL) {
		pr_err("sd %p\n", sd);
	  printk(KERN_CRIT "subdev_ioctl failed %d\n", __LINE__);
		return -EINVAL;
	}
	cpp_dev = v4l2_get_subdevdata(sd);
	if (cpp_dev == NULL) {
	...
	if (subdev_data == NULL) {
		pr_err("subdev_ioctl failed %d cmd %u", __LINE__, cmd);
		return -EINVAL;
	}

	rc = msm_cpp_validate_input(cmd, subdev_data->arg, &ioctl_ptr);
	if (rc != 0) {
	  printk(KERN_CRIT "subdev_ioctl failed %d\n", __LINE__);
	...
	case VIDIOC_MSM_CPP_CFG:
		rc = msm_cpp_cfg(cpp_dev, ioctl_ptr, subdev_data->vfh);
		break;
	...
	case VIDIOC_MSM_CPP_DELETE_STREAM_BUFF:
		printk(KERN_CRIT "VIDIOC_MSM_CPP_DELETE_STREAM_BUFF\n");
	case VIDIOC_MSM_CPP_APPEND_STREAM_BUFF_INFO:
		printk(KERN_CRIT "VIDIOC_MSM_CPP_APPEND_STREAM_BUFF_INFO\n");
	case VIDIOC_MSM_CPP_ENQUEUE_STREAM_BUFF_INFO: {
		...
		if (sizeof(struct msm_cpp_stream_buff_info_t) !=
			ioctl_ptr->len) {
			CPP_DBG("%s:%d: invalid length\n", __func__, __LINE__);
			return -EINVAL;
		}

		rc = msm_cpp_copy_from_ioctl_ptr(cpp_dev, &u_stream_buff_info,
			ioctl_ptr);
		if (rc) {
			ERR_COPY_FROM_USER();
			return -EINVAL;
		...
		/* Jeff added for VIVO - begin */
		rc = msm_cpp_check_priority_wait(cpp_dev, 
			u_stream_buff_info.stream_type, MSM_CPP_CHECK_PRIORITY_TIMEOUT, 
			subdev_data->vfh);
		if (rc < 0)
			return rc;
		/* Jeff added for VIVO - end */
		...
		if (k_stream_buff_info.num_buffs > MSM_CAMERA_MAX_STREAM_BUF) {
			CPP_DBG("%s:%d: unexpected large num buff requested\n",
				__func__, __LINE__);
			mutex_unlock(&cpp_dev->mutex);
			return -EINVAL;
		}
		...
		if (buff_queue_info == NULL) {
			...
			atomic_add(1, &cpp_dev->stream_cnt);
			CPP_DBG("stream_cnt:%d\n", atomic_read(&cpp_dev->stream_cnt));
			/* Jeff added for VIVO - begin */
			if (u_stream_buff_info.stream_type == MSM_CPP_STREAM_OFFLINE_PROC)
				atomic_add(1, &cpp_dev->offline_pproc_cnt);
			else if (u_stream_buff_info.stream_type == MSM_CPP_STREAM_REGULAR_PROC)
				atomic_add(1, &cpp_dev->regular_pproc_cnt);
			/* Jeff added for VIVO - end   */
		}
		buff_queue_info = msm_cpp_get_buff_queue_entry(cpp_dev,
			((k_stream_buff_info.identity >> 16) & 0xFFFF),
			(k_stream_buff_info.identity & 0xFFFF));
		if (buff_queue_info == NULL) {
			printk("error finding buffer queue entry identity:%d\n",
				k_stream_buff_info.identity);
			kfree(k_stream_buff_info.buffer_info);
			atomic_sub(1, &cpp_dev->stream_cnt);
			...
			if (u_stream_buff_info.stream_type == MSM_CPP_STREAM_OFFLINE_PROC) { 
				...
				if (atomic_read(&cpp_dev->offline_pproc_cnt) == 0 && 
						atomic_read(&cpp_dev->stream_cnt) > 0         &&
						atomic_read(&cpp_dev->regular_pproc_cnt) > 0)
					complete(&cpp_dev->wait_complete);
			} else if (u_stream_buff_info.stream_type == MSM_CPP_STREAM_REGULAR_PROC)
					atomic_sub(1, &cpp_dev->regular_pproc_cnt);
...
STREAM_BUFF_END:
	...
	case VIDIOC_MSM_CPP_DEQUEUE_STREAM_BUFF_INFO: {
		...
		identity = u_stream_buff_info.identity;
	...
	case VIDIOC_MSM_CPP_GET_EVENTPAYLOAD: {
		...
		kfree(event_qcmd);
		mutex_unlock(&cpp_dev->mutex);
	} /* VIDIOC_MSM_CPP_GET_EVENTPAYLOAD */
		break;
	...
	case VIDIOC_MSM_CPP_SET_CLOCK: {
		...
		rc = msm_cpp_copy_from_ioctl_ptr(cpp_dev, &clock_settings, ioctl_ptr);
		if (rc) {
			ERR_COPY_FROM_USER();
			return -EINVAL;
		}
		...
		/* Jeff added for VIVO - begin */
		rc = msm_cpp_check_priority_wait(cpp_dev, 
			clock_settings.stream_type, MSM_CPP_CHECK_PRIORITY_TIMEOUT,
			subdev_data->vfh);
		if (rc < 0)
			return rc;
		/* Jeff added for VIVO - end */
	...
	case MSM_SD_SHUTDOWN: {
		 pr_warn("shutdown cpp node. open cnt:%d\n",
			atomic_read(&cpp_dev->cpp_open_cnt));
		if (atomic_read(&cpp_timer.used))
			pr_debug("Timer state not cleared\n");

		while (atomic_read(&cpp_dev->cpp_open_cnt) != 0)
	...
	case VIDIOC_MSM_CPP_QUEUE_BUF: {
		...
		rc = msm_cpp_copy_from_ioctl_ptr(cpp_dev, &queue_buf_info, ioctl_ptr);
		if (rc) {
			ERR_COPY_FROM_USER();
			break;
		}
		...
		/* Jeff added for VIVO - begin */
		rc = msm_cpp_check_priority_wait(cpp_dev, 
			queue_buf_info.stream_type, MSM_CPP_CHECK_PRIORITY_TIMEOUT,
			subdev_data->vfh);
		if (rc < 0)
			return rc;
		/* Jeff added for VIVO - end */
	...
	case VIDIOC_MSM_CPP_POP_STREAM_BUFFER: {
		...
		rc = msm_cpp_copy_from_ioctl_ptr(cpp_dev, &frame_info, ioctl_ptr);
		if (rc) {
			ERR_COPY_FROM_USER();
			break;
		}
		...
		/* Jeff added for VIVO - begin */
		rc = msm_cpp_check_priority_wait(cpp_dev,
			frame_info.stream_type,	MSM_CPP_CHECK_PRIORITY_TIMEOUT,
			subdev_data->vfh);
		if (rc < 0)
			return rc;
		/* Jeff added for VIVO - end */
		...
		mutex_unlock(&cpp_dev->mutex);
	}  /* VIDIOC_MSM_CPP_POP_STREAM_BUFFER */
		break;

	/* Jeff added for VIVO - begin */
	case VIDIOC_MSM_CPP_GET_IMMU_STATE: {
		mutex_lock(&cpp_dev->mutex);
		if (copy_to_user((void __user *)ioctl_ptr->ioctl_ptr, &cpp_dev->iommu_state,
			sizeof(enum cpp_iommu_state))) { 
			/* sizeof(enum msm_cpp_iommu_state_t))) { */
		  CPP_DBG("VIDIOC_MSM_CPP_GET_IMMU_STATE failed %d\n", cpp_dev->iommu_state);
			mutex_unlock(&cpp_dev->mutex);
			return -EFAULT;
		}
		mutex_unlock(&cpp_dev->mutex);
	} /* VIDIOC_MSM_CPP_GET_IMMU_STATE */
	/* Jeff added for VIVO - end   */
		break;
	default:
	...
	case VIDIOC_MSM_CPP_IOMMU_ATTACH: {
		if (cpp_dev->iommu_state == CPP_IOMMU_STATE_DETACHED) {
			...
			rc = msm_cpp_copy_from_ioctl_ptr(cpp_dev, &cpp_attach_info,
				ioctl_ptr);
			if (rc < 0) {
	...
	struct file *file, unsigned int cmd, void *arg)
{
	...
	default:
		subdev_data.arg = arg;
		subdev_data.vfh = vfh;
		return v4l2_subdev_call(sd, core, ioctl, cmd, &subdev_data);
	}
	...
	struct msm_camera_v4l2_ioctl_t *kp_ioctl)
{
	...
	new_frame->stream_type = new_frame32->stream_type;
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case VIDIOC_MSM_CPP_CFG32:
	{
		...
		if (cpp_frame) {
			rc = msm_cpp_cfg_frame(cpp_dev, cpp_frame, vfh);
	...
	case VIDIOC_MSM_CPP_GET_IMMU_STATE32: {

		if (copy_to_user((void __user *)kp_ioctl.ioctl_ptr,
			&cpp_dev->iommu_state, sizeof(enum cpp_iommu_state))) {
			mutex_unlock(&cpp_dev->mutex);
			return -EFAULT;
		}

		cmd = VIDIOC_MSM_CPP_GET_IMMU_STATE;
  }
		break;
	...
	case VIDIOC_MSM_CPP_DELETE_STREAM_BUFF32:
	{
		...
		get_user(k_cpp_buff_info.stream_type,
			&u32_cpp_buff_info->stream_type);
	...
	case VIDIOC_MSM_CPP_SET_CLOCK32: {
		...
		get_user(clock_settings.stream_type,
			&clock_settings32->stream_type);
	...
	case VIDIOC_MSM_CPP_QUEUE_BUF32: {
		...
		get_user(k_queue_buf.stream_type,
			&u32_queue_buf->stream_type);
	...
	case VIDIOC_MSM_CPP_IOMMU_DETACH:
	case MSM_SD_SHUTDOWN:
		subdev_data.arg = &kp_ioctl;
		subdev_data.vfh = vfh;
		rc = v4l2_subdev_call(sd, core, ioctl, cmd, &subdev_data);
		break;
...
static int cpp_probe(struct platform_device *pdev)
{
	...
	int i = 0;
	int rc = 0;
  /* Jeff added for CPP -- begin */
	uint32_t session_id = 0;

  /* Jeff added for CPP -- begin */
	rc = camera_init_v4l2(&pdev->dev, &session_id, CAMERA_V4L2_DUMMY);
	if (rc < 0) {
		pr_err("failed: camera_init_v4l2 rc %d", rc);
		return rc;
	}

	rc = camera_init_v4l2(&pdev->dev, &session_id, CAMERA_V4L2_PPROC);
	if (rc < 0) {
		pr_err("failed: camera_init_v4l2 rc %d", rc);
		return rc;
	}
  /* Jeff added for CPP -- end   */
	...
	atomic_set(&cpp_dev->cpp_open_cnt, 0);
