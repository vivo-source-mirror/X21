...
static atomic_t msm_pm_qos_cnt;
...
static inline int __msm_sd_close_subdevs(struct msm_sd_subdev *msm_sd,
	struct msm_sd_close_ioctl *sd_close, enum msm_camera_close_t type)
{
	struct v4l2_subdev *sd;
	sd = &msm_sd->sd;
	pr_debug("%s: Shutting down subdev %s", __func__, sd->name);

	/* Do nothing for CPP close, because subdev close handles it */
	if ((type == MSM_CAMERA_CLOSE_CPP) &&
			(sd->entity.group_id == MSM_CAMERA_SUBDEV_CPP)) {
		return 0;
	} else if ((type == MSM_CAMERA_CLOSE_SENSOR) &&
						 (sd->entity.group_id != MSM_CAMERA_SUBDEV_CPP)) {
...
static int msm_open(struct file *filep)
{
	...
	/* Jeff: moved this to probe msm_pm_qos_add_request(); */
...
#endif
void msm_camera_open_check(struct device *dev)
{
	/* register msm_v4l2_pm_qos_request */
	if (atomic_read(&msm_pm_qos_cnt) == 0) {
		msm_pm_qos_add_request();
		pm_stay_awake(dev);
		msm_pm_qos_update_request(CAMERA_ENABLE_PC_LATENCY);
	}

	atomic_add(1, &msm_pm_qos_cnt);
}
EXPORT_SYMBOL(msm_camera_open_check);

void msm_camera_close_check(struct device *dev,
	enum msm_camera_close_t type)
{
	...
	if (atomic_read(&msm_pm_qos_cnt) == 0) {
		...
		if (!list_empty(&msm_v4l2_dev->subdevs))
			list_for_each_entry(msm_sd, &ordered_sd_list, list)
				__msm_sd_close_subdevs(msm_sd, &sd_close, type);
...
EXPORT_SYMBOL(msm_camera_close_check);
...
/* Jeff added here: No need to keep  - begin */
	rc = video_register_device(pvdev->vdev,
		VFL_TYPE_GRABBER, -1);
	if (WARN_ON(rc < 0))
		goto v4l2_fail;
 /* Jeff added here: No need to keep  - end */
