...
#if defined(PD1809) || defined(PD1809F_EX)
static int sar_gpio_num = 1;
#else
static int sar_gpio_num = 2;
#endif
bool sar_gpio_flag = true;

static void sar_power_irq_work(struct work_struct *work)
{
	int state;
	int state_1 = 0; /*added for irq_1*/
	int state_2 = 0; /*added for irq_1*/
	int state_3 = 0; /*added for irq_1*/
	...
	if (sarpower_state == 1) {
		...
		if (sarpower_data->rf_detect_gpio_1 >= 0) {
			ret = request_irq(sarpower_data->irq_1, sar_power_irq_handler,
							  IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, sarpower_data->sdev.name, sarpower_data);
			if (ret < 0) {
				printk(KERN_ERR "[%s]:[%s] request irq_1 %d err %d\n", TAG, __func__, sarpower_data->irq_1, ret);
				return;
			}
		}
...
static int sar_power_sarpower_probe(struct platform_device *pdev)
{
	...
	sarpower_data->enable = -1;
	/*sarpower_data->rf_detect_gpio = of_get_named_gpio_flags(
	...
	if (sarpower_data->rf_detect_gpio < 0) {
		printk(KERN_ERR "[%s]:[%s] It is not config sar gpio %d\n", TAG, __func__, sarpower_data->rf_detect_gpio);
		return 0;
	}
	/*ret = of_property_read_u32(pdev->dev.of_node, "sar-power-rf-detect_1,gpios", &sarpower_data->rf_detect_gpio_1);*/
	sarpower_data->rf_detect_gpio_1 = of_get_named_gpio(pdev->dev.of_node, "sar-power-rf-detect_1,gpios", 0);
	if (sarpower_data->rf_detect_gpio_1 < 0) {
		printk(KERN_ERR "[%s]:[%s] It is not the second gpio\n", TAG, __func__);
		/*sar_gpio_flag = false;*/
	}
