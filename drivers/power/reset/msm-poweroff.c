...
__attribute__((unused)) static void set_dload_mode_bbk(int on)
{
	...
	if (dload_mode_addr) {
		...
		__raw_writel(on ? 0xCE14091A : 0,
			dload_mode_addr + sizeof(unsigned int));
		*/
		/* conflict with dload_type */
		/*cookie_ptr->uefi_ram_dump_magic, equal to (*dload_type_addr), 32bit*/
		__raw_writel(on ? 0x2/*EMMC_DLOAD_TYPE*/ : 0xffffffff,
			dload_mode_addr + 7*sizeof(u32));
		...
		__raw_writeq(__pa(&log_first_idx),
			dload_mode_addr + 20*sizeof(u32));
		/*cookie_ptr->next_log, 64bit*/
		__raw_writeq(__pa(&log_next_idx),
			dload_mode_addr + 22*sizeof(u32));
		/*cookie_ptr->log_buf_len, 32bit*/
		__raw_writel(log_buf_len_get(),
			dload_mode_addr + 24*sizeof(u32));
		mb();
	}

	{
		int ret = scm_set_dload_mode(on ? SCM_DLOAD_FULLDUMP : 0, 0);
		if (ret)
			pr_err("Failed to set secure DLOAD mode: %d\n", ret);
	}
...
static int force_warm_reset(const char *val, struct kernel_param *kp)
{
...
static void __iomem *of_iomap_imem_dload_mode(struct device_node *np, int index)
{
	struct resource res;
	...
	/*默认"qcom,msm-imem-download_mode"范围大小为8字节*/
	if (resource_size(&res) < 8)
		return NULL;

	return ioremap(res.start, 8);
}
...
static int msm_restart_probe(struct platform_device *pdev)
{
	...
	if (!dload_mode_addr) {
		...
		if (np) {
			dload_mode_addr = of_iomap_imem_dload_mode(np, 0);
		}
	}
