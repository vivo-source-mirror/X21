...
/*add by vivo sensor team for haptic start*/
#include <linux/of_gpio.h>
#include <linux/gpio.h>
#include <linux/slab.h>
#include <linux/taskstats.h>
#include <linux/sched.h>
/*add by vivo sensor team for haptic end*/
...
struct qpnp_hap {
	...
	u32				sc_irq;
	u32				status_flags;
	u32				enable_gpio; /*dengkangkai added for vivo vibrator*/
	u16				base;
	u16				last_rate_cfg;
...
static void qpnp_hap_td_enable(struct timed_output_dev *dev, int time_ms)
{
	...
	int rc;
	if (time_ms < 0) {
		/* dengkangkai added for vivo vibrator */
		if (gpio_is_valid(hap->enable_gpio)) {
			rc = gpio_direction_output(hap->enable_gpio, 0);
			if (rc)
				printk(KERN_ERR "%s Unable to set enable gpio: %d to 0 \n", __func__, hap->enable_gpio);
		}
		/* dengkangkai add end */
		return;
	}
	printk(KERN_ERR "vibrator, the haptics time is %d , pid is %d\n", time_ms, current->pid);
    /*add for gpio control haptic start*/
    if (time_ms == 0) {
		/* dengkangkai added for vivo vibrator */
		if (gpio_is_valid(hap->enable_gpio)) {
			rc = gpio_direction_output(hap->enable_gpio, 0);
   ...
   /*add for gpio control haptic end*/
	mutex_lock(&hap->lock);
	...
	if (!hap->state) {
	 ...
	 /* dengkangkai added for vivo vibrator */
	if (gpio_is_valid(hap->enable_gpio)) {
		rc = gpio_direction_output(hap->enable_gpio, 1);
		if (rc)
			printk(KERN_ERR "%s Unable to set enable gpio: %d to 1 \n", __func__, hap->enable_gpio);
	}
	/* dengkangkai add end */
	mutex_unlock(&hap->lock);
...
static int qpnp_hap_parse_dt(struct qpnp_hap *hap)
{
	...
	/*change by vivo team sensor added for vivo vibrator*/
	hap->enable_gpio = of_get_named_gpio(pdev->dev.of_node, "vivo,vibrator_enable", 0);
	pr_err("kernel haptic enable gpio is %d\n", hap->enable_gpio);
	if (hap->enable_gpio < 0)
		hap->enable_gpio = -1;
	/* change by vivo team sensor  added end */
	hap->wave_shape = QPNP_HAP_WAV_SQUARE;
	rc = of_property_read_string(pdev->dev.of_node,
