...
static struct work_struct adsp_log_work;
...
static void log_failure_reason(const struct pil_tz_data *d)
{
	...
	if (name && !strcmp(name, "adsp") && (strnstr(reason, "adsp_mi2s_count", strlen(reason)) ||
			(!strnstr(reason, "adsp_stress_test", strlen(reason))))) {
		memset(crash_reason, '\0', MAX_SSR_REASON_LEN);
		strlcpy(crash_reason, smem_reason, min(size, MAX_SSR_REASON_LEN - 1));
		schedule_work(&adsp_log_work);
	}
...
static void adsp_crash_log_collect(struct work_struct *work)
{

	pr_err("%s adsp subsystem failure reason: %s.\n",
			__func__, crash_reason);
	return;
}

static int __init pil_tz_init(void)
{
	INIT_WORK(&adsp_log_work, adsp_crash_log_collect);
