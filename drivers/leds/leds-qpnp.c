...
#include <linux/delay.h>
/*add by vivo sensor team start*/
#include <linux/gpio.h>
/*add by vivo sensor team end*/
...
struct qpnp_led_data {
	...
	int				enable_gpio; /*dengkangkai added for vivo led*/
	int				duty_cycle; /*dengkangkai added for vivo current*/
};
	...
	if (led->cdev.brightness) {
		...
		if (led->enable_gpio > 0) {
			...
			if (led->rgb_cfg->pwm_cfg->mode == LPG_MODE) {
				...
				led->cdev.brightness = led->duty_cycle;
			} else {
				led->rgb_cfg->pwm_cfg->pwm_period_us = 2000;
			}
		}
		/* dengkangkai add end */
		...
		if (led->rgb_cfg->pwm_cfg->mode == PWM_MODE) {
			...
			if (period_us > INT_MAX / NSEC_PER_USEC) {
				...
				rc = pwm_config(
					led->rgb_cfg->pwm_cfg->pwm_dev,
					duty_ns,
					period_us * NSEC_PER_USEC + 10);/*dengkangkai modified for blink error; please refer to qpnp_pwm_config*/
			}
			if (rc < 0) {
				dev_err(&led->pdev->dev,
					"pwm config failed\n");
		...
		/* dengkangkai added for vivo led */
		if (led->enable_gpio > 0)
				...
				enum led_brightness value)
{
	...
	/*dengkangkai added for vivo led*/
	if (led->duty_cycle > 0 && value > 0)
		value = led->duty_cycle;
...
static int qpnp_leds_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(node, temp) {
		...
		dev_err(&led->pdev->dev, "The linux name is %s\n", led->cdev.name);
		...
		/* dengkangkai added for vivo led */
		rc = of_property_read_u32(temp, "vivo,led_enable", &led->enable_gpio);
		if (rc < 0) {
			dev_err(&led->pdev->dev,
				"Failure reading enable gpio, rc = %d\n", rc);
			led->enable_gpio = -1;
		}
		rc = of_property_read_u32(temp, "vivo,duty-cycle", &led->duty_cycle);
		if (rc < 0) {
			dev_err(&led->pdev->dev,
				"Failure reading duty-cycle, rc = %d\n", rc);
			led->duty_cycle = -1;
		}
		/* dengkangkai added end */
	...
	/*add by vivo sensor change for PD1709 light power */
	rc = qpnp_led_masked_write(led, led->base + 0x45, 0x03, 0x03);
	if (rc) {
	  dev_err(&led->pdev->dev, "Failed to write led LED_SRC_SEL reg\n");
	}
	/*add by vivo sensor change for PD1709 light power end */
