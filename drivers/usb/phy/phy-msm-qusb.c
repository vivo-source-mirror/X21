...
struct qusb_phy {
	...
	int			tune2_efuse_correction_temp;
...
static void qusb_phy_get_tune2_param(struct qusb_phy *qphy)
{
	...
	if (qphy->phy.otg_host_state) {
		if (qphy->tune2_efuse_correction_temp && qphy->tune2_val) {
			if (qphy->tune2_efuse_correction_temp > 5 ||
					qphy->tune2_efuse_correction_temp < -10)
				pr_warn("Correction value is out of range : %d\n",
						qphy->tune2_efuse_correction_temp);
			else
				qphy->tune2_val = qphy->tune2_val +
							qphy->tune2_efuse_correction_temp;
		}
		printk("qusb_phy_init:usb parameter separation @usb second paragram \n");
	} else {
		...
		printk("qusb_phy_init:usb parameter separation @usb default paragram \n");
	}
...
static int qusb_phy_probe(struct platform_device *pdev)
{
	...
	if (res) {
		...
		if (!IS_ERR_OR_NULL(qphy->tune2_efuse_reg)) {
			...
			of_property_read_u32(dev->of_node,
						"qcom,tune2-efuse-temp-correction",
						&qphy->tune2_efuse_correction_temp);
