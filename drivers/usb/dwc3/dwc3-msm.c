...
static int msm_otg_read_usbid_voltage(struct dwc3_msm *motg)
{
	int rc = 0;
	int usbid_mv = USBID_DEFAULT_VALUE_MV;
	struct qpnp_vadc_result results;
	struct device_node *np = motg->dev->of_node;
	u32 channel_num;

	mutex_lock(&motg->vadc_lock);
	if (IS_ERR_OR_NULL(motg->vadc_dev)) {
		motg->vadc_dev = qpnp_get_vadc(motg->dev, "usbidvol");
		if (IS_ERR(motg->vadc_dev)) {
			pr_debug("adc not register,default value used usbid_mv=%d\n", usbid_mv);
			mutex_unlock(&motg->vadc_lock);
			return usbid_mv;
		}
	}
	...
	rc = qpnp_vadc_read(motg->vadc_dev, channel_num, &results);
	if (rc) {
...
static void otg_disable_func(unsigned long data)
{
	...
	/*if(motg->otg_state == OTG_STATE_B_PERIPHERAL)
			return;
	pm_runtime_resume(motg->dev);
	...
	if (host_mode_enabled ^ enabled) {
		...
		if (enabled) {
			if (motg->usbid_adc_used) {
				...
				pr_warn("usb phy otg init otg_switch_disable_timer\n");
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk(KERN_ALERT "%s: write valueL %s\n", __func__, value);
...
static int dwc3_msm_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&mdwc->switchlock);
