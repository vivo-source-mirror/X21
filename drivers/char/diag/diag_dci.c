...
static int diag_dci_init_buffer(struct diag_dci_buffer_t *buffer, int type)
{
	...
	case DCI_BUF_CMD:
		...
		buffer->data = diagmem_alloc(driver, buffer->capacity,
						POOL_TYPE_DCI_CAPACITY);
		/*add by rongqianfeng for kmalloc faild end*/
		if (!buffer->data) {
			pr_err("diag: In %s, diagmem_alloc error\n", __func__);
			return -ENOMEM;
		}
		break;
	default:
...
int diag_dci_init(void)
{
	...
	if (driver->apps_dci_buf == NULL) {
		/*add by rongqianfeng for kmalloc faild start*/
		driver->apps_dci_buf = diagmem_alloc(driver, DCI_BUF_SIZE,
					  POOL_TYPE_DCI_BUF);
		/*add by rongqianfeng for kmalloc faild end*/
		if (driver->apps_dci_buf == NULL) {
			pr_err("diag: In %s, diagmem_alloc error\n", __func__);
...
err:
	...
	diagmem_free(driver, driver->apps_dci_buf, POOL_TYPE_DCI_BUF);
...
fail_alloc:
	pr_err("diag: In %s, alloc mem error\n", __func__);
	if (new_entry) {
		for (i = 0; ((i < new_entry->num_buffers) &&
			...
			if (proc_buf) {
				...
				if (proc_buf->buf_cmd) {
					diagmem_free(driver, proc_buf->buf_cmd->data, POOL_TYPE_DCI_CAPACITY);
