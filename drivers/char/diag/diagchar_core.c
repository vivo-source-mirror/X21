...
/*add by rongqianfeng for kmalloc faild start*/
static void vivo_diag_mempool_init(void)
{

	uint32_t itemsize_dci_buf = DCI_BUF_SIZE;
	uint32_t itemsize_capacity = DIAG_MAX_REQ_SIZE + DCI_BUF_SIZE;

	diagmem_setsize(POOL_TYPE_DCI_BUF, itemsize_dci_buf, 1);
	diagmem_setsize(POOL_TYPE_DCI_CAPACITY, itemsize_capacity, 7);

	diagmem_init(driver, POOL_TYPE_DCI_BUF);
	diagmem_init(driver, POOL_TYPE_DCI_CAPACITY);
}
/*add by rongqianfeng for kmalloc faild end*/
