...
/*add by rongqianfeng for kmalloc faild start*/
	...
	{
		.id = POOL_TYPE_DCI_CAPACITY,
		.name = "POOL_DCI_CAPACITY",
		.pool = NULL,
		.itemsize = 0,
		.poolsize = 0,
		.count = 0
...
void diagmem_setsize(int pool_idx, int itemsize, int poolsize)
{
	...
	pr_err("diag: Mempool %s sizes: itemsize %d poolsize %d\n",
...
void *diagmem_alloc(struct diagchar_dev *driver, int size, int pool_type)
{
	...
	if (!buf)
		pr_err_ratelimited("diag: In %s, pool id is error, pool_type %d\n", __func__, pool_type);
	return buf;
}
