				...
				bool cd_irq)
{
	...
	printk(KERN_ERR "%s: mmc_schedule_delayed_work ret = %d\n", mmc_hostname(host), ret);
}
...
void mmc_rescan(struct work_struct *work)
{
	...
	printk(KERN_ERR "%s === %s === rescan_disable=%d bus_dead=%d\n",
    mmc_hostname(host),  __func__, host->rescan_disable, host->bus_dead);
