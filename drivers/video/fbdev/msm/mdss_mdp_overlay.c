				...
				struct mdp_display_commit *data)
{
	...
	atomic_set(&mfd->ud_done, 0);
	if (g_ctrl_pdata)
	mutex_lock(&g_ctrl_pdata->hbm_state_lock);
	/*wake_up_all(&mfd->ud_flag_q);*/
	if (mfd->ud_flag == 1) {
...
int mdss_mdp_overlay_vsync_ctrl(struct msm_fb_data_type *mfd, int en)
{
	...
	if (!ctl->panel_data->panel_info.cont_splash_enabled &&
		...
		if (mdss_panel_is_power_on_ulp(ctl->power_state) ||
		   mdss_panel_is_power_on_lp(ctl->power_state))
			rc = 0;
		else
			rc = -EPERM;
		if (rc)
