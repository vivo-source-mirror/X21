		...
		struct dsi_shared_data *sdata)
{
	...
	for (i = DSI_CORE_PM; !rc && (i < DSI_MAX_PM); i++) {
		...
		pr_err("%s: success to init vregs for %s,name %s,min_voltage %d,max_voltage %d\n",
				__func__, __mdss_dsi_pm_name(i), sdata->power_data[i].vreg_config->vreg_name,
					sdata->power_data[i].vreg_config->min_voltage,
					sdata->power_data[i].vreg_config->max_voltage);
	}
...
static irqreturn_t test_hw_vsync_handler(int irq, void *data)
{
	...
	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
		if (ctrl_pdata) {
			if (&ctrl_pdata->ud_hbm_sync != NULL) {
			if (ctrl_pdata->ud_hbm_sync.te_con) {
			pr_err("UD dsi-ud--te_con hw_vsync_handler = %d\n", ctrl_pdata->ud_hbm_sync.te_con);
			ctrl_pdata->ud_hbm_sync.te_con = 0;
			wake_up_all(&ctrl_pdata->ud_hbm_sync.te_wq);
			}

			if (ctrl_pdata->ud_hbm_sync.te_con_use_vblank)
			...
			u32 bitrate)
{
	...
	pr_err("%s: ndx=%d switching to clk_rate=%d\n", __func__,
			ctrl->ndx, bitrate);

	mutex_lock(&ctrl->dynamic_clk_change_lock);
	...
	mutex_unlock(&ctrl->dynamic_clk_change_lock);
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_UNBLANK:
		...
		end_time = ktime_get();
		pr_err("mdss_dsi_unblank. Delay: %llu\n", (ktime_to_ms(end_time) - ktime_to_ms(start_time)));
		break;
	...
	case MDSS_EVENT_PANEL_UPDATE_DSI_TIMING:
		...
		end_time = ktime_get();
		pr_err("UPDATE_DSI_TIMING. Delay: %llu\n", (ktime_to_ms(end_time) - ktime_to_ms(start_time)));
		pr_err("Update dsi timing to %d\n",
				(u32) (unsigned long) arg);
		break;
...
static char *hbm_state_str = ":hbm_state=0x";
static char hbm_statesave_str[30] = {0};

int lcm_especial_id;
static char *lcm_especial_id_str = ":lcm_especial_id=0x";
static char lcm_especial_idsave_str[30] = {0};

int lcm_software_id = 0xFF;
	...
	if (!len) {
		...
		if (!str2) {
			...
			if (lcmid) {
				...
				strlcpy(lcm_software_id_str, lcmid, 4);
				lcm_software_id = str2int(lcm_software_id_str);
			} else {
				lcm_software_id = 0xFF;
			}
			/*add for lcm max brightness calibration start*/
			temp = strnstr(panel_cfg, lcm_max_brightness_str, len);
			if (temp) {
				temp += strlen(lcm_max_brightness_str);
				strlcpy(lcm_max_backlight_str, temp, 3);
				lcm_max_brightness = str2int(lcm_max_backlight_str);
			} else {
				lcm_max_brightness = 0x00;  /* errid: indicate no lcm softid send from LK*/
				}

			if (hw_subtype == 20 || hw_subtype == 21) {
				pr_err("%s: Lcm brightness calibration, set lcm_max_brightness \n", __func__);
				if (lcm_max_brightness < 51)/* less than 510nit ,don't need to calibrate*/
				panel_max_bl_level = 2047;
				else if (lcm_max_brightness <= 52)
					panel_max_bl_level = 2035;
				else if (lcm_max_brightness <= 54)
					panel_max_bl_level = 2028;
				else if (lcm_max_brightness <= 56)
					panel_max_bl_level = 2016;
				else if (lcm_max_brightness <= 58)
					panel_max_bl_level = 2003;
				else
					panel_max_bl_level = 2047;
				}
			/*add for lcm max brightness calibration end*/

			/*add for lcm special id start*/
			temp = strnstr(panel_cfg, lcm_especial_id_str, len);
			if (temp) {
				temp += strlen(lcm_especial_id_str);
				strlcpy(lcm_especial_idsave_str, temp, 3);
				lcm_especial_id = str2int(lcm_especial_idsave_str);
			} else {
				lcm_especial_id = 0x00;  /* errid: indicate no lcm softid send from LK*/
				}
			/*add for lcm special id end*/

			/*add for hbm state start*/
			temp = strnstr(panel_cfg, hbm_state_str, len);
			if (temp) {
				temp += strlen(hbm_state_str);
				strlcpy(hbm_statesave_str, temp, 3);
				hbm_value = str2int(hbm_statesave_str);
			}
			pr_err("dsi hbm_value = 0x%x\n", hbm_value);
			if (hbm_value == 0x00)
				hbm_state = 1;
			else
				hbm_state = 0;
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	if (pdata->panel_info.type == MIPI_CMD_PANEL) {
		...
		ctrl_pdata->ud_hbm_sync.panel_te_gpio = pdata->panel_te_gpio;
	}
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->double_enable_gpio = of_property_read_bool(ctrl_pdev->dev.of_node,
				"qcom,platform-enp-enn-gpio");/*///lrz add 2018 0409*/

	ctrl_pdata->vddio_1v8_enable_gpio = of_property_read_bool(ctrl_pdev->dev.of_node,
				"qcom,platform-vddio1v8-gpio");
	if (ctrl_pdata->double_enable_gpio) {
		ctrl_pdata->disp_enp_gpio = of_get_named_gpio(/*enp*/
	...
	/*lrz add for incell LCM 2018 0409*/
	...
	ctrl_pdata->oled_3v3_en_gpio = of_get_named_gpio(
			ctrl_pdev->dev.of_node,
			"qcom,platform-oled-3v3-enable-gpio", 0);

	if (!gpio_is_valid(ctrl_pdata->oled_3v3_en_gpio))
			pr_err("%s:%d, oled 3.3v enable gpio not specified\n",
					__func__, __LINE__);
	return 0;
}
