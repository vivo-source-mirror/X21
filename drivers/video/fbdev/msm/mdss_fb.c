...
extern unsigned int is_atboot;/*///lrz add for backlight  control in AT mode*/
extern int pre_bkg;
...
int  panel_lpm_power_state = MDSS_PANEL_POWER_ON;
extern struct work_struct hbm_disable_status;
extern int resume_hbm_state;
...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	if (is_atboot != 1) {/*///lrz add for backlight  control in AT mode*/
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		if (mfd->bl_level != bkl_lvl)
			...
			mfd->bl_level = bkl_lvl;
			mfd->bl_level_scaled = bkl_lvl; /* txj modify for silent reboot 20170519 */
		}
			     ...
			     int op_enable)
{
	...
	case FB_BLANK_UNBLANK:
		panel_lpm_power_state = MDSS_PANEL_POWER_ON;
	...
	case BLANK_FLAG_ULP:
		...
		panel_lpm_power_state = MDSS_PANEL_POWER_LP2;
		break;
	case BLANK_FLAG_LP:
		...
		ret = mdss_fb_blank_blank(mfd, req_power_state);
		panel_lpm_power_state = MDSS_PANEL_POWER_LP1;
		break;
...
static int mdss_fb_register(struct msm_fb_data_type *mfd)
{
	...
	init_waitqueue_head(&mfd->ud_flag_q);
...
static int mdss_fb_release_all(struct fb_info *info, bool release_all)
{
	...
	if (!mfd->ref_cnt || release_all) {
		...
		mutex_unlock(&mfd->bl_lock);
		cancel_work_sync(&hbm_disable_status);
	...
	unsigned long *argp, struct file *file)
{
	...
	if (commit.ud_flag == 1 || commit.ud_flag == 2) {
		if (ud_flag_old != commit.ud_flag) {
			ud_flag_old = commit.ud_flag;
			mfd->ud_flag = commit.ud_flag;
			if (mfd->ud_flag == 1)
				resume_hbm_state = 1;
			if (mfd->ud_flag == 2)
				resume_hbm_state = 2;
