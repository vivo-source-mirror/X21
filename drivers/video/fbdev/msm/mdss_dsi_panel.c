...
/* AT mode */
extern int lcm_especial_id;
extern int hbm_state;
int hbm_gamma_state;
int force_dimmng_control = 1;
int resume_hbm_state;
int panel_max_bl_level = 4095;
int lm3697_cabc_enable_level = 1574;
/*int wled_cabc_enable_level = 900;
int vddio_off_enable;*/
int pre_cabc_level = 1;
...
int oled_acl_state;
int oled_hbm_state;
int oled_hbm_state_ud;
int oled_hbm_on;
int panel_esd_status_state;
int panel_id;
int panel_backlight_on = 1;
unsigned int mdss_report_lcm_id(void);
unsigned int mdss_report_lcm_mipi_data_lane_number(void);
struct mdss_dsi_ctrl_pdata *g_ctrl_pdata;
/* indicate phone shut down state */
extern unsigned int phone_shutdown_state;  /* 0: no shutdown, 1: shutdown*/
int panel_on_state = 1;  /* 0: state off; 1: state on;*/
/* indicate panel off state */
int panel_off_state; /* 0: panel not off,  1:panel off*/
int oled_alpm_state;
int oled_hlpm_state;
...
int vivo_sre_enable;
int panel_backlight_hbm;
struct timer_list hbm_timer;
struct work_struct hbm_disable_status;
#define HBM_EXPIRES_TIME 600

static int oled_dimming_enable;
static int doze_mode_state;
int pre_bkg = 2000;
int dimming_state;
int brightness_compensate_state;
extern int panel_lpm_power_state;
static atomic_t panel_init_state = ATOMIC_INIT(1); /*0: not init; 1: power done*/
int hbm_send_state;
#define VIVO_WHITE_LCM_TYPE  0x00
#define VIVO_BLACK_LCM_TYPE   0x01
int ud_hbm_state;

/*dyh add for dynamic adjust mipi clk by setting porch*/
int dynamic_mipi_clk_enable_status;
int h_front_porch_state;
int h_back_porch_state;
int h_pulse_width_state;
int v_front_porch_state;
int v_back_porch_state;
int v_pulse_width_state;
int lcm_mipi_data_lane_number;

int color_temperature_state;
int ud_fingerprint_state;
/*unsigned int lcm_highbrightness_status;*/
int ud_fingerprint_off_state;
int resume_hbm_state;

struct work_struct hbm_on_status;
struct work_struct delay_on_status;
struct work_struct ps_off_status;
...
/*10bit dcs backlight beging*/
static char led_pwm2[3] = {0x51, 0x0, 0x0};	/* DTYPE_DCS_WRITE1 */
static struct dsi_cmd_desc backlight_cmd_10bit = {
	{DTYPE_DCS_LWRITE, 1, 0, 0, 1, sizeof(led_pwm2)},
	led_pwm2
};

static void mdss_dsi_panel_bklt_dcs_10bit(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo;
	...
	led_pwm2[1] = (unsigned char)((level>>8)&0xF);
	led_pwm2[2] = (unsigned char)(level&0xFF);
	pr_err("%s: hgih=0x%x,low = 0x%x\n", __func__, led_pwm2[1], led_pwm2[2]);
	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = &backlight_cmd_10bit;
	cmdreq.cmds_cnt = 1;
	cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
...
/*lrz transplant from PD1809 0720*/
static char samsung_unlock[3] = {0xF0, 0x5A, 0x5A};
static char reg_off_set[2]       = {0xB0, 0x08};
static char reg_hbm_diming[2]       = {0xB7, 0x93};
static char samsung_lock[3] = {0xF0, 0xA5, 0xA5};

static struct dsi_cmd_desc close_elvss_diming_cmd[4] = {
	{{DTYPE_GEN_LWRITE, 0, 0, 0, 0, sizeof(samsung_unlock)}, samsung_unlock},
	{{DTYPE_GEN_LWRITE, 0, 0, 0, 0, sizeof(reg_off_set)}, reg_off_set},
	{{DTYPE_GEN_LWRITE, 0, 0, 0, 0, sizeof(reg_hbm_diming)}, reg_hbm_diming},
	{{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(samsung_lock)}, samsung_lock},
};

static int dsi_panel_update_elvss_dimming(struct mdss_dsi_ctrl_pdata *ctrl, int enable)
{
	int rc = 0;
	...
	if (enable)
		elvss_dimming_state = lcm_especial_id | 0x80;
	else
		elvss_dimming_state = lcm_especial_id & 0x7F;

	reg_hbm_diming[1] = elvss_dimming_state;
	close_elvss_diming_cmd[2].payload = reg_hbm_diming;

	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = close_elvss_diming_cmd;
	cmdreq.cmds_cnt = 4;
	...
	if (rc < 0)
		pr_err("failed to update elvss_dimming_state:0x%x lcm_especial_id:0x%x \n", elvss_dimming_state, lcm_especial_id);
...
struct panel_ud_hbm_sync *get_ud_hbm_sync(void)
{
	if (g_ctrl_pdata != NULL)
		return &g_ctrl_pdata->ud_hbm_sync;
	else
		return NULL;
}
static void hbm_on_setting(struct work_struct *work)
{
	struct dsi_panel_cmds *hbm_on_cmds_ud = &g_ctrl_pdata->hbm_ud_fingerprint_cmds;
	if (hbm_on_cmds_ud->cmd_cnt)
		mdss_dsi_panel_cmds_send(g_ctrl_pdata, hbm_on_cmds_ud, CMD_REQ_COMMIT);
	pr_err("dsi-ud---func: %s;\n", __func__);
}

static void delay_on_setting(struct work_struct *work)
{
	struct dsi_panel_cmds *delay_on_cmds_ud = &g_ctrl_pdata->dsi_delay_on_cmds;
	mdelay(17);
	if (delay_on_cmds_ud->cmd_cnt)
		mdss_dsi_panel_cmds_send(g_ctrl_pdata, delay_on_cmds_ud, CMD_REQ_COMMIT);
	pr_err("dsi-ud---func: %s;\n", __func__);
}

static void ps_off_setting(struct work_struct *work)
{
	struct dsi_panel_cmds *dsi_ps_off_cmds = &g_ctrl_pdata->dsi_ps_off_cmds;

	if (dsi_ps_off_cmds->cmd_cnt)
		mdss_dsi_panel_cmds_send(g_ctrl_pdata, dsi_ps_off_cmds, CMD_REQ_COMMIT);
	pr_err("dsi-ud---func: %s; line: %d set hbm on fail---\n", __func__, __LINE__);
	ud_hbm_state = 1;
	oled_dimming_enable = 0;
	oled_hbm_state = 4;
}

int dsi_panel_set_hbm_ud_fingerprint(u32 hbm_lvl)
{
	int rc = 0;
	int ret = 0;

	struct panel_ud_hbm_sync *ud_hbm_sync;
	struct dsi_panel_cmds *dimming_off_cmds = &g_ctrl_pdata->dimming_speed_cmds_off;
	struct dsi_panel_cmds *hbm_off_cmds_ud = &g_ctrl_pdata->hbm_ud_fingerprint_off_cmds;
	struct dsi_panel_cmds *dsi_ps_off_cmds = &g_ctrl_pdata->dsi_ps_off_cmds;
	struct dsi_panel_cmds *dsi_delay_on_cmds = &g_ctrl_pdata->dsi_delay_on_cmds;

	if (!g_ctrl_pdata) {
		pr_err("dsi hbm invalid params\n");
		return -EINVAL;
	}
	ud_hbm_sync = &g_ctrl_pdata->ud_hbm_sync;
	if (!ud_hbm_sync) {
		pr_err("dsi get_ud_hbm_sync fail\n");
		return -EINVAL;
	}

	if (panel_backlight_hbm == 0) {
		return rc;
	}
	/*pr_err(" UD dsi_panel_set_hbm_ud_fingerprint %d\n", hbm_lvl);*/
	pr_err("dsi hbm state%d\n", hbm_lvl);
	if ((hbm_lvl == 4) && (ud_hbm_state == 0)) {

		enable_irq(gpio_to_irq(g_ctrl_pdata->ud_hbm_sync.panel_te_gpio));

		if (!ud_hbm_sync->te_con_use_vblank) {
			ud_hbm_sync->te_con = 1;
			ret = wait_event_timeout(ud_hbm_sync->te_wq, !ud_hbm_sync->te_con, msecs_to_jiffies(23));
			if (!ret) {
				printk("front error:wait timeout 1\n");
				ud_hbm_sync->te_con_use_vblank = 1;
			}
		}


		mdss_dsi_panel_cmds_send(g_ctrl_pdata, dimming_off_cmds, CMD_REQ_COMMIT);
		pr_err("mdss complete dimming_off_cmds\n");
		dsi_panel_update_elvss_dimming(g_ctrl_pdata, 0);

		ud_hbm_sync->te_con = 1;

		ret = wait_event_timeout(ud_hbm_sync->te_wq, !ud_hbm_sync->te_con, msecs_to_jiffies(23));
		if (!ret) {
			printk("front error:wait timeout 2\n");
			ud_hbm_sync->te_con_use_vblank = 1;
		}

		if (ud_hbm_sync->te_con_use_vblank)
			usleep_range(333, 388);

		ud_hbm_sync->te_con = 1;

		ret = wait_event_timeout(ud_hbm_sync->te_wq, !ud_hbm_sync->te_con, msecs_to_jiffies(23));
		if (!ret) {
			printk("front error:wait timeout 3\n");
			ud_hbm_sync->te_con_use_vblank = 1;
		}
		...
		disable_irq_nosync(gpio_to_irq(g_ctrl_pdata->ud_hbm_sync.panel_te_gpio));
	} else if ((hbm_lvl == 2) && (ud_hbm_state == 0)) {
		if (dsi_ps_off_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, dsi_ps_off_cmds, CMD_REQ_COMMIT);
		ud_hbm_state = 1;
		oled_dimming_enable = 0;
		...
		if (!ud_hbm_sync->te_con_use_vblank) {
			...
			ret = wait_event_timeout(ud_hbm_sync->te_wq, !ud_hbm_sync->te_con, msecs_to_jiffies(23));
			if (!ret) {
				printk("front error:wait timeout 4\n");
				ud_hbm_sync->te_con_use_vblank = 1;
			}
		}
		if (dsi_delay_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, dsi_delay_on_cmds, CMD_REQ_COMMIT);
		ud_hbm_sync->te_con = 1;
		ret = wait_event_timeout(ud_hbm_sync->te_wq, !ud_hbm_sync->te_con, msecs_to_jiffies(23));
		if (!ret) {
			printk("front error:wait timeout 5\n");
			ud_hbm_sync->te_con_use_vblank = 1;
		}

		if (hbm_off_cmds_ud->cmd_cnt) {
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, hbm_off_cmds_ud, CMD_REQ_COMMIT);
			pr_err("mdss complete hbm_off_cmds_ud\n");
		}
		mdss_dsi_panel_bklt_dcs_10bit(g_ctrl_pdata, panel_backlight_hbm);
		ud_hbm_state = 0;
		ud_fingerprint_off_state = 1;
		oled_hbm_state = 5;
		force_dimmng_control = 1;
		disable_irq_nosync(gpio_to_irq(g_ctrl_pdata->ud_hbm_sync.panel_te_gpio));
		} else if (hbm_lvl == 3) {
		;
	}
...
static void mdss_dsi_panel_update_hbm(struct mdss_dsi_ctrl_pdata *ctrl_pdata,
	u32 hbm_lvl)
{
	struct dsi_panel_cmds *hbm_on_cmds_level1 = &ctrl_pdata->hbm_level1_cmds;
	struct dsi_panel_cmds *hbm_on_cmds_level2 = &ctrl_pdata->hbm_level2_cmds;
	struct dsi_panel_cmds *hbm_off_cmds = &ctrl_pdata->hbm_off_cmds;
	int rc = 0;

	if (!ctrl_pdata) {
		pr_err("mdss hbm invalid params\n");
		return;
	}
	...
	if (hbm_lvl == 0) {
		if (hbm_off_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, hbm_off_cmds, CMD_REQ_COMMIT);
		mdss_dsi_panel_bklt_dcs_10bit(ctrl_pdata, pre_bkg);
		del_timer_sync(&hbm_timer);
	} else {
	rc = dsi_panel_update_elvss_dimming(g_ctrl_pdata, 1);

	if (hbm_lvl == 1) {
		if (hbm_on_cmds_level1->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, hbm_on_cmds_level1, CMD_REQ_COMMIT);
		} else {
			if (hbm_on_cmds_level2->cmd_cnt)
				mdss_dsi_panel_cmds_send(ctrl_pdata, hbm_on_cmds_level2, CMD_REQ_COMMIT);
		}
		mod_timer(&hbm_timer, jiffies + HZ*HBM_EXPIRES_TIME);
	}
	if (hbm_lvl)
		oled_hbm_on = 1;
	else
...
static void mdss_dsi_panel_update_hbm_ud_fingerprint(struct mdss_dsi_ctrl_pdata *ctrl_pdata,
	u32 hbm_lvl)
{
	struct dsi_panel_cmds *hbm_on_cmds_ud = &ctrl_pdata->hbm_ud_fingerprint_cmds;
	struct dsi_panel_cmds *hbm_off_cmds_ud = &ctrl_pdata->hbm_ud_fingerprint_off_cmds;
	struct dsi_panel_cmds *dimming_off_cmds = &ctrl_pdata->dimming_speed_cmds_off;
	struct dsi_panel_cmds *dsi_ps_off_cmds = &ctrl_pdata->dsi_ps_off_cmds;
	struct dsi_panel_cmds *dsi_ps_on_cmds = &ctrl_pdata->dsi_ps_on_cmds;

	if (!ctrl_pdata) {
		pr_err("dsi hbm invalid params\n");
		return;
	}
	pr_err("dsi hbm normal sate%d\n", hbm_lvl);
	if (hbm_lvl == 4) {
		if (dimming_off_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, dimming_off_cmds, CMD_REQ_COMMIT);

		dsi_panel_update_elvss_dimming(g_ctrl_pdata, 0);
		if (hbm_on_cmds_ud->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, hbm_on_cmds_ud, CMD_REQ_COMMIT);
		if (dsi_ps_off_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, dsi_ps_off_cmds, CMD_REQ_COMMIT);
		ud_hbm_state = 1;
		oled_dimming_enable = 0;
		oled_hbm_state = 4;
	} else if (hbm_lvl == 5) {
		if (hbm_off_cmds_ud->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, hbm_off_cmds_ud, CMD_REQ_COMMIT);
		if (dsi_ps_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, dsi_ps_on_cmds, CMD_REQ_COMMIT);
		mdss_dsi_panel_bklt_dcs_10bit(ctrl_pdata, panel_backlight_hbm);
		ud_hbm_state = 0;
		ud_fingerprint_off_state = 1;
		oled_hbm_state = 5;
	}
...
static void mdss_dsi_panel_update_hbm_aod_ud_fingerprint(struct mdss_dsi_ctrl_pdata *ctrl_pdata,
	u32 hbm_lvl)
{
	struct dsi_panel_cmds *hbm_on_cmds_aod = &ctrl_pdata->hbm_ud_fingerprint_cmds;
	struct dsi_panel_cmds *aod_on_cmds = &ctrl_pdata->alpm_on_cmds;
	struct dsi_panel_cmds *aod_off_cmds = &ctrl_pdata->alpm_off_cmds;
	if (!ctrl_pdata) {
		pr_err("dsi hbm invalid params\n");
		return;
	}
	pr_err("dsi hbm aod sate%d\n", hbm_lvl);
	if (hbm_lvl == 6) {
		if (aod_off_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, aod_off_cmds, CMD_REQ_COMMIT);

		dsi_panel_update_elvss_dimming(g_ctrl_pdata, 0);

		if (hbm_on_cmds_aod->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, hbm_on_cmds_aod, CMD_REQ_COMMIT);
		doze_mode_state = 0;
		ud_hbm_state = 1;
	} else if (hbm_lvl == 7) {
		if (aod_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl_pdata, aod_on_cmds, CMD_REQ_COMMIT);
		doze_mode_state = 1;
		ud_hbm_state = 0;
	}
}

void  hbm_enable_check(unsigned long arg)
{
	   pr_err("mdss hbm expires disable schedule\n");
	   schedule_work(&hbm_disable_status);
 }

 static void hbm_disable_setting(struct work_struct *work)
 {
	struct dsi_panel_cmds *hbm_off_cmds;
	hbm_off_cmds = &g_ctrl_pdata->hbm_off_cmds;
	mutex_lock(&g_ctrl_pdata->hbm_state_lock);
	if (oled_hbm_on == 1) {
		if (hbm_off_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, hbm_off_cmds, CMD_REQ_COMMIT);
		oled_hbm_on = 0;
	}
	mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
	pr_err("mdss hbm disabled for expires\n");
 }
...
static int mdss_dsi_oled_ctrl_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int rc = 0;

	/* oled 3.3v enable gpio */
	if (gpio_is_valid(ctrl_pdata->oled_3v3_en_gpio)) {
		rc = gpio_request(ctrl_pdata->oled_3v3_en_gpio,
						"oled_3v3_enable");
		if (rc) {
			pr_err("request oled_3v3_enable gpio failed, rc=%d\n",
				       rc);
			goto oled_3v3_en_gpio_err;
		}
	}
	
	return rc;

oled_3v3_en_gpio_err:
	if (gpio_is_valid(ctrl_pdata->oled_3v3_en_gpio))
		gpio_free(ctrl_pdata->oled_3v3_en_gpio);
	return rc;
}

int mdss_dsi_oled_vci_power_ctrl(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		rc = mdss_dsi_oled_ctrl_request_gpios(ctrl_pdata);
		if (rc) {
		...
		if (!pinfo->cont_splash_enabled) {
			if (gpio_is_valid(ctrl_pdata->oled_3v3_en_gpio)) {
				...
				rc = gpio_direction_output(
					ctrl_pdata->oled_3v3_en_gpio, 1);
				if (rc) {
					pr_err("%s: unable to set dir for oled 3.3v gpio\n",
						__func__);
					goto gpio_err;
				}
				 mdelay(10);
		...
		if (gpio_is_valid(ctrl_pdata->oled_3v3_en_gpio)) {
			gpio_set_value((ctrl_pdata->oled_3v3_en_gpio), 0);
			gpio_free(ctrl_pdata->oled_3v3_en_gpio);
		}
	}
	return 0;
...
/* add for incell panel ,  for Touch panel to control reset */
void incell_tp_reset_output(int enable)
{
	if (gpio_is_valid(g_ctrl_pdata->tp_rst_gpio))
		gpio_set_value((g_ctrl_pdata->tp_rst_gpio), enable);
}
							...
							u32 bl_level)
{
	...
	case BL_DCS_CMD:
		...
		break;
	case BL_I2C:
		lm3697_brightness_set(bl_level);
		break;
	default:
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	struct dsi_panel_cmds *seed_on_cmds;
	int ret = 0;
	...
	/* dyh add for change color temperature D65*/
	if (color_temperature_state) {
		seed_on_cmds = &ctrl->seed_on_d65_cmds;
		if (seed_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl, seed_on_cmds, CMD_REQ_COMMIT);
	}
	...
	int enable)
{
	...
	alpm_on_cmds = &ctrl->alpm_on_cmds;
	alpm_off_cmds = &ctrl->alpm_off_cmds;

	mutex_lock(&ctrl->hbm_state_lock);
	if (enable) {
		if (doze_mode_state == 1) {
			pr_debug("%s already in doze mode\n", __func__);
			mutex_unlock(&ctrl->hbm_state_lock);
			return 0;
		}
		if (alpm_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl, alpm_on_cmds, CMD_REQ_COMMIT);
		doze_mode_state = 1;
	} else {
		if (doze_mode_state == 0) {
			pr_debug("%s already exit doze mode\n", __func__);
			mutex_unlock(&ctrl->hbm_state_lock);
			return 0;
		}
		if (alpm_off_cmds->cmd_cnt) {
			mdss_dsi_panel_cmds_send(ctrl, alpm_off_cmds, CMD_REQ_COMMIT);
			doze_mode_state = 0;
		}
	}
...
static int mdss_dsi_gen_read_status(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	if (!mdss_dsi_cmp_panel_reg_v2(ctrl_pdata)) {
		pr_err("%s: Read back value from panel is incorrect value is %0x\n",
	...
	if (data) {
		if (!strcmp(data, "bl_ctrl_wled")) {
			...
			pr_debug("%s: Configured DCS_CMD bklt ctrl\n",
								__func__);
		} else if (!strcmp(data, "bl_i2c")) {
			ctrl_pdata->bklt_ctrl = BL_I2C;
			pr_debug("%s: Configured BL_I2C bklt ctrl\n",
								__func__);
		}
	}
	return 0;
		...
		struct mdss_panel_data *panel_data)
{
	...
	h_front_porch_state = pt->timing.h_front_porch;
	h_back_porch_state = pt->timing.h_back_porch;
	h_pulse_width_state = pt->timing.h_pulse_width;
	v_back_porch_state = pt->timing.v_back_porch;
	v_front_porch_state = pt->timing.v_front_porch;
	v_pulse_width_state = pt->timing.v_pulse_width;
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	/*lrz add for dynamic mipi clk 2018 1115*/
	...
	/*OLED ALPM mode begin*/
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->alpm_on_cmds,
		"qcom,mdss-oled-alpm-on-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->alpm_off_cmds,
		"qcom,mdss-oled-alpm-off-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hlpm_hbrightness_on_cmds,
		"qcom,mdss-oled-hlpm-hbirghtness-on-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hlpm_lbrightness_on_cmds,
		"qcom,mdss-oled-hlpm-lbirghtness-on-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->alpm_hbrightness_on_cmds,
		"qcom,mdss-oled-alpm-hbirghtness-on-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->alpm_lbrightness_on_cmds,
		"qcom,mdss-oled-alpm-lbirghtness-on-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_level1_cmds,
		"qcom,mdss-oled-hbm-on-command-level1", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_level2_cmds,
		"qcom,mdss-oled-hbm-on-command-level2", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_ud_fingerprint_cmds,
		"qcom,mdss-oled-hbm-on-command-ud-fingerprint", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_off_cmds,
		"qcom,mdss-oled-hbm-off-command", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_ud_fingerprint_off_cmds,
		"qcom,mdss-oled-hbm-off-command-ud-fingerprint", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dsi_ps_off_cmds,
		"qcom,mdss-oled-ps-off-command", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dsi_ps_on_cmds,
		"qcom,mdss-oled-ps-on-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->acl_on_cmds_level1,
		"qcom,mdss-oled-acl-on-command-level1", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->acl_on_cmds_level2,
		"qcom,mdss-oled-acl-on-command-level2", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->acl_on_cmds_level3,
		"qcom,mdss-oled-acl-on-command-level3", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->acl_off_cmds,
		"qcom,mdss-oled-acl-off-command", "qcom,mdss-alpm-command-state");
	/*seed setting color temperature for D65 or D78*/
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->seed_on_d65_cmds,
		"qcom,mdss-oled-seed-on-command-d65", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->seed_on_d78_cmds,
		"qcom,mdss-oled-seed-on-command-d78", "qcom,mdss-alpm-command-state");
	/*oled dimming speed setting*/
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dimming_speed_cmds,
		"qcom,mdss-oled-dimming-speed", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dimming_speed_fingerprint_cmds,
		"qcom,mdss-oled-dimming-speed-fingerprint", "qcom,mdss-alpm-command-state");
       mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dimming_speed_cmds_off,
	    "qcom,mdss-oled-dimming-speed-off", "qcom,mdss-alpm-command-state");
	/*oled brightness compensate setting*/
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->brightness_compensate_level1_cmds,
		"qcom,mdss-brightness-compensate-level1", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->brightness_compensate_level2_cmds,
		"qcom,mdss-brightness-compensate-level2", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->brightness_compensate_level3_cmds,
		"qcom,mdss-brightness-compensate-level3", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->brightness_compensate_level4_cmds,
		"qcom,mdss-brightness-compensate-level4", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->brightness_compensate_level5_cmds,
		"qcom,mdss-brightness-compensate-level5", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->brightness_compensate_off_cmds,
		"qcom,mdss-brightness-compensate-off", "qcom,mdss-alpm-command-state");
	/*oled brightness compensate end*/
	/*oled aor test*/
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->aor_test_on_cmds,
	"qcom,mdss-oled-aor-test-on-command", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->aor_test_off_cmds,
		"qcom,mdss-oled-aor-test-off-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dsi_hbm_gamma_cmds,/*lrz add for hbm gamma 1009*/
		"qcom,mdss-oled-hbm-gamma-command", "qcom,mdss-alpm-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dsi_delay_on_cmds,
		"qcom,mdss-oled-delay-on-command", "qcom,mdss-alpm-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->dsi_delay_off_cmds,
		"qcom,mdss-oled-delay-off-command", "qcom,mdss-alpm-command-state");
						...
						struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret) {
		pr_err("Invalid input for oled_acl_store\n");
	}
	...
	if (!panel_backlight_on) {
		pr_err("mdss Invalid panel backlight state for acl\n");
		return count;
	}
	oled_acl_state = temp;

	switch (oled_acl_state) {
	case 0:
		acl_off_cmds = &g_ctrl_pdata->acl_off_cmds;
		break;
	case 1:
		 acl_on_cmds = &g_ctrl_pdata->acl_on_cmds_level1;
		 break;
	case 2:
		 acl_on_cmds = &g_ctrl_pdata->acl_on_cmds_level2;
		 break;
	case 3:
		 acl_on_cmds = &g_ctrl_pdata->acl_on_cmds_level2;/*lrz modify 0719,to close acl3*/
		 break;
	default:
		pr_err("mdss Invalid input acl level\n");
		return count;
	}

	if (oled_acl_state) {
		if (acl_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, acl_on_cmds, CMD_REQ_COMMIT);
	} else {
		if (acl_off_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, acl_off_cmds, CMD_REQ_COMMIT);
	}
						...
						struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	ret = kstrtoint(buf, 10, &temp);
	if (ret) {
		pr_err("Invalid input for oled_hbm_store\n");
	}
	pr_err("mdss oled hbm level %d and current backlight %d, temp: %d, oled_hbm_state_ud: %d\n", oled_hbm_state,
		panel_backlight_hbm, temp, oled_hbm_state_ud);

	if (temp == 0 || temp == 1 || temp == 2) {
		/*if (oled_hbm_state == 4 || oled_hbm_state == 6 || oled_hbm_state == 7) {*/
		if (resume_hbm_state == 1 || oled_hbm_state == 6 || oled_hbm_state == 7) {
			mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
			pr_err("sre can't update hbm state while UDFinger hbm state\n");
			return count;
		}
	}

	if (temp == oled_hbm_state) {
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		pr_err("repeated hbm state\n");
		return count;
	}
	...
	if (!(g_ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_INIT)) {
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		pr_err("mdss Invalid panel state for HBM\n");
		return count;
	}
	/*avoid operate dsi clk result crash when power off*/
	if (panel_backlight_hbm == 0 && panel_lpm_power_state == MDSS_PANEL_POWER_OFF) {
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		pr_err("mdss Invalid panel power state for HBM\n");
		return count;
	}
	/*if panel has been init done,backlight can be accessd*/
	if (atomic_read(&panel_init_state) == 0) {
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		pr_err("mdss hbm can not operate,panel has not init done\n");
		return count;
	}

	if (0 == panel_backlight_hbm && (oled_hbm_state == 4 || oled_hbm_state == 5)) {
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		pr_err("dsi can't set hbm 4 or 5 while backlight is 0, it will be set again when open backlight\n");
		return count;
	}

	if ((oled_hbm_state == 6 || oled_hbm_state == 7) && (panel_backlight_hbm > 0 ||
			panel_lpm_power_state == MDSS_PANEL_POWER_OFF)) {
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		pr_err("mdss can't set hbm 6 or 7 while not in doze. panel_backlight_hbm = %d, panel_power_mode_state = %d\n",
			panel_backlight_hbm, panel_lpm_power_state);
		return count;
	}

	if (oled_hbm_state == 4 || oled_hbm_state == 5) {
		start_time = ktime_get();
		mdss_dsi_panel_update_hbm_ud_fingerprint(g_ctrl_pdata, oled_hbm_state);
		end_time = ktime_get();
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		pr_err("%s: hbm%d: %llu\n", __func__, oled_hbm_state, (ktime_to_ms(end_time) - ktime_to_ms(start_time)));
		return count;
	}

	if (oled_hbm_state == 6 || oled_hbm_state == 7) {
		start_time = ktime_get();
		mdss_dsi_panel_update_hbm_aod_ud_fingerprint(g_ctrl_pdata, oled_hbm_state);
		end_time = ktime_get();
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		pr_err("%s:  aod_ud hbm%d: %llu\n", __func__, oled_hbm_state, (ktime_to_ms(end_time) - ktime_to_ms(start_time)));
		return count;
	}

	if (panel_backlight_hbm < 1022) {
		mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
		return count;
	}
	start_time = ktime_get();
	mdss_dsi_panel_update_hbm(g_ctrl_pdata, oled_hbm_state);
	end_time = ktime_get();
	mutex_unlock(&g_ctrl_pdata->hbm_state_lock);
	pr_err("mdss_dsi_panel_update_hbm %d: %llu\n", oled_hbm_state, (ktime_to_ms(end_time) - ktime_to_ms(start_time)));
	return count;
}

static struct kobj_attribute oled_hbm_attribute =
__ATTR(oled_hbm, 0444, oled_hbm_show, oled_hbm_store);

/* tengxiaojun :add ESD status regsiters for BSP test : 20170414*/
static ssize_t panel_esd_status_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	panel_esd_status_state = g_ctrl_pdata->return_buf[0];
	return snprintf(buf, 3, "%02x\n", panel_esd_status_state);
}

static ssize_t panel_esd_status_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
    int ret, temp;
    ret = kstrtoint(buf, 10, &temp);
    if (ret) {
		pr_err("Invalid input for panel_esd_status_store\n");
	}
	
	return count;
}
static struct kobj_attribute panel_esd_status_attribute =
__ATTR(panel_esd_status, 0644, panel_esd_status_show, panel_esd_status_store);
/* tengxiaojun :add ESD status regsiters for BSP test : 20170414*/
...
{
	int ret, temp;
	struct dsi_panel_cmds *alpm_hbrightness_on_cmds;
	struct dsi_panel_cmds *alpm_lbrightness_on_cmds;
	...
	if (panel_lpm_power_state == MDSS_PANEL_POWER_OFF || panel_lpm_power_state == MDSS_PANEL_POWER_ON) {
		pr_err("mdss invalid mode for AOD, power state = %d\n", panel_lpm_power_state);
		return count;
	}
	pr_err(" input for oled_alpmmode_store %d 0221 1151\n", oled_alpm_state);
	oled_alpm_state = temp;

	/* Any panel specific low power commands/config */
	alpm_hbrightness_on_cmds = &g_ctrl_pdata->alpm_hbrightness_on_cmds;
	alpm_lbrightness_on_cmds = &g_ctrl_pdata->alpm_lbrightness_on_cmds;

	if (oled_alpm_state) {
		if (alpm_hbrightness_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, alpm_hbrightness_on_cmds, CMD_REQ_COMMIT);
	} else {
		if (alpm_lbrightness_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, alpm_lbrightness_on_cmds, CMD_REQ_COMMIT);
	}
...
{
	int ret, temp;
	struct dsi_panel_cmds *hlpm_hbrightness_on_cmds;
	struct dsi_panel_cmds *hlpm_lbrightness_on_cmds;
	...
	/* Any panel specific low power commands/config */
	hlpm_hbrightness_on_cmds = &g_ctrl_pdata->hlpm_hbrightness_on_cmds;
	hlpm_lbrightness_on_cmds = &g_ctrl_pdata->hlpm_lbrightness_on_cmds;

    if (oled_hlpm_state) {
		if (hlpm_hbrightness_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, hlpm_hbrightness_on_cmds, CMD_REQ_COMMIT);
	} else {
		if (hlpm_lbrightness_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, hlpm_lbrightness_on_cmds, CMD_REQ_COMMIT);
	}
...
__ATTR(vivo_esd_check_ps, 0644, vivo_esd_check_show, vivo_esd_check_store);
...
__ATTR(vivo_esd_check_enable, 0644, vivo_esd_check_enable_show, vivo_esd_check_enable_store);
/*esd check end*/
						...
						struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!(g_ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_INIT) || !pre_bklt_level) {
		pr_err("Invalid panel state,  pre_bklt_level is %d 0213 1428\n", pre_bklt_level);
		return count;
	}
...
static struct kobj_attribute lcm_cabc_attribute =
__ATTR(lcm_cabc, 0644, lcm_cabc_show, lcm_cabc_store);


/* aor test begin*/
static ssize_t aor_test_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, 4, "%02x\n", aor_test_state);
}

static ssize_t aor_test_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret, temp;
	struct dsi_panel_cmds *aor_test_on_cmds = &g_ctrl_pdata->aor_test_on_cmds;
	struct dsi_panel_cmds *aor_test_off_cmds = &g_ctrl_pdata->aor_test_off_cmds;
	...
	if (panel_lpm_power_state == MDSS_PANEL_POWER_OFF) {
		pr_err("Invalid mode input for aor test\n");
		return 0;
	}

	aor_test_state = temp;
	pr_err(" mdss input aor test state = %d\n", aor_test_state);

	if (aor_test_state) {
		if (aor_test_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, aor_test_on_cmds, CMD_REQ_COMMIT);
	} else {
		if (aor_test_off_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, aor_test_off_cmds, CMD_REQ_COMMIT);
	}

	return count;
}

static struct kobj_attribute aor_test_attribute =
__ATTR(aor_test, 0644, aor_test_show, aor_test_store);

/* aor test end*/

/*panel color show beigin*/
static ssize_t lcm_color_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{

	return snprintf(buf, 4, "%02x\n", panel_color);
}
static struct kobj_attribute lcm_color_attribute =
__ATTR(lcm_color, 0644, lcm_color_show, NULL);
/*panel color show end*/
						...
						struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    ret = kstrtoint(buf, 10, &temp);
    if (ret) {
		pr_err("mdss Invalid input for diming_speed_store\n");
	}

	force_dimmng_control = temp;
	pr_err("mdss  dimming state is %d\n", force_dimmng_control);

	if (!panel_backlight_on) {
	pr_err("sde Invalid panel backlight state for dimming\n");
	return count;
	}

	if (!(g_ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_INIT)) {
		pr_err("mdss Invalid panel state\n");
		return count;
	}
	...
	/* Any panel specific low power commands/config */
	switch (force_dimmng_control) {
	case 0:
		dimming_off_cmds = &g_ctrl_pdata->dimming_speed_cmds_off;
		break;
	case 1:
		dimming_on_cmds = &g_ctrl_pdata->dimming_speed_cmds;
		break;
	case 2:
		dimming_on_cmds = &g_ctrl_pdata->dimming_speed_fingerprint_cmds;
		break;
	}

	mutex_lock(&g_ctrl_pdata->hbm_state_lock);

    if (force_dimmng_control) {
		if (dimming_on_cmds->cmd_cnt)
			mdss_dsi_panel_cmds_send(g_ctrl_pdata, dimming_on_cmds, CMD_REQ_COMMIT);
...
/*oled panel brightness compensation for temperature beigin*/
static ssize_t brightness_compensate_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, 4, "%02x\n", brightness_compensate_state);
}

static ssize_t brightness_compensate_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
    int ret, temp;
    struct dsi_panel_cmds *brightness_compensate_cmds;

    ret = kstrtoint(buf, 10, &temp);
    if (ret) {
		pr_err("Invalid input for brightness_compensate_store\n");
	}

    brightness_compensate_state = temp;

	pr_err("mdss  brightness compensate state is %d\n", brightness_compensate_state);
	...
	if (brightness_compensate_state < 0 || brightness_compensate_state > 6) {
		pr_err("Invalid input for brightness compensate state\n");
		return count;
	}

	switch (brightness_compensate_state) {
	case 0:
		brightness_compensate_cmds = &g_ctrl_pdata->brightness_compensate_off_cmds;
		break;
	case 1:
		brightness_compensate_cmds = &g_ctrl_pdata->brightness_compensate_level1_cmds;
		break;
	case 2:
		brightness_compensate_cmds = &g_ctrl_pdata->brightness_compensate_level2_cmds;
		break;
	case 3:
		brightness_compensate_cmds = &g_ctrl_pdata->brightness_compensate_level3_cmds;
		break;
	case 4:
		brightness_compensate_cmds = &g_ctrl_pdata->brightness_compensate_level4_cmds;
		break;
	case 5:
		brightness_compensate_cmds = &g_ctrl_pdata->brightness_compensate_level5_cmds;
		break;
	}

	if (brightness_compensate_cmds->cmd_cnt)
		mdss_dsi_panel_cmds_send(g_ctrl_pdata, brightness_compensate_cmds, CMD_REQ_COMMIT);

	return count;
}

static struct kobj_attribute brightness_compensate_attribute =
__ATTR(brightness_compensate, 0644, brightness_compensate_show, brightness_compensate_store);

/*Dynamic mipi clk enable*/
static ssize_t dynamic_mipi_clk_enable_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, 4, "%x\n", dynamic_mipi_clk_enable_status);
}

static ssize_t dynamic_mipi_clk_enable_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
    int ret, temp;
    ret = kstrtoint(buf, 10, &temp);
    if (ret) {
		pr_err("Invalid input for dynamic_mipi_clk_enable_store\n");
		return count;
	}

     dynamic_mipi_clk_enable_status = temp;
     pr_err("dynamic_mipi_clk_enable_status = %d \n", dynamic_mipi_clk_enable_status);
	return count;
}

static struct kobj_attribute dynamic_mipi_clk_enable_attribute =
__ATTR(dynamic_mipi_clk_enable, 0444, dynamic_mipi_clk_enable_show, dynamic_mipi_clk_enable_store);

/*h_front_porch*/
static ssize_t h_front_porch_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
       return snprintf(buf, 4, "%d\n", h_front_porch_state);
}

static ssize_t h_front_porch_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
       int ret, temp;
	 struct mdss_panel_info *pinfo;
       ...
       if (!g_ctrl_pdata) {
		pr_err("%s: Invalid g_ctrl_pdata \n", __func__);
		return -ENODEV;
       }

	if (!dynamic_mipi_clk_enable_status) {
		pr_err("%s:mdss dynamic_mipi_clk_enable_status is not enable \n", __func__);
		return count;
	}

	pinfo = &g_ctrl_pdata->panel_data.panel_info;

       ret = kstrtoint(buf, 10, &temp);
       if (ret) {
		pr_err("Invalid input for h_front_porch\n");
       }

       h_front_porch_state = temp;

	pinfo->lcdc.h_front_porch = h_front_porch_state;
	g_ctrl_pdata->refresh_clk_rate = true;
       return count;
}
static struct kobj_attribute h_front_porch_attribute =
__ATTR(h_front_porch, 0644, h_front_porch_show, h_front_porch_store);

/*h_back_porch*/
static ssize_t h_back_porch_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
       return snprintf(buf, 6, "%d\n", h_back_porch_state);
}

static ssize_t h_back_porch_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
       int ret, temp;
	 struct mdss_panel_info *pinfo;
	...
	pinfo = &g_ctrl_pdata->panel_data.panel_info;
       ...
       ret = kstrtoint(buf, 10, &temp);
       if (ret) {
		pr_err("Invalid input for h_back_porch\n");
       }

       h_back_porch_state = temp;

	pinfo->lcdc.h_back_porch = h_back_porch_state;
	g_ctrl_pdata->refresh_clk_rate = true;
       return count;
}
static struct kobj_attribute h_back_porch_attribute =
__ATTR(h_back_porch, 0644, h_back_porch_show, h_back_porch_store);

/*h_pulse_width*/
static ssize_t h_pulse_width_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
       return snprintf(buf, 6, "%d\n", h_pulse_width_state);
}

static ssize_t h_pulse_width_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
       int ret, temp;
	 struct mdss_panel_info *pinfo;
	...
	pinfo = &g_ctrl_pdata->panel_data.panel_info;
       ...
       ret = kstrtoint(buf, 10, &temp);
       if (ret) {
		pr_err("Invalid input for h_pulse_width\n");
       }

       h_pulse_width_state = temp;

	pinfo->lcdc.h_pulse_width = h_pulse_width_state;
	g_ctrl_pdata->refresh_clk_rate = true;
       return count;
}
static struct kobj_attribute h_pulse_width_attribute =
__ATTR(h_pulse_width, 0644, h_pulse_width_show, h_pulse_width_store);

/*v_front_porch*/
static ssize_t v_front_porch_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
       return snprintf(buf, 4, "%d\n", v_front_porch_state);
}

static ssize_t v_front_porch_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
       int ret, temp;
	 struct mdss_panel_info *pinfo;
	...
	pinfo = &g_ctrl_pdata->panel_data.panel_info;
       ...
       ret = kstrtoint(buf, 10, &temp);
       if (ret) {
		pr_err("Invalid input for v_front_porch\n");
       }

       v_front_porch_state = temp;

	pinfo->lcdc.v_front_porch = v_front_porch_state;
	g_ctrl_pdata->refresh_clk_rate = true;
       return count;
}
static struct kobj_attribute v_front_porch_attribute =
__ATTR(v_front_porch, 0644, v_front_porch_show, v_front_porch_store);

/*v_back_porch*/
static ssize_t v_back_porch_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
       return snprintf(buf, 4, "%d\n", v_back_porch_state);
}

static ssize_t v_back_porch_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
       int ret, temp;
	 struct mdss_panel_info *pinfo;
	...
	pinfo = &g_ctrl_pdata->panel_data.panel_info;
       ...
       ret = kstrtoint(buf, 10, &temp);
       if (ret) {
		pr_err("Invalid input for v_back_porch\n");
       }

       v_back_porch_state = temp;

	pinfo->lcdc.v_back_porch = v_back_porch_state;
	g_ctrl_pdata->refresh_clk_rate = true;
       return count;
}
static struct kobj_attribute v_back_porch_attribute =
__ATTR(v_back_porch, 0644, v_back_porch_show, v_back_porch_store);

/*v_pulse_width*/
static ssize_t v_pulse_width_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
       return snprintf(buf, 3, "%d\n", v_pulse_width_state);
}

static ssize_t v_pulse_width_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
       int ret, temp;
	 struct mdss_panel_info *pinfo;
	...
	pinfo = &g_ctrl_pdata->panel_data.panel_info;
       ...
       ret = kstrtoint(buf, 10, &temp);
       if (ret) {
		pr_err("Invalid input for v_pulse_width\n");
       }

       v_pulse_width_state = temp;

	pinfo->lcdc.v_pulse_width = v_pulse_width_state;
	g_ctrl_pdata->refresh_clk_rate = true;
       return count;
}
static struct kobj_attribute v_pulse_width_attribute =
__ATTR(v_pulse_width, 0644, v_pulse_width_show, v_pulse_width_store);

/* show lcm_mipi_data_lane_number */
static ssize_t lcm_mipi_data_lane_number_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	mdss_report_lcm_mipi_data_lane_number();
	return snprintf(buf, 4, "%02x\n", lcm_mipi_data_lane_number);
}

static struct kobj_attribute lcm_mipi_data_lane_number_attribute =
__ATTR(lcm_mipi_data_lane_number, 0444, lcm_mipi_data_lane_number_show, NULL);

/* show color temperature state */
static ssize_t color_temperature_show(struct kobject *kobj,
						struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, 4, "%02x\n", color_temperature_state);
}

static ssize_t color_temperature_store(struct kobject *kobj,
						struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret, temp;
	struct dsi_panel_cmds *seed_on_cmds;
	...
	ret = kstrtoint(buf, 10, &temp);
	if (ret) {
		pr_err("Invalid input for color_temperature_store\n");
	}

	color_temperature_state = temp;

	if (!(g_ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_INIT)) {
		pr_err("mdss Invalid panel state for color temperature\n");
		return count;
	}

	pr_err("mdss color temperature state is %d\n", color_temperature_state);

	if (color_temperature_state)
		seed_on_cmds = &g_ctrl_pdata->seed_on_d65_cmds;
	else
		seed_on_cmds = &g_ctrl_pdata->seed_on_d78_cmds;

	if (seed_on_cmds->cmd_cnt)
		mdss_dsi_panel_cmds_send(g_ctrl_pdata, seed_on_cmds, CMD_REQ_COMMIT);

	return count;
}

static struct kobj_attribute color_temperature_attribute =
__ATTR(color_temperature, 0644, color_temperature_show, color_temperature_store);

static struct attribute *disp_lcm_sys_attrs[] = {
	&lcm_id_attribute.attr,
	&oled_acl_attribute.attr,
	&oled_hbm_attribute.attr,
	&panel_esd_status_attribute.attr,
	...
	&lcm_color_attribute.attr,
	&vivo_sre_check_enable_attribute.attr,
	&diming_speed_attribute.attr,
	&brightness_compensate_attribute.attr,
	&dynamic_mipi_clk_enable_attribute.attr,
	&h_front_porch_attribute.attr,
	&h_back_porch_attribute.attr,
	&v_front_porch_attribute.attr,
	&v_back_porch_attribute.attr,
	&h_pulse_width_attribute.attr,
	&v_pulse_width_attribute.attr,
	&lcm_mipi_data_lane_number_attribute.attr,
	&color_temperature_attribute.attr,
	&lcm_cabc_attribute.attr,
	&aor_test_attribute.attr,
	NULL
};
	...
	int ndx)
{
	...
	pinfo->sre_enabled = of_property_read_bool(node,
		"qcom,mdss-panel-sre");
	...
	hbm_timer.expires = jiffies+20*HZ;
	hbm_timer.data = 0;
	hbm_timer.function = hbm_enable_check;
	INIT_WORK(&hbm_disable_status, hbm_disable_setting);
	hbm_send_state = 0;
	oled_hbm_state_ud = 0;
	doze_mode_state = 0;
	dimming_state = 0;
	ctrl_pdata->ud_hbm_sync.te_con = 0;
	ctrl_pdata->ud_hbm_sync.te_con_use_vblank = 0;
	init_waitqueue_head(&ctrl_pdata->ud_hbm_sync.te_wq);
	INIT_WORK(&ps_off_status, ps_off_setting);
	INIT_WORK(&hbm_on_status, hbm_on_setting);
	INIT_WORK(&delay_on_status, delay_on_setting);
	return 0;
}
...
/* add report lcm_mipi_data_lane_number for dynamic mipi clk */
unsigned int mdss_report_lcm_mipi_data_lane_number(void)
{
	struct mdss_panel_info *pinfo;
	int lane_config = 0;
	...
	if (pinfo->mipi.data_lane3)
		lane_config++;
	lcm_mipi_data_lane_number = lane_config;
	return lcm_mipi_data_lane_number;
}
