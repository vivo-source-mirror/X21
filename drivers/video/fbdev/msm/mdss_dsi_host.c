			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	mutex_init(&ctrl->panel_patial_lock);
	mutex_init(&ctrl->hbm_state_lock);
	mutex_init(&ctrl->panel_power_lock);/*lrz add 0409*/
	mutex_init(&ctrl->dynamic_clk_change_lock);
...
int mdss_dsi_cmdlist_commit(struct mdss_dsi_ctrl_pdata *ctrl, int from_mdp)
{
	...
	end_time = ktime_get();
	pr_debug("mdss_dsi_clkrate_update. Delay: %llu lrz add 2018 1115\n", (ktime_to_ms(end_time) - ktime_to_ms(start_time)));
	/*
