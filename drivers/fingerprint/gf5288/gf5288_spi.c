...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case GF_IOC_GET_POWER_STATE:
			...
			retval = gf5288_hw_get_power_state(gf_dev);
			break;
    case GF_IOC_WAKE_UP:
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	*/
    ...
    if (need_wake_flag == 1) {
		...
		input_report_key(gf_dev->input, GF_KEY_INPUT_WAKE, 0);
		input_sync(gf_dev->input);
		//wake_lock_timeout(&vivo_wakeup_t.fp_wakelock, msecs_to_jiffies(1500));
    }*/ 
	pr_warn("gf,%s,need_wake_flag:%d.\n", __func__, need_wake_flag);
	pr_warn("gf,%s,current mode is %d,ff_mode_state is %d.\n", __func__, vivo_wakeup_t.current_mode, vivo_wakeup_t.ff_mode_state);
	if (vivo_wakeup_t.current_mode == GF_FF_MODE && vivo_wakeup_t.ff_mode_state) {
		vivo_wakeup_t.ff_mode_state = 0;
		input_report_key(gf_dev->input, GF_KEY_INPUT_WAKE, 1);
		input_sync(gf_dev->input);
		input_report_key(gf_dev->input, GF_KEY_INPUT_WAKE, 0);
		input_sync(gf_dev->input);
		pr_warn("report wake event\n");
	}
	wake_lock_timeout(&vivo_wakeup_t.fp_wakelock, msecs_to_jiffies(500));
	pr_warn("gf,wake_lock 500ms\n");
	return IRQ_HANDLED;
}
...
#endif
{
	...
	if (status == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(gf5288_maps); i++)
			input_set_capability(gf_dev->input, gf5288_maps[i].type, gf5288_maps[i].code);
	...
	*/
	...
	gf_dev->irq = gf5288_irq_num(gf_dev);
...
#ifdef CONFIG_BBK_FP_ID
    if (get_fp_id() != GOODIX_GF5288) {
		printk("%s(): wrong fp id, not gf5288 driver, exit\n", __func__);
		return 0;
    }
#endif         
