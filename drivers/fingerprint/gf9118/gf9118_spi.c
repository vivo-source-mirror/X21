...
#define GF_SPIDEV_NAME     "goodix,fingerprintud"
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case GF_IOC_GET_POWER_STATE:
			...
			retval = gf9118_hw_get_power_state(gf_dev);
			break;
	default:
...
#endif
{
	...
	if (status == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(key_maps); i++)
			input_set_capability(gf_dev->input, key_maps[i].type, key_maps[i].code);
	...
	gf_dev->irq = gf9118_irq_num(gf_dev);
...
#ifdef CONFIG_BBK_FP_ID
    if (get_fp_id() != GOODIX_GF9118) {
	printk("%s(): wrong fp id, not gf9118 driver, exit\n", __func__);
	return 0;
    }
#endif
