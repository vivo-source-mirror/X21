...
bool ufshcd_eh_in_progress(struct ufs_hba *h)
{
	return (h->eh_flags & UFSHCD_EH_IN_PROGRESS);
}
...
bool ufshcd_is_operational(struct ufs_hba *hba)
{
	unsigned long flags;
	bool ret;

	spin_lock_irqsave(hba->host->host_lock, flags);
	ret = (hba->ufshcd_state == UFSHCD_STATE_OPERATIONAL);
	spin_unlock_irqrestore(hba->host->host_lock, flags);

	return ret;
}
EXPORT_SYMBOL(ufshcd_is_operational);
				  ...
				  u32 param_size)
{
	...
	if (desc_id >= QUERY_DESC_IDN_MAX) {
		dev_err(hba->dev, "[%s:%d] desc_id %d is invalid\n",
				__func__, __LINE__, desc_id);
		return -EINVAL;
	}

	buff_len = ufs_query_desc_max_size[desc_id];
	if ((param_offset + param_size) > buff_len) {
		dev_err(hba->dev, "[%s:%d] param_offset or param_size is invalid\n",
				__func__, __LINE__);
		return -EINVAL;
	}
				...
				u32 size, bool ascii)
{
	...
	if (err) {
		dev_err(hba->dev, "%s: reading String Desc failed after %d retries. desc_index %d, err %d\n",
			__func__, QUERY_REQ_RETRIES, desc_index, err);
		goto out;
	}
...
static int ufshcd_slave_configure(struct scsi_device *sdev)
{
	...
	if (sdev->lun < UFS_UPIU_MAX_GENERAL_LUN) {
		...
		printk("%s:%d: ufshpb set lun %lld sdev %p q %p n",
...
static int ufs_read_device_desc_data(struct ufs_hba *hba)
{
	...
	memset(serial_buf, 0, QUERY_DESC_MAX_SIZE);
	...
	err = ufshcd_read_string_desc(hba, serial_index, serial_buf,
			QUERY_DESC_HEALTH_DEF_SIZE, UTF16_STD);
	if (err) {
		dev_err(hba->dev, "[%s:%d] Failed to read serial string descriptor: %d\n",
				__func__, __LINE__, err);
		goto __free_serial;
	}

	serial_len = serial_buf[0] - 2;
	snprintf(hba->ufsid, 5, "%04x", w_manufacturer_id);
	ptemp = hba->ufsid + 4;
		...
		struct ufs_ioctl_query_data *ioctl_data, void __user *buffer)
{
	...
	if (ioctl_data->idn == QUERY_DESC_IDN_STRING) {
		...
		buf_size = IOCTL_DEV_CTX_MAX_SIZE;
	} else {
		kernel_buf = kzalloc(QUERY_DESC_MAX_SIZE, GFP_KERNEL);
		buf_size = QUERY_DESC_MAX_SIZE;
	}
	...
	if (ioctl_data->buf_size > buf_size) {
		dev_err(hba->dev,
				"%s: buf_size is invalid: %d.\n",
				__func__, ioctl_data->buf_size);
		err = -ENOMEM;
...
static ssize_t ufshcd_dev_desc_show(struct ufs_hba *hba, char *buf)
{
	int err = 0;
	u8 desc_buf[QUERY_DESC_DEVICE_MAX_SIZE];
	...
	return (ssize_t)ufs_debug_print_buf(hba, buf, 512,
			(char *)desc_buf, QUERY_DESC_DEVICE_MAX_SIZE);
}

static ssize_t ufshcd_manf_name_show(struct ufs_hba *hba, char *buf)
{
	int err = 0;
	u8 desc_buf[QUERY_DESC_DEVICE_MAX_SIZE];

	if (hba->manf_name[0]) {
		return snprintf(buf, hba->manf_name[QUERY_DESC_LENGTH_OFFSET],
				"%s\n", hba->manf_name+QUERY_DESC_HDR_SIZE);
	}
	...
	err = ufshcd_read_string_desc(hba,
			...
			hba->manf_name, sizeof(hba->manf_name), ASCII_STD);
	pm_runtime_put_sync(hba->dev);
	return snprintf(buf, hba->manf_name[QUERY_DESC_LENGTH_OFFSET],
			"%s\n", hba->manf_name+QUERY_DESC_HDR_SIZE);
}

static ssize_t ufshcd_product_name_show(struct ufs_hba *hba, char *buf)
{
	int err = 0;
	u8 desc_buf[QUERY_DESC_DEVICE_MAX_SIZE];

	if (hba->product_name[0]) {
		return snprintf(buf,
				hba->product_name[QUERY_DESC_LENGTH_OFFSET],
				"%s\n", hba->product_name+QUERY_DESC_HDR_SIZE);
	}
	...
	err = ufshcd_read_string_desc(hba,
			...
			hba->product_name, sizeof(hba->product_name),
			ASCII_STD);
	pm_runtime_put_sync(hba->dev);
	return snprintf(buf, hba->product_name[QUERY_DESC_LENGTH_OFFSET],
			"%s\n", hba->product_name+QUERY_DESC_HDR_SIZE);
}

static ssize_t ufshcd_ufsid_show(struct ufs_hba *hba, char *buf)
{
	if (hba->ufsid[0])
		return snprintf(buf, 512, hba->ufsid);

	return snprintf(buf, 512, "ufsid is invalid.\n");
}
		...
		u8 offset, char *buf)
{
	...
	if (hba->pre_end_life[0]) {
		switch (offset) {
		case 2:
			err = snprintf(buf, 128, hba->pre_end_life);
		break;
		case 3:
			err = snprintf(buf, 128, hba->life_time_a);
		break;
		case 4:
			err = snprintf(buf, 128, hba->life_time_b);
		break;
		default:
	...
	if (!err) {
		snprintf(hba->pre_end_life, sizeof(hba->pre_end_life),
				"0x%02x\n", desc_buf[2]);
		snprintf(hba->life_time_a, sizeof(hba->life_time_a),
				"0x%02x\n", desc_buf[3]);
		snprintf(hba->life_time_b, sizeof(hba->life_time_b),
				"0x%02x\n", desc_buf[4]);
		...
		struct attribute *attr, char *buf)
{
	...
	if (!strcmp(attr->name, "ufsid"))
		return ufshcd_ufsid_show(hba, buf);

	if (!strcmp(attr->name, "manufacturer"))
		return ufshcd_manf_name_show(hba, buf);

	if (!strcmp(attr->name, "product"))
		return ufshcd_product_name_show(hba, buf);

	if (!strcmp(attr->name, "device_desc"))
		return ufshcd_dev_desc_show(hba, buf);

	if (!strcmp(attr->name, "pre_end_life"))
		return ufshcd_life_time_info_show(hba, 2, buf);

	if (!strcmp(attr->name, "life_time_a"))
		return ufshcd_life_time_info_show(hba, 3, buf);

	if (!strcmp(attr->name, "life_time_b"))
...
static struct attribute  ufshcd_ufsid_attr = {
	.name = "ufsid",
	.mode = S_IRUGO,
};

static struct attribute  ufshcd_manf_attr = {
	.name = "manufacturer",
	.mode = S_IRUGO,
};

static struct attribute  ufshcd_product_attr = {
	.name = "product",
	.mode = S_IRUGO,
};

static struct attribute  ufshcd_device_desc_attr = {
	.name = "device_desc",
	.mode = S_IRUGO,
};

static struct attribute  ufshcd_pre_end_life_attr = {
	.name = "pre_end_life",
	.mode = S_IRUGO,
};

static struct attribute  ufshcd_life_time_a_attr = {
	.name = "life_time_a",
	.mode = S_IRUGO,
};

static struct attribute  ufshcd_life_time_b_attr = {
	.name = "life_time_b",
	.mode = S_IRUGO,
};
...
static struct attribute *def_ufshcd_feature_attrs[] = {
	...
	&ufshcd_manf_attr,
	&ufshcd_product_attr,
	&ufshcd_device_desc_attr,
	&ufshcd_pre_end_life_attr,
	&ufshcd_life_time_a_attr,
	&ufshcd_life_time_b_attr,
	NULL,
};
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	memset(hba->ufsid, 0, sizeof(hba->ufsid));
	memset(hba->pre_end_life, 0, sizeof(hba->pre_end_life));
	memset(hba->life_time_a, 0, sizeof(hba->life_time_a));
	memset(hba->life_time_b, 0, sizeof(hba->life_time_b));
	...
	err = kobject_init_and_add(&hba->ufs_feature_kobject,
			&ufshcd_feature_object_type, NULL, "ufs");
	if (err)
		dev_err(hba->dev, "Create kobject for ufsid failed.\n");
