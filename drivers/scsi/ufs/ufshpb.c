...
static void ufshpb_set_dirty(struct ufshpb_lu *hpb,
		struct ufshcd_lrb *lrbp, int region, int subregion, int subregion_offset)
{
	struct ufshpb_region *cb;
...
void ufshpb_prep_fn(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if (!lrbp || (lrbp->lun >= UFS_UPIU_MAX_GENERAL_LUN))
		return;
	...
	if (ufshpb_is_write_discard_lrbp(lrbp)) {
		...
		ufshpb_set_dirty(hpb, lrbp, region, subregion, subregion_offset);
		spin_unlock_bh(&hpb->hpb_lock);
		return;
	}
	...
	if (blk_rq_sectors(lrbp->cmd->request) != SECTORS_PER_BLOCK) {
		TMSG(hpb, "%llu + %u READ_10 many_blocks %d - %d",
				...
				(unsigned int) blk_rq_sectors(lrbp->cmd->request), region, subregion);
		return;
	}
	...
	if (cb->region_state == HPBREGION_INACTIVE ||
		...
		if (cb->region_state == HPBREGION_INACTIVE) {
			...
			TMSG(hpb, "%llu + %u READ_10 ( %d %d ) %d - %d",
				...
				cb->region_state, cp->subregion_state, region, subregion);
		spin_unlock_bh(&hpb->hpb_lock);
		return;
	}
...
static void ufshpb_clean_active_page(
		struct ufshpb_lu *hpb, struct ufshpb_subregion *cp)
{
	struct ufshpb_region *cb;
...
static void ufshpb_error_active_page(
		struct ufshpb_lu *hpb, struct ufshpb_subregion *cp)
{
	struct ufshpb_region *cb;
		...
		struct ufshpb_map_req *map_req)
{
	...
	HPB_DEBUG(hpb, "Profiling: start~workq1 = %lld, workq1~issue = %lld, issue~endio = %lld, endio~end = %lld",
	...
	ufshpb_clean_active_page(hpb,
			hpb->region_tbl[map_req->region].subregion_tbl + map_req->subregion);
		...
		if (sshdr.sense_key == ILLEGAL_REQUEST) {
			...
			if (cb->region_state == HPBREGION_PINNED) {
				if (sshdr.asc == 0x06 && sshdr.ascq == 0x01) {
					HPB_DEBUG(hpb, "retry pinned rb %d - %d", map_req->region,
							  map_req->subregion);
					INIT_LIST_HEAD(&map_req->list_map_req);
					list_add_tail(&map_req->list_map_req, &hpb->lh_map_req_retry);
					spin_unlock(&hpb->hpb_lock);

					schedule_delayed_work(&hpb->ufshpb_retry_work, msecs_to_jiffies(5000));
					return;

				} else {
					HPB_DEBUG(hpb, "pinned rb %d - %d(dirty)", map_req->region,
							  map_req->subregion);

					ufshpb_error_active_page(hpb, cb->subregion_tbl + map_req->subregion);
					spin_unlock(&hpb->hpb_lock);
				}
			} else {
				spin_unlock(&hpb->hpb_lock);
				...
				HPB_DEBUG(hpb, "Non-pinned rb %d is added to rsp_info_list",
						  map_req->region);
...
static void print_buf(unsigned char *buf, int length)
{
	...
	printk("%s:%d Device Context Print Start!!\n", __func__, __LINE__);
	for (i = 0; i < length; i++) {
		if (i % line_max == 0)
			printk("(0x%.2x) :", i);
		printk(" %.2x", buf[i]);
		...
		struct request_queue *q, struct ufshpb_map_req *map_req)
{
	...
	/* we check req and map_req address in this time whether working normaly */
	HPB_DEBUG(hpb, "(%d - %d) map_req %p req %p q %p t %x f %llx",
			map_req->region, map_req->subregion, map_req, req, q,
			req->cmd_type, (unsigned long long)req->cmd_flags);
	HPB_DEBUG(hpb, "(%d - %d) cmd %.2x %.2x %.2x %.2x cmd_len %x",
			map_req->region, map_req->subregion,
			req->cmd[0], req->cmd[1], req->cmd[2], req->cmd[3],
			req->cmd_len);
...
static inline int ufshpb_add_region(struct ufshpb_lu *hpb, struct ufshpb_region *cb)
{
	...
	HPB_DEBUG(hpb, "\x1b[44m\x1b[32m E->active region: %d \x1b[0m", cb->region);
	...
	for (subregion = 0; subregion < cb->subregion_count; subregion++) {
		...
		if (!cp->mctx) {
			HPB_DEBUG(hpb, "get mctx failed. err %d subregion %d free_table %d",
...
static struct ufshpb_region *ufshpb_victim_lru_info(struct victim_select_info *lru_info)
{
	struct ufshpb_region *cb;
	...
	switch (lru_info->selection_type) {
	case LRU:
		victim_cb = list_first_entry(&lru_info->lru, struct ufshpb_region, list_region);
		break;
	case LFU:
		...
		struct ufshpb_rsp_info *rsp_info)
{
	...
	for (iter = 0; iter < rsp_info->active_cnt; iter++) {
		...
		ret = ufshpb_set_map_req(hpb, region, subregion, cp->mctx, rsp_info);
		if (ret) {
...
void ufshpb_rsp_upiu(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if (!data_seg_len) {
		...
		do_workq = !list_empty(&hpb->lh_rsp_info);
	...
	case HPB_RSP_REQ_REGION_UPDATE:
		...
		for (num = 0; num < rsp_field->active_region_cnt; num++) {
			...
			HPB_DEBUG(hpb, "active num: %d, #block: %d, page#: %d",
				  num + 1,
				  rsp_info->active_list.region[num],
				  rsp_info->active_list.subregion[num]);
		}
		...
		for (num = 0; num < rsp_field->inactive_region_cnt; num++) {
			...
			HPB_DEBUG(hpb, "inactive num: %d, #block: %d",
...
static int ufshpb_read_unit_desc(struct ufs_hba *hba, int lun, u8 *desc_buf, u32 size)
{
		...
		struct ufshpb_subregion *cp)
{
	...
	bvec = kmalloc(sizeof(struct bio_vec) * hpb->mpages_per_subregion, GFP_KERNEL);
	if (!bvec)
		return -ENOMEM;
	...
	if (req.errors) {
		...
		spin_lock_bh(&hpb->hpb_lock);
		ufshpb_error_active_page(hpb, cp);
		spin_unlock_bh(&hpb->hpb_lock);
	} else {
		ret = 0;
		spin_lock_bh(&hpb->hpb_lock);
...
static int ufshpb_issue_map_req_from_list(struct ufshpb_lu *hpb)
{
	...
	list_for_each_entry_safe(cp, next_cp, &req_list, list_subregion) {
		...
		spin_lock_bh(&hpb->hpb_lock);
		ufshpb_clean_active_page(hpb, cp);
...
static void ufshpb_task_worker(struct work_struct *work)
{
	struct ufshpb_lu *hpb;
	struct ufshpb_rsp_info *rsp_info;
	unsigned long flags;
		...
		struct ufshpb_region *cb)
{
	...
	for (subregion = 0 ; subregion < cb->subregion_count ; subregion++) {
		...
		cp->mctx = ufshpb_get_map_ctx(hpb, &err);
		if (err) {
			ERR_MSG("get mctx failed. err %d subregion %d free_table %d",
				err, subregion, hpb->debug_free_table);
			goto release;
		}
		spin_lock_bh(&hpb->hpb_lock);
...
static inline void ufshpb_init_jobs(struct ufshpb_lu *hpb)
{
	...
	INIT_WORK(&hpb->ufshpb_task_workq, ufshpb_task_worker);
}
		...
		int entry_count, int entry_byte)
{
	...
	for (i = 0 ; i < num_regions * subregions_per_region ; i++) {
		...
		mctx->m_page = kzalloc(sizeof(struct page *) * hpb->mpages_per_subregion, GFP_KERNEL);
		if (!mctx->m_page)
			goto release_mem;

		mctx->ppn_dirty = (unsigned int *)vzalloc(entry_count >> bits_per_byte_shift);
		if (!mctx->ppn_dirty)
			goto release_mem;
...
static int ufshpb_req_mempool_init(struct ufs_hba *hba, struct ufshpb_lu *hpb, int queue_depth)
{
	...
	hpb->rsp_info = kzalloc(sizeof(struct ufshpb_rsp_info) * queue_depth, GFP_KERNEL);
	if (!hpb->rsp_info)
		goto release_mem;

	map_req_cnt = max(hpb->subregions_per_lu, queue_depth);

	hpb->map_req = kzalloc(sizeof(struct ufshpb_map_req) * map_req_cnt, GFP_KERNEL);
	if (!hpb->map_req)
		goto release_mem;
	/* check map_req address for issueing read-buffer request */
	INIT_INFO("hpb->map_req %p - (%p - 1)", hpb->map_req,
			hpb->map_req + map_req_cnt);
		...
		struct ufshpb_func_desc *func_desc)
{
	...
	hpb->subregion_mem_size = hpb->subregion_unit_size / BLOCK * HPB_ENTRY_SIZE;
	...
	INIT_INFO("lu_num_blocks = %d, regions_per_lu = %d, subregions_per_lu = %d",
		  hpb->lu_num_blocks, hpb->regions_per_lu, hpb->subregions_per_lu);

	INIT_INFO("subregions_per_region = %d", hpb->subregions_per_region);
	INIT_INFO("entries_per_region %u shift %u mask 0x%X", entries_per_region,
		...
		struct ufshpb_lu_desc *lu_desc, int lun)
{
	...
	region_table = kzalloc(sizeof(struct ufshpb_region) * hpb->regions_per_lu,
			GFP_KERNEL);
	if (!region_table)
		goto out;
	...
	ret = ufshpb_table_mempool_init(hpb,
			lu_desc->lu_max_active_hpb_regions, hpb->subregions_per_region,
		...
		struct ufshpb_lu_desc *lu_desc, int lun)
{
	...
	lu_desc->lu_logblk_cnt =
		...
		SHIFT_BYTE_3((u64)logical_buf[UNIT_DESC_PARAM_LOGICAL_BLK_COUNT + 4]) |
		SHIFT_BYTE_2((u64)logical_buf[UNIT_DESC_PARAM_LOGICAL_BLK_COUNT + 5]) |
		SHIFT_BYTE_1((u64)logical_buf[UNIT_DESC_PARAM_LOGICAL_BLK_COUNT + 6]) |
		SHIFT_BYTE_0((u64)logical_buf[UNIT_DESC_PARAM_LOGICAL_BLK_COUNT + 7]);
...
void ufshpb_init_handler(struct work_struct *work)
{
	...
	if (hba->host->async_scan == 1) {
		...
		schedule_delayed_work(&hba->ufshpb_init_work, msecs_to_jiffies(100));
		return;
	}
	spin_unlock_irqrestore(hba->host->host_lock, flags);
	mutex_unlock(&hba->host->scan_mutex);
	...
	/* If error is handling, wait for it to finish. */
	if (scsi_host_in_recovery(hba->host)
			|| ufshcd_eh_in_progress(hba)
			|| !ufshcd_is_operational(hba)) {
		INIT_INFO("Wait for error handling finish. So re-sched.");
		if (!delayed_work_pending(&hba->ufshpb_init_work))
			schedule_delayed_work(&hba->ufshpb_init_work, msecs_to_jiffies(500));
		return;
	}

	if (hba->ufshpb_state == HPB_NEED_INIT) {
		printk("%s:%d HPB_INIT_START\n", __func__, __LINE__);
		err = ufshpb_init(hba);
		if (err) {
			dev_warn(hba->dev, "UFSHPB driver initialization failed - "
		...
		const char *buf, size_t count)
{
	...
	ufshpb_get_pos_from_lpn(hpb, lpn, &region, &subregion, &subregion_offset);
...
static ssize_t ufshpb_sysfs_map_req_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "rb_noti conunt %lld map_req count %lld\n",
					rb_noti_cnt, map_req_cnt);
}
...
static ssize_t ufshpb_sysfs_miss_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	SYSFS_INFO("Total : %lld, region_miss %lld, subregion_miss %lld, entry_dirty_miss %lld, rb_fail %lld",
		   region_miss + subregion_miss + entry_dirty_miss,
		   region_miss, subregion_miss, entry_dirty_miss, rb_fail);

	return snprintf(buf, PAGE_SIZE, "Total: %lld, region %lld subregion %lld entry %lld rb_fail %lld\n",
			region_miss + subregion_miss + entry_dirty_miss,
			region_miss, subregion_miss, entry_dirty_miss, rb_fail);
}
...
static ssize_t ufshpb_sysfs_active_list_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	list_for_each_entry(region, &hpb->lru_info.lru, list_region) {
		ret = snprintf(buf + count, PAGE_SIZE, "%d(cnt=%d) ",
			      region->region, region->hit_count);
		count += ret;
	}
...
static ssize_t ufshpb_sysfs_active_block_status_show(struct ufshpb_lu *hpb, char *buf)
{
	int ret = 0, count = 0, region;

	ret = snprintf(buf, PAGE_SIZE, "PINNED=%d ACTIVE=%d INACTIVE=%d\n",
			HPBREGION_PINNED, HPBREGION_ACTIVE, HPBREGION_INACTIVE);
	count = ret;

	for (region = 0 ; region < hpb->regions_per_lu ; region++) {
		ret = snprintf(buf + count, PAGE_SIZE - count, "%d:%d ", region,
			  ...
			  struct ufshpb_lu *hpb, char *buf)
{
	...
	for (region = 0; region < hpb->regions_per_lu; region++) {
		...
		for (sub = 0; sub < hpb->region_tbl[region].subregion_count;
		     sub++) {
			printk("region %d subregion %d state %d\n",
	...
	ret = snprintf(buf + count, PAGE_SIZE, "\n");
	count += ret;
