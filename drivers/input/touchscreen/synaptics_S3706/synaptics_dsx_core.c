		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		VTI("gesture type:0x%x", gesture_type);
...
int s3706_idleEnableOrDisable(int state)
{
	...
	if (g_rmi4_data != NULL) {
		...
		VTE("failed to enable or disable idle mode!");
		return MINUS_ONE;
	}
}
...
if (0) {
	VTI("power reset enter");

	if (vivoTsGetVtsData()->hasLcdShutoff == 1) {
		 VTI("lcd is off,no power reset");
		 return 0;
	}
...
static int syna_set_gesture_switch(unsigned int data)
{
	int ret;
	unsigned char temp;
	unsigned short addr = 0x1c;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	ret = synaptics_rmi4_reg_read(rmi4_data, addr, &temp, 1);
	if (ret < 0) {
		VTI("failed to read");
		goto END;
	}
	VTI("before write the switch state is %d",temp);
	...
	VTI("the data write to the addr is %d",temp);
	ret = synaptics_rmi4_reg_write(rmi4_data, addr, &temp, 1);
	if (ret < 0) {
	...
	VTI("after write the switch state is %d",temp);
...
static int syna_dclick_longpress_mode_change(int dl_switch)
{
	...
	if (dl_switch == FP_DETECT_OPEN) {
		tmp = 129;
		ret = synaptics_rmi4_reg_write(rmi4_data, 0x426, &tmp, sizeof(tmp));
		if (ret < 0) {
...
{
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	unsigned short gameModeAddr = 0x446;
	int ret = 0;
	unsigned char temp = 0;

	ret = synaptics_rmi4_reg_read(rmi4_data, gameModeAddr, &temp, 1);
		if (ret < 0) {
		VTI("failed to read addr 0x%x", gameModeAddr);
		ret = -EINVAL;
		goto exit;
	}
	VTI("before write:addr[0x%x] data[0x%x]", gameModeAddr, temp);
	if (mode >= 0) {
	...
	ret = synaptics_rmi4_reg_write(rmi4_data, gameModeAddr, &temp, 1);
	if (ret < 0) {
		VTI("failed to read addr 0x%x, data 0x%x", gameModeAddr, temp);
		ret = -EINVAL;
		goto exit;
	}
	...
	VTI("after  write:addr[0x%x] data[0x%x]", gameModeAddr, temp);
	ret = 0;
exit:
	return ret;
}
static int syna_process_by_package(unsigned char *package_name)
{
	VTI("get app name:%s", package_name);
...
static int syna_scan_rate_change (int rate)
{
	...
	ret = synaptics_rmi4_reg_read(rmi4_data, 0x445, &temp, 1);
		if (ret < 0) {
		VTI("failed to read");
		goto END;
	}
	VTI("before change, read from 445 is :0x%x", temp);
	if (temp == rate) {
	...
	ret = synaptics_rmi4_reg_write(rmi4_data, 0x445, &temp, 1);
	if (ret < 0) {
	...
	VTI("after change, read from 445 is :0x%x", temp);
...
static int syna_set_edge_bit(int state)
{
	...
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	if (vivoTsGetVtsData()->lcdRtState == 0) {
		VTI("lcd RT state is 0, not to set edge bit");
		return 0;
	}

	retval = synaptics_rmi4_reg_read(rmi4_data, 0x40E, &tmp, 1);
	if (retval < 0) {
		VTI("failed to read\n");
		return retval;
	}
	VTI("read from 40E is :0x%x", tmp);


	retval = synaptics_rmi4_reg_read(rmi4_data, 0x414, &temp, 1);
	if (retval < 0) {
		VTI("failed to read\n");
		return retval;
	}
	VTI("read from 414 is :0x%x", temp);


    if (state == 1) {
		tmp1 = tmp & 0xf7;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x40E, &tmp1, 1);
		if (retval < 0) {
		...
		/*0xc0 is for the over two coner, 0x03 is for the lower two corner*/
		temp1 = 0xc3;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x414, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to ic edge bit  %d", retval);
			return retval;
		}
		...
		temp1 = 0x20;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x415, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to the size of 0x414 edge bit  %d", retval);
			return retval;
		}
		temp1 = 0x60;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x416, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to the size of 0x415 edge bit  %d", retval);
			return retval;
		}
		temp1 = 0x3c;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x417, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to the size of 0x416 edge bit  %d", retval);
			return retval;
		}
		temp1 = 0xfa;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x418, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to the size of 0x417 edge bit  %d", retval);
			return retval;
		}
		...
		/*0x30 is for the over two coner, 0x0C is for the lower two corner*/
		temp1 = 0x3C;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x414, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to ic edge bit  %d", retval);
	...
	retval = synaptics_rmi4_reg_read(rmi4_data, 0x40E, &tmp1, 1);
	if (retval < 0) {
		VTI("failed to read 0x40E");
		return retval;
	}
	VTI("after edge setbit read from 40C is :0x%x", tmp1);
	retval = synaptics_rmi4_reg_read(rmi4_data, 0x414, &temp1, 1);
	if (retval < 0) {
		VTI("failed to read 0x414");
		return retval;
	}
	VTI("after edge setbit read from 414 is :0x%x", temp1);
		return 0;
}
...
static int syna_set_charger_bit(int state)
{
	...
	if (vivoTsGetVtsData()->lcdRtState == 0) {
		VTI("lcd RT state is 0, not to write charger");
		return 0;
	}
...
{
	VTD("iic suspend state = %d", qup_i2c_suspended);
...
static unsigned int report_lcm_id(void)
{
	...
	if (lcm_id == 0x20 || lcm_id == 0x21 || lcm_id == 0x23) {
		lcm_id = 0x00;
	}
	return lcm_id;
}
...
static int synaptics_3706_set_power(struct synaptics_rmi4_data *rmi4_data, int on)
{
	...
	if (on == 1 && has_power_on == 0) {
if (strcmp(productName, "PD1816")) {		
		if (gpio_is_valid(bdata->power_gpio)) {
			rc = gpio_direction_output(bdata->power_gpio, 1);	/* 3.3V */
			if (rc) {
		...
		if (rc < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to enable 3V power regulator\n",
					__func__);
			goto exit;
		}
}
...
if (strcmp(productName, "PD1816")) {
		...
		if (rc < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to disable 3V power regulator\n",
					__func__);
			goto exit;
		}
}
	...
	VTI("power state:%d", has_power_on);
	exit:
	return rc;
	return 0;
}

#include "pd1728a_fw.h"
#include "pd1728b_fw.h"
#include "PD1728a_limit.h"
#include "pd1814f_fw.h"
#include "pd1816_fw.h"
...
int syna_bbk_regulator_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->bus_reg = regulator_get(rmi4_data->pdev->dev.parent,
			"vcc_i2c");
	if (IS_ERR(rmi4_data->bus_reg)) {
		VTI("Failed to get 1.8V i2c regulator");
		retval = PTR_ERR(rmi4_data->bus_reg);
		goto regulator_put1;
	}
	retval = regulator_set_voltage(rmi4_data->bus_reg, 1850000,
								1850000);
	if (retval) {
...
if (!strcmp(productName, "PD1816")) {	
	rmi4_data->pwr_reg = regulator_get(rmi4_data->pdev->dev.parent,
			"vcc_power");
	if (IS_ERR(rmi4_data->pwr_reg)) {
		VTI("Failed to get 3V power regulator");
		retval = PTR_ERR(rmi4_data->pwr_reg);
		goto regulator_put2;
	}
	retval = regulator_set_voltage(rmi4_data->pwr_reg, 3000000,
								3000000);
	if (retval) {
		VTI("regulator set_vtg:vcc_power failed retval = %d", retval);
		return retval;
	}
}	
	return 0;
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	vts_data->sensorTestKey = "com.vivotouchscreen.sensortest:ActivityVIVOProduction:com.vivotouchscreen.sensortest:0:com.vivotouchscreen.sensortest";
	vts_data->lcmNoiseTestKey = "com.vivotouchscreen.deltadiff:MainActivity:null:null:null";

	vivoTsFwAdd(VTS_FW_TYPE_FW, 	pd1728a_fw, 	sizeof(pd1728a_fw), 	"PD1728A", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_LIMIT, 	(const unsigned char *)pd1728a_limit, 	sizeof(pd1728a_limit), 	"PD1728A", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, 	pd1728b_fw, 	sizeof(pd1728b_fw), 	"PD1728B", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_LIMIT, 	(const unsigned char *)pd1728a_limit, 	sizeof(pd1728a_limit), 	"PD1728B", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, 	pd1814f_fw, 	sizeof(pd1814f_fw), 	"PD1814F_EX", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_LIMIT, 	(const unsigned char *)pd1728a_limit, 	sizeof(pd1728a_limit), 	"PD1814F_EX", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, 	pd1816_fw, 	sizeof(pd1816_fw), 	"PD1816", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_LIMIT, 	(const unsigned char *)pd1728a_limit, 	sizeof(pd1728a_limit), 	"PD1816", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	...
	retval = syna_S3706_pin_ctr(pin_config.pin_active);
	if (retval > 0) {
		VTI("Pinctrl active failed");
		goto err_init_reg;
	}
