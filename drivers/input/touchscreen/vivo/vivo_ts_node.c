					...
					struct kobj_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 2048, "bad_screen:%s\nlcm_noise:%s\n", vivo_ts_data->sensorTestKey, vivo_ts_data->lcmNoiseTestKey);
}
		...
		if (val < 0x80) {
			if (val == 2 || val == 3) {
				...
				if (val == 6) {
					...
					vivo_ts_data->fingerIcoState = 1;
					vivo_ts_data->scanRateChange(1);
				} else if (val == 7) {
					VTI("The Fingerprint icon is extinguish");
					vivo_ts_data->fingerIcoState = 0;
					vivo_ts_data->scanRateChange(3);
				}
			}
		} else {
			VTI("Invalide parameter passed(%d)", val);
