...
int vivoTsInputReport(int event, int touchIdOrGestureEvent, int x, int y, int z)
{
	...
	case VTS_TOUCH_DOWN:
		...
		input_report_abs(vivoTsData->inputDev, ABS_MT_TOUCH_MAJOR, z);
		input_report_abs(vivoTsData->inputDev, ABS_MT_PRESSURE, z);
		input_sync(vivoTsData->inputDev);
		if (touchIdOrGestureEvent == 0) {
			if (vivoTsData->keyType != VTS_KEY_NOKEY) {
...
*/
	...
	if (vivoTsData->icNum == VTS_IC_TD4322 || vivoTsData->icNum == VTS_IC_TD3320 || vivoTsData->icNum == VTS_IC_S3706 || vivoTsData->icNum == VTS_IC_NT36525
		|| vivoTsData->icNum == VTS_IC_NT36670 || vivoTsData->icNum == VTS_IC_NT36772) {
				 ...
				 unsigned long event, void *data)
{
	...
	if (event == vivoTsData->resumeEventBlank) {	/*wake_p*/
		...
		if (*blank == FB_BLANK_UNBLANK && fb_suspend_flag == 1) {
			...
			if (vivoTsData->icNum == VTS_IC_TD4322 || vivoTsData->icNum == VTS_IC_FT8006 || vivoTsData->icNum == VTS_IC_NT36525 || vivoTsData->icNum == VTS_IC_TD4330 
				|| vivoTsData->icNum == VTS_IC_FT8719 || vivoTsData->icNum == VTS_IC_NT36670 || vivoTsData->icNum == VTS_IC_NT36772) {
	...
	if (event == vivoTsData->suspendEventBlank) {	/* wake_p */
		...
		if (*blank == FB_BLANK_POWERDOWN && fb_suspend_flag == 0) {
			...
			if (vivoTsData->icNum == VTS_IC_TD4322 || vivoTsData->icNum == VTS_IC_TD3320 || vivoTsData->icNum == VTS_IC_FT8006 || vivoTsData->icNum == VTS_IC_NT36772
				|| vivoTsData->icNum == VTS_IC_NT36670 || vivoTsData->icNum == VTS_IC_NT36525 || vivoTsData->icNum == VTS_IC_TD4330 || vivoTsData->icNum == VTS_IC_FT8719) {
		...
		if (vivoTsData->icNum == VTS_IC_TD4322 || vivoTsData->icNum == VTS_IC_TD3320 || vivoTsData->icNum == VTS_IC_NT36772
			|| vivoTsData->icNum == VTS_IC_NT36670 || vivoTsData->icNum == VTS_IC_NT36525) {
			blank = evdata->data;		/* 0:wakeup 4:suspend */
			if (*blank == FB_BLANK_POWERDOWN && fb_suspend_flag == 0) {
...
*/
		...
		if (vtsData->icNum == VTS_IC_TD4322 || vtsData->icNum == VTS_IC_TD3320 || vtsData->icNum == VTS_IC_FT8006  || vtsData->icNum == VTS_IC_NT36772
			|| vtsData->icNum == VTS_IC_NT36670 || vtsData->icNum == VTS_IC_NT36525 || vtsData->icNum == VTS_IC_TD4330 || vtsData->icNum == VTS_IC_FT8719) {
