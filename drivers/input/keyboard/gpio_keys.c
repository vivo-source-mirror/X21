...
struct gpio_button_data {
	struct gpio_keys_button *button; /*change by vivo sensor team*/
...
{
	/*change by vivo sensor team start*/
	struct gpio_keys_button *button = bdata->button;
	/*change by vivo sensor team end*/
	...
	/*ignore the same state report by wangle*/
	if (state == button->last_state) {
		printk(KERN_ERR "PMIC input: code=%d, ignore the same state sts=0x%hhx\n", button->code, state);
		return;
	} else {
...
{
	struct gpio_button_data *bdata = dev_id;
	/*change by vivo sensro team start*/
	struct gpio_keys_button *button = bdata->button;
	/*change by vivo sensro team end*/
	struct input_dev *input = bdata->input;
	unsigned long flags;
...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		/*change by vivo sensor team start*/
		button->last_state = 0xff; /* init button last_state is 0xff*/
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		struct gpio_keys_button *button = &pdata->buttons[i]; /*change by vivo sensor team*/
		struct gpio_button_data *bdata = &ddata->data[i];
...
static int gpio_keys_resume(struct device *dev)
{
	...
	if (error)
		return error;
     /*change by vivo sensor team start*/
	/*resume can not report, this can cause double report(wake func)
	gpio_keys_report_state(ddata);*/
	return 0;
}
