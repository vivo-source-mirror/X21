...
#define BLK_LP_RESERVE_DEPTH	2
#define BLK_THRES_ON	8
...
struct request {
	...
	struct list_head prio_dist_list;
	union {
...
struct blk_queue_tag {
	...
	int max_lp_count;		/* what we will send to device from bg thread */
...
struct request_queue {
	...
	struct list_head	hp_head;
	struct list_head	lp_head;
	unsigned int		in_processing[2];
...
#ifdef CONFIG_BLK_DEV_REQ_PRIO
static inline void blk_inc_request(struct request_queue *queue,
					    struct request *req, bool front)
{
	...
	if (!front)
		list_add_tail(&req->prio_dist_list, head);
	else
		list_add(&req->prio_dist_list, head);
}

static inline void blk_inc_processing(struct request_queue *queue,
					     struct request *req)
{
	if (req->cmd_flags & REQ_FG)
		queue->in_processing[BLK_RW_HI_PRIO]++;
	else
		queue->in_processing[BLK_RW_LOW_PRIO]++;
}

static inline void blk_del_processing(struct request_queue *queue,
					     struct request *req)
{
	if (req->cmd_flags & REQ_FG)
		queue->in_processing[BLK_RW_HI_PRIO]--;
