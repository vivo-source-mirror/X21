...
DECLARE_EVENT_CLASS(writeback_work_class,
	...
	TP_fast_assign(
		...
		__entry->work_id = work->work_id;
		strlcpy(__entry->comm, work->comm, 16);
	),
	TP_printk("bdi %s: sb_dev %d:%d nr_pages=%ld sync_mode=%d "
		  "kupdate=%d range_cyclic=%d background=%d reason=%s cgroup=%s workid=%ld comm=%s",
		  __entry->name,
