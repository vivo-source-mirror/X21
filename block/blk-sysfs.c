...
static u64 wbt_default_latency_nsec(struct request_queue *q)
{
	 ...
	 * We use emmc/ufs is non-rotational storage.
	 */
	return 2000000ULL;
}

static int blk_wb_init(struct request_queue *q)
{
	struct rq_wb *rwb;
...
static ssize_t queue_wc_show(struct request_queue *q, char *page)
{
	if (test_bit(QUEUE_FLAG_WC, &q->queue_flags))
		return snprintf(page, 64, "write back\n");

	return snprintf(page, 64, "write through\n");
}
...
{
	return snprintf(page, 256, "%s samples=%llu, mean=%lld, min=%lld, max=%lld\n",
...
static ssize_t queue_max_lp_count_show(struct request_queue *q, char *page)
{
	ssize_t ret;
	...
	ret = snprintf(page, 4096, "%d\n", q->queue_tags->max_lp_count);
	return ret;
}

static ssize_t queue_max_lp_count_store(struct request_queue *q,
					const char *page, size_t count)
{
	unsigned long val;
	int ret;
...
static struct queue_sysfs_entry queue_max_lp_count_entry = {
	.attr = {.name = "max_lp_count", .mode = S_IRUGO | S_IWUSR },
	.show = queue_max_lp_count_show,
	.store = queue_max_lp_count_store,
};
...
int blk_register_queue(struct gendisk *disk)
{
	...
	ret = blk_wb_init(q);
	if (ret)
		pr_err("blk_wb_init init failed \n");
