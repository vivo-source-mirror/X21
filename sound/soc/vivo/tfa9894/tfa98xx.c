...
static int tfa9874_calibrate(struct tfa98xx *tfa98xx, int once)
{
	...
	err = tfa98xx_set_mtp_v1(tfa98xx->tfa, 0, (1<<TFA_MTP_OTC) | (1<<TFA_MTP_OTC));/* val mask */
	if (err)
		pr_err("MTP write failed\n");
	
	pr_info("tfaContWriteRegsProf start\n");
	/* write all the registers from the profile list */
	err = tfaContWriteRegsProf_v1(tfa98xx->tfa, cal_profile);
	if (err) {
	...
	err = tfaContWriteFilesProf_v1(tfa98xx->tfa, cal_profile, 0);
	if (err) {
	...
	err = tfa_dsp_cmd_id_write_read_v1(tfa98xx->tfa, MODULE_FRAMEWORK, FW_PAR_ID_GET_STATUS_CHANGE, 3, (unsigned char *)buffer);

	/* We need to send zero's to trigger calibration */
	memset(bytes, 0, 6);
	err = tfa_dsp_cmd_id_write_v1(tfa98xx->tfa, MODULE_SPEAKERBOOST, SB_PARAM_SET_RE25C, sizeof(bytes), bytes);
	...
	imp = tfa_get_calibration_info_v1(tfa98xx->tfa, 0);

	pr_info("Calibration value: %d.%d ohm\n", imp/1000, imp%1000);

	if (once) {
		err = tfa98xx_set_mtp_v1(tfa98xx->tfa, (1<<TFA_MTP_OTC), (1<<TFA_MTP_OTC));/* val mask */
		if (err)
...
static ssize_t tfa98xx_dbgfs_read(struct file *file,
										...
										loff_t *pos)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;	const int size = 512;
	char buffer[size];
	int n = 0;
