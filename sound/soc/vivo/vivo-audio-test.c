...
static int vivo_adsp_restart(void)
{
	int ret = 0;
	struct apr_hdr hdr;
...
static int __init vivo_audio_test_init(void)
{
	int ret = 0;

	vivo_apr_init();
	ret = misc_register(&vivo_audio_test_device);

	return ret;
}

device_initcall(vivo_audio_test_init);
