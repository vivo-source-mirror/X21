...
#define TFA98XX_NOMINAL_IMPEDANCE_MAX 9500 //shijianxing add for max impdance
#define TFA98XX_NOMINAL_IMPEDANCE_MIN 6500//shijianxing add for min impdance
...
static int tfa98xx_runtime_verbose = 1;
...
static void tfa_set_query_info (int dev_idx)
{
	...
	case 0x12:
		...
		break;
	default:
		pr_err("[SmartPA-%d]set_query_info: unknown device type : 0x%02x\n", __LINE__,handles_local[dev_idx].rev);
		_ASSERT(0);
		break;
	}
}
...
// TODO: how do we deal with old bugs?
		...
		if (Tfa98xx_Error_Ok != error) {
			...
			pr_debug("\n[SmartPA-%d]probe:Error: Unable to read revid from slave:0x%02x \n", __LINE__,slave_address/2);
			return error;
		}
		...
		*pHandle = idx;
		...
		tfa98xx_trace_printk("tfa98xx: slave:0x%02x revid:0x%04x\n", slave_address, rev);
		pr_debug("[SmartPA-%d]probe: slave:0x%02x revid:0x%04x\n", __LINE__,slave_address, rev);
#endif
		break;
	default:
		pr_info("[SmartPA-%d]probe: Unknown slave adress! \n",__LINE__);
		/* wrong slave address */
...
tfa98xx_open(Tfa98xx_handle_t handle) {
	...
	{
		pr_err("[SmartPA-%d]open:Error_InUse.\n",__LINE__);
		return Tfa98xx_Error_InUse;
	} else
	{
		handles_local[handle].in_use = 1;
		pr_info("[SmartPA-%d]open:Ok.\n",__LINE__);
...
enum Tfa98xx_Error tfa98xx_close(Tfa98xx_handle_t handle)
{
	if (tfa98xx_handle_is_open(handle)) {
		...
		pr_err("[SmartPA-%d]close:Ok.\n",__LINE__);
		return Tfa98xx_Error_Ok;
	} else {
		pr_err("[SmartPA-%d]close:Error_NotOpen.\n",__LINE__);
...
enum Tfa98xx_Error tfa98xx_dsp_reset(Tfa98xx_handle_t dev_idx, int state)
{
	...
	pr_info("[SmartPA-%d]dsp_reset:reset\n",__LINE__);
        ...
        int *ready)
{
	...
	pr_info("[SmartPA-%d]dsp_system_stable:system stable\n",__LINE__);

	/* check the contents of the STATUS register */
	value = TFA_READ_REG(handle, AREFS);
	pr_info("[SmartPA-%d]dsp_system_stable: AREFS %d\n", __LINE__,value);
	if (value < 0) {
		error = -value;
		*ready = 0;
	...
	*ready = !((TFA_GET_BF_VALUE(handle, AREFS, status) == 0)
	...
	pr_info("[SmartPA-%d]dsp_system_stable::AREFS | CLKS %d\n", __LINE__,*ready);
...
void tfa98xx_apply_deferred_calibration(Tfa98xx_handle_t handle)
{
	...
	pr_info("[SmartPA-%d]deferred_calibration:otc.triggered = %d\n",__LINE__,controls->otc.triggered);
	if (controls->otc.deferrable && controls->otc.triggered) {
		pr_info("[SmartPA-%d]deferred_calibration:Deferred writing otc = %d\n", __LINE__,controls->otc.wr_value);
		...
		if (err != Tfa98xx_Error_Ok) {
			pr_err("[SmartPA-%d]deferred_calibration: Unable to apply deferred MTP OTC write. Error=%d\n",__LINE__,
			       err);
		} else {
	...
	if (controls->mtpex.deferrable && controls->mtpex.triggered) {
		pr_info("[SmartPA-%d]deferred_calibration: Deferred writing mtpex = %d\n",__LINE__, controls->mtpex.wr_value);
		err = tfa98xx_set_mtp(handle,
		                      (uint16_t)controls->mtpex.wr_value << TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_POS,
		                      1 << TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_POS);
		if (err != Tfa98xx_Error_Ok) {
			pr_err("[SmartPA-%d]deferred_calibration:: Unable to apply deferred MTPEX write. Rrror=%d\n",__LINE__,
			       err);
		} else {
	...
	if (controls->calib.triggered) {
		...
		if(err) {
			pr_info("[SmartPA-%d]deferred_calibration: Deferred calibration failed: %d\n", __LINE__,err);
		} else {
			pr_info("[SmartPA-%d]deferred_calibration: Deferred calibration ok\n",__LINE__);
...
enum Tfa98xx_Error tfa98xx_dsp_system_stable(Tfa98xx_handle_t dev_idx, int *ready)
{
	...
	pr_info("[SmartPA-%d]dsp_system_stable:stable\n",__LINE__);
...
enum Tfa98xx_Error tfa98xx_init(Tfa98xx_handle_t handle)
{
	...
	case 0x80:
		pr_debug("[SmartPA-%d]init: Dev ID %x\n",__LINE__,(TFA_GET_BF(handle, REV) & 0xff));
		break;
	default:
...
enum Tfa98xx_Error tfa98xx_dsp_write_tables(Tfa98xx_handle_t handle, int sample_rate)
{
	...
	pr_info("[SmartPA-%d]dsp_write_tables:sample_rate:%d.\n",__LINE__, sample_rate);
...
enum Tfa98xx_Error tfa98xx_get_mtp(Tfa98xx_handle_t handle, uint16_t *value)
{
	...
	pr_info("[SmartPA-%d]get_mtp\n",__LINE__);

	/* not possible if PLL in powerdown */
	if ( TFA_GET_BF(handle, PWDN) ) {
		pr_debug("[SmartPA-%d]get_mtp:: PLL in powerdown\n",__LINE__);
	...
	if (tfa98xx_dev_family(handle) == 1) {
		tfa98xx_dsp_system_stable(handle, &status);
		if (status==0) {
			pr_debug("[SmartPA-%d]get_mtp:: PLL not running\n",__LINE__);
			return Tfa98xx_Error_NoClock;
		}
	}
	...
	*value = (uint16_t)result;
	pr_info("[SmartPA-%d]get_mtp:mtp value %d.\n",__LINE__, *value);
                                   ...
                                   uint16_t mask)
{
	...
	if (error)
		return error;
	if (status==0) {
		pr_err("[SmartPA-%d]set_mtp:status = 0, No clock.\n",__LINE__);
		return Tfa98xx_Error_NoClock;
	}
	...
	for(loop=0; loop<100 /*x10ms*/ ; loop++) {
		...
		if (TFA_GET_BF(handle, MTPB) == 0) {
			pr_info("[SmartPA-%d]set_mtp: MTPB 0",__LINE__);
			return Tfa98xx_Error_Ok;
		}
	}
...
int tfa_calibrate(Tfa98xx_handle_t handle)
{
	...
	pr_info("[SmartPA-%d]dsp:calibrate\n",__LINE__);
...
void tfa98xx_set_exttemp(Tfa98xx_handle_t handle, short ext_temp)
{
	if ((-256 <= ext_temp) && (ext_temp <= 255)) {
		...
		pr_debug("[SmartPA-%d]set_exttemp: Using ext temp %d C\n", __LINE__,twos(ext_temp));
		TFA_SET_BF(handle, TROS, 1);
		TFA_SET_BF(handle, EXTTS, twos(ext_temp));
	} else {
		pr_debug("[SmartPA-%d]set_exttemp: Clearing ext temp settings\n",__LINE__);
...
enum Tfa98xx_Error tfa98xx_set_volume_level(Tfa98xx_handle_t handle, unsigned short vol)
{
	...
	pr_info("[SmartPA-%d]set_volume_level:volume %d.\n",__LINE__, vol);
...
tfa98xx_set_mute_tfa1(Tfa98xx_handle_t handle, enum Tfa98xx_Mute mute) {
	...
	{
	case Tfa98xx_Mute_Off:
		pr_info("[SmartPA-%d]mute_tfa1:Mute_Off.\n",__LINE__);
		...
		break;
	case Tfa98xx_Mute_Digital:
		pr_info("[SmartPA-%d]mute_tfa1:Mute_Digital.\n",__LINE__);
		...
		break;
	case Tfa98xx_Mute_Amplifier:
		pr_info("[SmartPA-%d]mute_tfa1:Mute_Amplifier.\n",__LINE__);
		...
		break;
	default:
		pr_info("[SmartPA-%d]mute_tfa1:Error_Bad_Parameter.\n",__LINE__);
		return Tfa98xx_Error_Bad_Parameter;
	}
...
tfa98xx_set_mute(Tfa98xx_handle_t handle, enum Tfa98xx_Mute mute) {
	if (!tfa98xx_handle_is_open(handle))
	{
		pr_err("[SmartPA-%d]set_mute: device not opened\n",__LINE__);
		return Tfa98xx_Error_NotOpen;
	}
...
tfa98xx_process_patch_file(Tfa98xx_handle_t handle, int length,
	...
	pr_info("[SmartPA-%d]process_patch_file:len %d.\n",__LINE__, length);
	...
	while (index < length)
	{
		...
		if ((index + size) > length) {
			...
			pr_err("[SmartPA-%d]process_patch_file:outside the buffer, error in the input data.\n",__LINE__);
			return Tfa98xx_Error_Bad_Parameter;
		}

		if (size > handles_local[handle].buffer_size) {
			/* too big, must fit buffer */
			pr_err("[SmartPA-%d]process_patch_file:too big, must fit buffer.\n",__LINE__);
			return Tfa98xx_Error_Bad_Parameter;
		}

		error = tfa98xx_write_raw(handle, size, &bytes[index]);
		if (error != Tfa98xx_Error_Ok) {
			pr_err("[SmartPA-%d]process_patch_file:writing dsp patch failed %d\n", __LINE__, error);
			break;
		}
		index += size;
	}
...
tfa98xx_check_ic_rom_version(Tfa98xx_handle_t handle, const unsigned char patchheader[]) {
	...
	{
		pr_debug("[SmartPA-%d]check_ic_rom_version:checkrev = %d lsb_revid = %d\n",__LINE__,checkrev,lsb_revid);
	...
	if (checkaddress != 0xFFFF)
	{
		...
		if (error == Tfa98xx_Error_Ok) {
			if (value != checkvalue) {
				pr_err("[SmartPA-%d]check_ic_rom_version: patch file romid type check failed [0x%04x]: expected 0x%02x, actual 0x%02x\n",__LINE__,
	...
	} else   /* == 0xffff */
	{
		...
		if ( checkvalue != 0xFFFFFF && checkvalue != 0) {
			...
			if ( revid != handles_local[handle].rev) {
				pr_err("[SmartPA-%d]check_ic_rom_version: patch file device type check failed: expected 0x%02x, actual 0x%02x\n",__LINE__,
...
tfa_dsp_patch(Tfa98xx_handle_t handle, int patchLength,
	...
	pr_info("[SmartPA-%d]check_ic_rom_version:patch\n",__LINE__);
	...
	if (Tfa98xx_Error_Ok != error)
	{
		pr_err("[SmartPA-%d]check_ic_rom_version:Error, patch in container file does not match device!\n",__LINE__);
		return error;
	}
	error = tfa98xx_process_patch_file(handle, patchLength - PATCH_HEADER_LENGTH,
	                                   patchBytes + PATCH_HEADER_LENGTH);
	if (error)
		pr_err("[SmartPA-%d]check_ic_rom_version::Error, process_patch_file  return error!\n",__LINE__);
...
tfa98xx_wait_result(Tfa98xx_handle_t handle, int wait_retry_count) {
	...
	if (error == Tfa98xx_Error_DSP_not_running && log_cnt)
	{
		uint16_t regvalue[3];
		tfa98xx_read_register16(handle, 0x0, &regvalue[0]);
		tfa98xx_read_register16(handle, 0x9, &regvalue[1]);
		tfa98xx_read_register16(handle, 0x73, &regvalue[2]);

		pr_err("%s: tfa Error DSP not running, wait_result "
		       "regvalue[0]:0x%x, regvalue[1]:0x%x, regvalue[2]:0x%x.\n",
		       __func__, regvalue[0], regvalue[1], regvalue[2]);
...
enum Tfa98xx_Error tfa_dsp_msg(Tfa98xx_handle_t handle, int length, const char *buf)
{
	...
	if (rpc_status != Tfa98xx_I2C_Req_Done) {
		...
		pr_debug("[SmartPA-%d]dsp_msg: DSP msg status: %d (%s)\n",
		         __LINE__, rpc_status,
		         tfa98xx_get_i2c_status_id_string(rpc_status));
		pr_info("[SmartPA-%d]dsp_msg: buf[0]:0x%x, buf[1]:0x%x, buf[2]:0x%x.\n",
		        __LINE__, buf[0], buf[1], buf[2]);
	}
...
enum Tfa98xx_Error tfa_dsp_msg_id(Tfa98xx_handle_t handle, int length, const char *buf, uint8_t cmdid[3])
{
	...
	if (rpc_status != Tfa98xx_I2C_Req_Done) {
		...
		pr_debug("[SmartPA-%d]dsp_msg: DSP msg status: %d (%s)\n",
		         __LINE__, rpc_status,
		         tfa98xx_get_i2c_status_id_string(rpc_status));
		pr_info("[SmartPA-%d]tfa_dsp_msg_id: cmdid[0]:0x%x, cmdid[1]:0x%x, cmdid[2]:0x%x.\n",
		        __LINE__, cmdid[0], cmdid[1], cmdid[2]);
	}
...
enum Tfa98xx_Error tfa98xx_powerdown(Tfa98xx_handle_t handle, int powerdown)
{
	...
	pr_info("[SmartPA-%d]dsp:powerdown %d.\n",__LINE__, powerdown);
...
int tfa_set_bf(Tfa98xx_handle_t dev_idx, const uint16_t bf, const uint16_t value)
{
	...
	if (err) {
		pr_err("[SmartPA-%d]set_bf: Error getting bf :%d \n", __LINE__,-err);
		return -err;
	}
	...
	if(oldvalue != regvalue) {
		...
		if (err) {
			pr_err("[SmartPA-%d]set_bf: Error setting bf :%d \n", __LINE__,-err);
			return -err;
		}
	}
...
int tfa_set_bf_volatile(Tfa98xx_handle_t dev_idx, const uint16_t bf, const uint16_t value)
{
	...
	err = tfa98xx_read_register16(dev_idx, address, &regvalue);
	if (err) {
		pr_err("[SmartPA-%d]bf_volatile: Error getting bf :%d \n",__LINE__, -err);
		return -err;
	}
	...
	err = tfa98xx_write_register16(dev_idx, address, regvalue);
	if (err) {
		pr_err("[SmartPA-%d]bf_volatile: Error setting bf :%d \n",__LINE__, -err);
		return -err;
	}
...
int tfa_get_bf(Tfa98xx_handle_t dev_idx, const uint16_t bf)
{
	...
	err = tfa98xx_read_register16(dev_idx, address, &regvalue);
	if (err) {
		pr_err("[SmartPA-%d]get_bf: Error getting bf :%d \n",__LINE__, -err);
		return -err;
	}
...
enum Tfa98xx_Error tfa_cf_powerup(Tfa98xx_handle_t handle)
{
	...
	if (tfa98xx_runtime_verbose)
		pr_info("[SmartPA-%d]cf_powerup: Waiting for DSP system stable...\n",__LINE__);
	...
	if (tries==0) {// timedout
		pr_err("[SmartPA-%d]cf_powerup:DSP subsystem start timed out\n",__LINE__);
		return Tfa98xx_Error_StateTimedOut;
	}
...
enum Tfa98xx_Error tfa98xxSetCalibrateOnce(Tfa98xx_handle_t handle)
{
	...
	pr_info("[SmartPA-%d]SetCalibrateOnce:\n",__LINE__);
...
enum Tfa98xx_Error show_current_state(Tfa98xx_handle_t handle)
{
	...
	printk("[SmartPA-%d]current_state: Current HW manager state: ",__LINE__);

	switch(manstate) {
	case 0:
		printk("[SmartPA-%d]current_state: power_down_state \n",__LINE__);
		break;
	case 1:
		printk("[SmartPA-%d]current_state: wait_for_source_settings_state \n",__LINE__);
		break;
	case 2:
		printk("[SmartPA-%d]current_state: connnect_pll_input_state \n",__LINE__);
		break;
	case 3:
		printk("[SmartPA-%d]current_state: disconnect_pll_input_state \n",__LINE__);
		break;
	case 4:
		printk("[SmartPA-%d]current_state: enable_pll_state \n",__LINE__);
		break;
	case 5:
		printk("[SmartPA-%d]current_state: enable_cgu_state \n",__LINE__);
		break;
	case 6:
		printk("[SmartPA-%d]current_state: init_cf_state \n",__LINE__);
		break;
	case 7:
		printk("[SmartPA-%d]current_state: enable_amplifier_state \n",__LINE__);
		break;
	case 8:
		printk("[SmartPA-%d]current_state: alarm_state \n",__LINE__);
		break;
	case 9:
		printk("[SmartPA-%d]current_state: operating_state \n",__LINE__);
		break;
	case 10:
		printk("[SmartPA-%d]current_state: mute_audio_state \n",__LINE__);
		break;
	case 11:
		printk("[SmartPA-%d]current_state:disable_cgu_pll_state \n",__LINE__);
		break;
	default:
		printk("[SmartPA-%d]current_state: Unable to find current state \n",__LINE__);
		break;
	}
...
enum Tfa98xx_Error tfaRunSpeakerBoost(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	pr_info("[SmartPA-%d]SpeakerBoost:SpeakerBoost\n",__LINE__);
	...
	tfa98xx_trace_printk("SmartPA:%s %sstart\n",
	...
	if ((value == 1) && (!istap_prof)) {
		...
		if (TFA_GET_BF(handle, CFE) == 0) {
			pr_debug("[SmartPA-%d]SpeakerBoost: TFA_GET_BF err= %d ",__LINE__,err);
			return err;
		}
		...
		/* Restart the dsp */
		//add for speaker impedance check
	} else if (istap_prof)  {
		/* Save the current profile and set the vstep to 0 */
...
enum Tfa98xx_Error tfaRunSpeakerStartup(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	pr_info("[SmartPA-%d]SpeakerStartup:\n",__LINE__);
	pr_info("[SmartPA-%d]SpeakerStartup: coldstart%s :", __LINE__,force? " (forced)":"");
...
enum Tfa98xx_Error tfaRunSpeakerCalibration(Tfa98xx_handle_t handle, int profile)
{
	...
	pr_info("[SmartPA-%d]SpeakerCalibration:\n",__LINE__);
	...
	if (err != Tfa98xx_Error_Ok) {
		if ((tfa98xx_dev_family(handle) == 2 && TFA_GET_BF(handle, REFCKSEL) == 1)) {
			pr_err("[SmartPA-%d]SpeakerCalibration: Unable to calibrate the device with the internal clock! \n",__LINE__);
		}
	}
	if(err == Tfa98xx_Error_Ok) {
		...
		if (spkr_count == 1) {
			printk("[SmartPA-%d]SpeakerCalibration:%d.%d Ohms \n", __LINE__,handles_local[handle].mohm[0]/1000,
			       handles_local[handle].mohm[0]%1000);
		} else {
			printk("[SmartPA-%d]SpeakerCalibration: Prim:%d mOhms, Sec:%d mOhms\n",__LINE__,
			       handles_local[handle].mohm[0],
			       handles_local[handle].mohm[1]);
		}
	}
...
void tfa_verbose(int level)
{
	...
	tfa98xx_runtime_verbose = 1;//shijianxing add for debug
...
#define CF_CONTROL 0x8100
	...
	pr_info("[SmartPA-%d]Coldboot:Coldboot\n",__LINE__);
	...
	while ( state != TFA_GET_BF(handle, ACS)) {
		...
		if (tries-- == 0) {
			pr_debug("[SmartPA-%d]Coldboot:coldboot (ACS) did not %s\n",__LINE__, state ? "set":"clear");
...
enum Tfa98xx_Error tfaRunStartup(Tfa98xx_handle_t handle, int profile)
{
	...
	pr_info("[SmartPA-%d]Startup:Startup\n",__LINE__);
	...
	if(!noinit) {
		...
		pr_debug("\nWarning: SmartPA No init keyword found in the cnt file. Init is skipped! \n");
	}
	...
	case 0x80:
		...
		pr_debug("SmartPA Startup Reset DSP after power on\n");
		break;
	default:
		break;
	}
	...
	if (tfa98xx_runtime_verbose) {
		if (TFA_GET_BF(handle, NOCLK) && (tfa98xx_dev_family(handle) == 2))
			pr_debug("SmartPA Using internal clock\n");
		pr_debug("SmartPA: Waiting for DSP system stable...\n");
	}
	...
	if (tries == CFSTABLE_TRIES) {
		if (tfa98xx_runtime_verbose) pr_err("SmartPA: Timed out\n");
		return Tfa98xx_Error_StateTimedOut;
	}  else if (tfa98xx_runtime_verbose) printk("SmartPA: OK (tries=%d)\n", tries);
...
enum Tfa98xx_Error tfaRunColdStartup(Tfa98xx_handle_t handle, int profile)
{
	...
	pr_info("[SmartPA-%d]ColdStartup:ColdStartup\n",__LINE__);
...
enum Tfa98xx_Error tfaRunWaitCalibration(Tfa98xx_handle_t handle, int *calibrateDone)
{
	...
	pr_info("[SmartPA-%d]WaitCalibration:wait calibration\n",__LINE__);

	*calibrateDone = 0;

	/* in case of calibrate once wait for MTPEX */
	if (TFA_GET_BF(handle, MTPOTC)) {
		printk("SmartPA: Calibration Once time.\n");
		...
		pr_info("[SmartPA-%d]WaitCalibration: 1st read status 0x%x.\n", __LINE__, tfa9891_read_reg(TFA98XX_STATUSREG));
		if(tries_mtp_busy < MTPBWAIT_TRIES) {
			...
			pr_info("[SmartPA-%d]WaitCalibration: 2nd read status 0x%x.\n", __LINE__, tfa9891_read_reg(TFA98XX_STATUSREG));
			if (tries >= MTPEX_WAIT_NTRIES) {
				tries = TFA98XX_API_WAITRESULT_NTRIES;
			}
		} else {
			pr_err("SmartPA: MTP bussy after %d tries\n", MTPBWAIT_TRIES);
		}
	}
	pr_info("[SmartPA-%d]WaitCalibration:3rd read status 0x%x.\n", __LINE__, tfa9891_read_reg(TFA98XX_STATUSREG));
	...
	pr_info("[SmartPA-%d]WaitCalibration: 4th read status 0x%x.\n", __LINE__, tfa9891_read_reg(TFA98XX_STATUSREG));

	if(*calibrateDone != 1) {
		pr_err("SmartPA: Calibration failed! \n");
		err = Tfa98xx_Error_Bad_Parameter;
	}
	if (tries==TFA98XX_API_WAITRESULT_NTRIES) {
		pr_err("SmartPA: Calibration has timedout! \n");
		//	err = Tfa98xx_Error_StateTimedOut;
	}
	if(tries_mtp_busy == 50) {
		pr_err("SmartPA: Calibrate Failed: MTP_busy stays high! \n");
		//	err = Tfa98xx_Error_StateTimedOut;
	}
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
...
#endif
	if ( devcount < 1 ) {
		pr_err("SmartPA: No or wrong container file loaded\n");
		return	tfa_error_bad_param;
	}
	pr_info("[SmartPA-%d]start:devcount %d\n",__LINE__,devcount);
	for( dev=0; dev < devcount; dev++) {
		...
		if (cal_profile >= 0) {
			next_profile = 0;
			pr_debug("SmartPA: Loading %s profile! \n", tfaContProfileName(dev, next_profile));
		}
	}
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if(strstr(tfaContProfileName(dev, next_profile), ".standby") != NULL) {
			...
			/* SL: For power up we need to check system stable */
...
enum tfa_error tfa_stop(void)
{
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if ( err != Tfa98xx_Error_Ok)
			goto error_exit;
		if (tfa98xx_runtime_verbose)
			printk("SmartPA: Stopping device [%s]\n", tfaContDeviceName(dev));
		/* mute */
		tfaRunMute(dev);
		/* powerdown CF */
		err = tfa98xx_powerdown(dev, 1 );
...
enum tfa_error tfa_reset(void)
{
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if ( err != Tfa98xx_Error_Ok)
			break;
		if (tfa98xx_runtime_verbose)
			pr_debug("SmartPA: resetting device [%s]\n", tfaContDeviceName(dev));
...
tfa98xx_write_data(Tfa98xx_handle_t handle,
	...
	{
		pr_info("SmartPA_Error_Bad_Parameter.\n");
		return Tfa98xx_Error_Bad_Parameter;
	}
...
tfa_dsp_get_calibration_impedance(Tfa98xx_handle_t handle) {
	...
	pr_info("[SmartPA-%d]calibration_impedance:Get impedance\n",__LINE__);
	...
	if (error == Tfa98xx_Error_Ok)
	{
		...
		if (TFA_GET_BF(handle, MTPOTC) && ((handles_local[handle].rev & 0xff) == 0x88)) {
			if (tfa98xx_runtime_verbose)
				pr_info("SmartPA: Getting calibration values from MTP\n");
			...
			if (tfa98xx_runtime_verbose)
				pr_info("SmartPA: Getting calibration values from Speakerboost\n");
