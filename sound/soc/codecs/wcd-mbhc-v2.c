...
#define HS_DETECT_PLUG_TIME_MS 2800
...
// @leichenji
#define VIVO_HP_IMPED_THRHLD	100		// 610 means actual imped is larger than 18k ohm
#define VIVO_MIC_IMPED_THRHLD	360		// 360 mapping to 800 ohm
#define VIVO_HP_IMPED_120_OHM	100		// 100 mappimg to 120 ohm
/* this defines the times we can do another detect after we detected a headset
* insertion but its hp imped detected was more than 120 ohm.
*/
#define VIVO_HEADSET_DET_THRHLD_IMPED	1
#define VIVO_HEADSET_DET_THRHLD	4
#define VIVO_SWAP_DET_THRESHLD 1
#define	VIVO_HIGH_HPG_THRHLD	2	// 220ms each time, 330m including imped detect
...
static unsigned long dual_irq_cnt = 0;
...
// add this function to fix hp impedance @leichenji
void vivo_imped_fix(struct wcd_mbhc *mbhc, uint32_t *impl, uint32_t *impr)
{
	if ((*impr > VIVO_HP_IMPED_THRHLD) || (*impl > VIVO_HP_IMPED_THRHLD)) {
		if (((*impl) * 3) < (*impr) || ((*impr) * 3) < (*impl)) {
			pr_info("%s: zl %d or zr %d may be much larger than 100ohm, fixed them impl value as a HIGH_HPH.\n",
					__func__, *impl, *impr);
			*impr = *impl;
		} else {
			pr_info("%s: zl %d or zr %d \n", __func__, *impl, *impr);
			*impl = *impr = (*impl + *impr)/2;
		}
	} else {
		if (((*impl) * 3) < (*impr) || ((*impr) * 3) < (*impl)) {
			pr_info("%s: zl %d or zr %d the smaller imp may be deteted abnormal, fixed them the larger imp.\n",
					__func__, *impl, *impr);
			*impl = *impr = (*impl > *impr) ? *impl : *impr;
		} else {
			pr_info("%s: zl %d or zr %d \n", __func__, *impl, *impr);
			*impl = *impr = (*impl + *impr)/2;
		}
	}
	if (((*impr < 5) && (*impl < 5)) || (*impl > 1400)) {
		pr_info("%s:  zl %d, zr %d, pa is on, imp detect failed, fixed them old value.\n", __func__, *impl, *impr);
		*impl = 16;
		*impr = 16;
	}
	...
	pr_debug("%s wcd_mbhc_fixed zl %d, zr %d.\n", __func__, *impl, *impr);
}

static void wcd_mbhc_jack_report(struct wcd_mbhc *mbhc,
				struct snd_soc_jack *jack, int status, int mask)
{
	wake_lock_timeout(&mbhc_key_lock, 3 * HZ);
...
//out_micb_en:
		...
		// clear pull-down enable bit for micbias2 @leichenji
			...
			uint32_t *zr)
{
	...
	*zr = mbhc->zr;
	...
	if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->get_hp_imped_fixed_state &&
			mbhc->mbhc_cfg->get_hp_imped_fixed_state()) {
		pr_info("%s: hp imped fixed, set a certain impedance for setting gain.\n",
					__func__);
		*zl = *zr = 51;
	}
				...
				enum snd_jack_types jack_type)
{
	...
	u32 old_hph_status = mbhc->hph_status;
	struct snd_soc_codec *codec = mbhc->codec;
	...
	if (!insertion) {
		...
		if (old_jack_type == SND_JACK_HEADPHONE)
			...
			mbhc->current_plug = MBHC_PLUG_TYPE_HIGH_HPH;
		else if (old_jack_type == SND_JACK_ANC_HEADPHONE)
		...
		// if hp has been enabled, do not detect imped again @leichenji
		if (mbhc->impedance_detect && mbhc->mbhc_cb->compute_impedance
			&& !wcd_mbhc_is_hph_pa_on(mbhc) && mbhc->mbhc_cfg  && mbhc->mbhc_cfg->imped_detect_en
			&& ((mbhc->mbhc_cfg->get_hp_switch_state
			&& (mbhc->mbhc_cfg->get_hp_switch_state() == HP_PATH_NORMAL))
			|| !mbhc->mbhc_cfg->get_hp_switch_state)) {
			mbhc->mbhc_cb->compute_impedance(mbhc,
					&mbhc->zl, &mbhc->zr);
			pr_debug("%s: zl %dohm, zr %dohm\n", __func__,
					mbhc->zl, mbhc->zr);
			// set imped to default if imped is invalid @leichenji
		...
		pr_debug("%s: Reporting insertion %d(%x)\n", __func__,
			 jack_type, mbhc->hph_status);
		if (old_hph_status != mbhc->hph_status)
			wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
...
//we've already set micbias to 2.4v, so we do not need this.@leichenji
#if 0
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	bool micbias2 = false;
	bool micbias1 = false;
	int headset_cnt = 0, headset_swap_cnt = 0;
	bool is_normal_path = true;
	u16 btn_dbnc = 0;
	...
	codec = mbhc->codec;
	mbhc->in_swch_correct = true;
	WCD_MBHC_REG_READ(WCD_MBHC_BTN_DBNC, btn_dbnc);
	/*
	...
	usleep_range(240 * 1000, 240 * 1000);
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (mbhc->btn_press_intr) {
			...
			mbhc->btn_press_intr = 0;
		}

		mbhc->impedance_detect = true;

		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_BTN_DBNC, 0);
		 ...
		 */
		usleep_range(70*1000, 70*1000);

		// re-check stop requsted again @leichenji
		if (mbhc->hs_detect_work_stop) {
			pr_debug("%s: stop request revieved: %d\n", __func__,
					mbhc->hs_detect_work_stop);
			wcd_enable_curr_micbias(mbhc,
						WCD_MBHC_EN_NONE);
			goto exit;
		...
		if (mbhc->mbhc_cfg->imped_detect_en && mbhc->mbhc_cb->hph_pa_on_status)
			is_pa_on = mbhc->mbhc_cb->hph_pa_on_status(codec);

		if (mbhc->mbhc_cfg->get_hp_switch_state) {
			is_normal_path = (mbhc->mbhc_cfg->get_hp_switch_state() == HP_PATH_NORMAL);
			if (!mbhc->mbhc_cfg->imped_detect_en &&
				!is_normal_path) {
				if (plug_type == MBHC_PLUG_TYPE_INVALID)
					plug_type = mbhc->current_plug;
				if (plug_type == MBHC_PLUG_TYPE_HEADSET) {
					pr_info("%s:Not HP Normal Mode and detect hesdset, stop correct plug type.\n", __func__);
					goto report;
				}
			}
		}
		...
		*/
		...
		if (!mbhc->btn_press_intr) {
			...
			plug_type = MBHC_PLUG_TYPE_HEADSET;
			if (mbhc->impedance_detect && mbhc->mbhc_cb->compute_impedance
						&& !is_pa_on && is_normal_path && mbhc->mbhc_cfg->imped_detect_en) {
				WCD_MBHC_RSC_LOCK(mbhc);
				mbhc->mbhc_cb->compute_impedance(mbhc,
					&mbhc->zl, &mbhc->zr);
				WCD_MBHC_RSC_UNLOCK(mbhc);
			...
			// if headset detected, exit the correct work @leichenji
			if (mbhc->current_plug != MBHC_PLUG_TYPE_HEADSET) {
				if (!mbhc->mbhc_cfg->imped_detect_en) {
					if(((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) && (headset_cnt > VIVO_HEADSET_DET_THRHLD))
						|| (mbhc->current_plug != MBHC_PLUG_TYPE_NONE) || is_pa_on || !is_normal_path) {
						wrk_complete = false;
						goto report;
					}
				} else {
					// give more chance if detected hp imped is more than 120 ohm
					if ((!is_pa_on && is_normal_path && ((mbhc->zl < VIVO_HP_IMPED_120_OHM &&
						mbhc->zr < VIVO_HP_IMPED_120_OHM) || (headset_cnt > VIVO_HEADSET_DET_THRHLD_IMPED)))
						|| is_pa_on || !is_normal_path) {
						wrk_complete = false;
			...
			headset_cnt = 0;
			// if OMTP headset detected, report headphone and exit @leichenji
			if (!(mbhc->btn_press_intr & SND_JACK_BTN_0)) {
				pr_info("%s cable maybe OMTP Headset.\n", __func__);
				headset_swap_cnt++;
				// for phones which can do hp imped detection
				if (mbhc->mbhc_cfg->imped_detect_en) {
					plug_type = MBHC_PLUG_TYPE_HEADPHONE;
					if (mbhc->current_plug == MBHC_PLUG_TYPE_NONE) {
						WCD_MBHC_RSC_LOCK(mbhc);
						// not do impedance detection here;
				...
				/* for phones which can not do hp imped detection, add for hifi-always-on @zgbing */
				if (!mbhc->mbhc_cfg->imped_detect_en) {
					plug_type = MBHC_PLUG_TYPE_HEADPHONE;
					if (mbhc->current_plug != MBHC_PLUG_TYPE_HEADPHONE) {
						WCD_MBHC_RSC_LOCK(mbhc);
			...
			} else if (mbhc->btn_press_intr & SND_JACK_BTN_0) {
				plug_type = MBHC_PLUG_TYPE_HEADPHONE;
				pr_info("%s Plug Headphone Detected.\n", __func__);
				if (mbhc->current_plug != MBHC_PLUG_TYPE_HEADPHONE) {
					WCD_MBHC_RSC_LOCK(mbhc);
					wcd_mbhc_find_plug_and_report(mbhc, plug_type);
					WCD_MBHC_RSC_UNLOCK(mbhc);
				} else {
					// if pa is not on, we have chance to correct imped @leichenji
					if ((headset_swap_cnt > 0) && mbhc->impedance_detect && mbhc->mbhc_cb->compute_impedance
						&& !is_pa_on && is_normal_path && mbhc->mbhc_cfg->imped_detect_en) {
						WCD_MBHC_RSC_LOCK(mbhc);
						mbhc->mbhc_cb->compute_impedance(mbhc,
							&mbhc->zl, &mbhc->zr);
...
report:
	mbhc->impedance_detect = true;
	/* Close switch, added by ChenJinQuan. */
	if (mbhc->mbhc_cfg->set_hp_switch_state &&
		mbhc->mbhc_cfg->get_hp_switch_state &&
		!wrk_complete && (mbhc->current_plug == MBHC_PLUG_TYPE_NONE))
	...
	if (wrk_complete && (plug_type == MBHC_PLUG_TYPE_HEADPHONE)) {
		if (mbhc->mbhc_cfg->swap_gnd_mic)
			mbhc->mbhc_cfg->swap_gnd_mic(codec, 1);
	}
	...
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_BTN_DBNC, btn_dbnc);
	pr_debug("%s: restore btn dbnc %d before report\n",
		__func__, btn_dbnc);
...
exit:
	...
	if (0 && mbhc->mbhc_cfg->detect_extn_cable &&
	    ((plug_type == MBHC_PLUG_TYPE_HEADPHONE) ||
	     (plug_type == MBHC_PLUG_TYPE_HEADSET)) &&
	    !mbhc->hs_detect_work_stop) {
		WCD_MBHC_RSC_LOCK(mbhc);
	...
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_BTN_DBNC, btn_dbnc);
	pr_debug("%s: restore btn dbnc %d\n", __func__, btn_dbnc);

	mbhc->mbhc_cb->lock_sleep(mbhc, false);
	mbhc->in_swch_correct = false;
...
static void wcd_mbhc_detect_plug_type(struct wcd_mbhc *mbhc)
{
	...
	/* can't run below codes if hifi-always-on @zgbing */
	if (0) {
		do {
...
static void wcd_mbhc_swch_gpio_irq_handler(struct wcd_mbhc *mbhc)
{
	bool micbias1 = false;
	struct snd_soc_codec *codec = mbhc->codec;
	...
	// In case of dual-irq issue, the second irq has already been ignored in irq handler,
	// and here if the first irq is insertion irq, also ignore it to prevent from stepping into
	// headset mode, thus even if the dual-irq issue happened, no voice issue should not be met.
	if (mbhc->ignore_mech_irq && mbhc->swch_detection_type) {
		pr_warn("%s: because of double-irq issue, ignore this insertion irq.\n", __func__);
		mbhc->in_swch_irq_handler = false;
		mbhc->ignore_mech_irq = false;
		pr_debug("%s: exit\n", __func__);
		return;
	}
	...
	if (((mbhc->swch_detection_type && mbhc->gpio_detection_type) &&
		(mbhc->current_plug != MBHC_PLUG_TYPE_NONE)) || !mbhc->mbhc_cfg)
		goto jump_to_end;
	...
	if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->swap_gnd_mic &&
		mbhc->mbhc_cfg->swap_gnd_mic(codec, 0)) {
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	if (mbhc->ignore_mech_irq && detection_type) {
		pr_warn("%s: because of double-irq issue, ignore this irq.\n", __func__);
		mbhc->in_swch_irq_handler = false;
		mbhc->ignore_mech_irq = false;
		pr_debug("%s: exit\n", __func__);
		return;
	...
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_MECH_DETECTION_TYPE,
				 !detection_type);
	pr_debug("%s: mbhc->current_plug: %d detection_type: %d, dual_irq_cnt %lu\n", __func__,
			mbhc->current_plug, detection_type, dual_irq_cnt);
...
static irqreturn_t wcd_mbhc_mech_plug_detect_irq(int irq, void *data)
{
	...
	if (unlikely((mbhc->mbhc_cb->lock_sleep(mbhc, true)) == false)) {
		...
		pr_err("%s: double-irq issue occurs, ignore second irq"
				"and schedule dwork to get recovery.\n", __func__);
		WCD_MBHC_RSC_LOCK(mbhc);
		mbhc->ignore_mech_irq = true;
		WCD_MBHC_RSC_UNLOCK(mbhc);
		dual_irq_cnt++;
	} else {
		/* Call handler */
...
static irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	mbhc->btn_press_intr = 1;
	mbhc->is_btn_press = true;
	msec_val = jiffies_to_msecs(jiffies - mbhc->jiffies_atreport);
	pr_debug("%s: msec_val = %ld, dual_irq_cnt %lu\n", __func__, msec_val, dual_irq_cnt);
	...
	/* HIGH_HPH type may have button too, changed by chenjinquan */
	if ((mbhc->current_plug != MBHC_PLUG_TYPE_HEADSET &&
		mbhc->current_plug != MBHC_PLUG_TYPE_HIGH_HPH)
		|| mbhc->in_swch_correct) {
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	if (mbhc->in_swch_correct) {
		pr_info("%s: in correct detection, ignore this release.\n", __func__);
		goto exit;
	}
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	/* change value from 2 to 0 to turn MIC_CLAMP off @zgbing */
	...
	if (mbhc->mbhc_cfg->enable_usbc_analog) {
		/* Insertion debounce set to 64ms */ //change from 256ms to 64ms @leichenji
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_INSREM_DBNC, 5);
	} else {
	...
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_BTN_DBNC, 3);//shijianxing:Modify Debounce time
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case MBHC_AT_IOCTL_MOS_TEST:
		...
		at_mbhc->mbhc_cfg->swap_gnd_mic(at_mbhc->codec, 1);
		usleep_range(MBHC_AT_PULLDWN_US, MBHC_AT_PULLDWN_US);
		at_mbhc->mbhc_cfg->swap_gnd_mic(at_mbhc->codec, 0);
		      ...
		      bool impedance_det_en)
{
	...
	mbhc->headset_gpio = of_get_named_gpio(codec->dev->of_node,
