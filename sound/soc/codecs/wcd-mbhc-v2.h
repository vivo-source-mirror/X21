...
struct wcd_mbhc_config {
	...
	bool (*swap_gnd_mic) (struct snd_soc_codec *codec, int value);
	bool hs_ext_micbias;
	bool gnd_det_en;
	...
	/* Mic switch control for mbhc, changed by ChenJinQuan. */
	int (*get_hp_switch_state)(void);
	int (*set_hp_switch_state)(int);
	int (*get_mic_switch_state)(void);
	int (*set_mic_switch_state)(int);
	/* get hp imped fixed state for mbhc @zgbing */
	int (*get_hp_imped_fixed_state)(void);
...
struct wcd_mbhc {
	...
	bool ignore_mech_irq;
	bool in_gpio_irq_handler;
};
