...
static uint32_t hp_gain_imped_ref = 38;
...
exit:
	...
	/* vivo add this to avoid pop noise */
	if (snd_soc_read(codec, MSM89XX_PMIC_ANALOG_MICB_1_EN) & 0x80) {
		/* enable pull up */
		snd_soc_update_bits(codec, MSM89XX_PMIC_ANALOG_MICB_2_EN, 0x60, 0x40);
		/* update reg3 value */
		msleep(1);
		snd_soc_write(codec, MSM89XX_PMIC_ANALOG_MICB_2_EN, reg3 | 0x40);
		msleep(1);
	}
			     ...
			     uint32_t imped, bool set_gain)
{
	...
	if (set_gain) {
		...
		case DIANGU:
		case DRAX_CDC:
			if (value >= hp_gain_imped_ref) {/* ori:13 */
				if (codec_version < DIANGU)
					snd_soc_update_bits(codec,
						MSM89XX_PMIC_ANALOG_RX_EAR_CTL,
						0x20, 0x20);
				else
					    ...
					    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (get_codec_version(sdm660_cdc) >= DIANGU) {
			snd_soc_update_bits(codec, MSM89XX_PMIC_ANALOG_RX_COM_BIAS_DAC,
			    0x06, pm660_ear_pa_gain);
		} else {
			snd_soc_update_bits(codec, MSM89XX_PMIC_ANALOG_RX_EAR_CTL,
			    0x20, pm660_ear_pa_gain);
		}
		snd_soc_update_bits(codec, MSM89XX_PMIC_ANALOG_RX_EAR_CTL,
			    0x80, 0x80);
		if (get_codec_version(sdm660_cdc) < CONGA)
...
static const struct snd_soc_dapm_widget msm_anlg_cdc_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("RCV_REGULATOR", SND_SOC_NOPM,
		ON_DEMAND_RCV, 0,
...
static const struct sdm660_cdc_reg_mask_val
	 ...
	 */
	{MSM89XX_PMIC_ANALOG_RX_COM_OCP_CTL, 0xFF, 0x1F}, /* OCP 280mv and 15 retry times */
...
static int msm_anlg_cdc_soc_probe(struct snd_soc_codec *codec)
{
	...
	msm_anlg_cdc_update_micbias_regulator(
				sdm660_cdc,
				on_demand_supply_name[ON_DEMAND_RCV],
				&sdm660_cdc->on_demand_list[ON_DEMAND_RCV]);
	atomic_set(&sdm660_cdc->on_demand_list[ON_DEMAND_RCV].ref,
		   0);
...
static int msm_anlg_cdc_soc_remove(struct snd_soc_codec *codec)
{
	...
	atomic_set(&sdm660_cdc_priv->on_demand_list[ON_DEMAND_MICBIAS].ref,
		   0);
	sdm660_cdc_priv->on_demand_list[ON_DEMAND_RCV].supply = NULL;
	atomic_set(&sdm660_cdc_priv->on_demand_list[ON_DEMAND_RCV].ref,
		   0);
...
static void wcd_hp_gain_imped_ref(struct device *dev)
{
	if (of_find_property(dev->of_node, "vivo,hp-gain-imped-ref", NULL)) {
		of_property_read_u32(dev->of_node, "vivo,hp-gain-imped-ref",
				&hp_gain_imped_ref);
		pr_debug("%s: hp gain impid ref %d", __func__, hp_gain_imped_ref);
	}
}
...
static int msm_anlg_cdc_probe(struct platform_device *pdev)
{
	...
	/* get the imped ref so that it can set hp gain according diff imped @zgbing */
	wcd_hp_gain_imped_ref(&pdev->dev);
