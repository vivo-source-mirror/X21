...
#define BTN_NUM				5
#define BTN_DEFS			(2 * BTN_NUM)
static u16 mbhc_btn_defs[BTN_DEFS] = {0};
...
#undef S
	...
	//set btn threshold
	for (i = 0; i < BTN_NUM; i++) {
		btn_low[i] = mbhc_btn_defs[i];
		btn_high[i] = mbhc_btn_defs[i + BTN_NUM];
	}

	pr_debug("%s btn_low %d, %d, %d, %d, %d.\n", __func__, btn_low[0], btn_low[1],
		btn_low[2], btn_low[3], btn_low[4]);
	pr_debug("%s btn_high %d, %d, %d, %d, %d.\n", __func__, btn_high[0], btn_high[1],
		btn_high[2], btn_high[3], btn_high[4]);
#endif
...
struct snd_soc_dai_link_component hifi_maximspk_rx[] = {
	{
		.name = "hifi-codec",
		//.of_node = NULL,
		.dai_name = "vivo-hifi-rx",
	},
	{
		.name = "max98927",
		//.of_node = NULL,
...
//Add for maxim dsm
static struct snd_soc_dai_link mi2s_sec_tx_hostless_dai[] = {
	{/* hw:x,40 */
...
//Add for maxim dsm
static struct snd_soc_dai_link mi2s_tert_tx_hostless_dai[] = {
	{/* hw:x,40 */
...
static struct snd_soc_dai_link msm_mi2s_be_dai_links[] = {
	...
	},
	{
		...
		.platform_name = "msm-pcm-routing",
		.codec_name = "hifi-codec",
		.codec_dai_name = "vivo-hifi-rx",
		.no_pcm = 1,
		.dpcm_playback = 1,
...
static struct msm_soc_dynamic_dai_links sdm660_dynamic_dai_links[] = {
	/* Mi2s Backend DAI Links */
	{
		.id = "qcom,mi2s-tert-hifi-rx",
		.dai_link = {
			.name = LPASS_BE_TERT_MI2S_RX,
			.stream_name = "Tertiary MI2S Playback",
			.cpu_dai_name = "msm-dai-q6-mi2s.2",
	...
	},
	{
		...
		.dai_link = {
			...
			.be_id = MSM_BACKEND_DAI_SECONDARY_MI2S_TX,
			.be_hw_params_fixup = msm_common_be_hw_params_fixup,
			.init = vivo_maxim_tx_init,
			.ops = &msm_mi2s_be_ops,
			.ignore_suspend = 1,
		}
	},
	{
		.id = "qcom,mi2s-tert-hifi-maximspk-rx",
		.dai_link = {
			.name = LPASS_BE_TERT_MI2S_RX,
			.stream_name = "Tertiary MI2S Playback",
			.cpu_dai_name = "msm-dai-q6-mi2s.2",
			.platform_name = "msm-pcm-routing",
			.codecs = hifi_maximspk_rx,
			.num_codecs = ARRAY_SIZE(hifi_maximspk_rx),
			.no_pcm = 1,
			.dpcm_playback = 1,
			.be_id = MSM_BACKEND_DAI_TERTIARY_MI2S_RX,
			.be_hw_params_fixup = msm_common_be_hw_params_fixup,
			.init = vivo_hifi_maximspk_rx_init,
			.ops = &msm_mi2s_be_ops,
			.ignore_suspend = 1,
			.ignore_pmdown_time = 1,
		}
	...
	{
		.id = "qcom,mi2s-tert-maxim-spk-tx",
		.dai_link = {
			.name = LPASS_BE_TERT_MI2S_TX,
			.stream_name = "Tertiary MI2S Capture",
			.cpu_dai_name = "msm-dai-q6-mi2s.2",
...
ARRAY_SIZE(mi2s_sec_tx_hostless_dai) +
ARRAY_SIZE(mi2s_tert_tx_hostless_dai) +
ARRAY_SIZE(sdm660_dynamic_dai_links)];
						...
						struct device *dev)
{
	...
	if (of_property_read_bool(dev->of_node,
				  "qcom,mi2s_sec_tx_hostless_dai")) {
		memcpy(dailink + len1,
		       mi2s_sec_tx_hostless_dai,
		       sizeof(mi2s_sec_tx_hostless_dai));
		len1 += ARRAY_SIZE(mi2s_sec_tx_hostless_dai);
	}
	if (of_property_read_bool(dev->of_node,
				  "qcom,mi2s_tert_tx_hostless_dai")) {
		memcpy(dailink + len1,
		       mi2s_tert_tx_hostless_dai,
		       sizeof(mi2s_tert_tx_hostless_dai));
		len1 += ARRAY_SIZE(mi2s_tert_tx_hostless_dai);
	}
	...
	num_dy_dailinks = ARRAY_SIZE(sdm660_dynamic_dai_links);
	...
	for (i = 0; i < num_dailinks; i++) {
		...
		for (found = 0,j = 0; j < num_dy_dailinks; j++) {
			if (!strcmp(sdm660_dynamic_dai_links[j].id, name)) {
				memcpy(dailink + len1,
				       &sdm660_dynamic_dai_links[j].dai_link,
...
{
	u32 btn_defs[BTN_DEFS];
	int i = 0;

	if (of_find_property(np, "vivo,mbhc-btn-defs", NULL)) {
		of_property_read_u32_array(np, "vivo,mbhc-btn-defs", btn_defs, BTN_DEFS);
		for (i = 0; i < BTN_DEFS; i++) {
			mbhc_btn_defs[i] = 0xffff & btn_defs[i];
		}
		pr_info("%s btn_low %d, %d, %d, %d, %d.\n", __func__, mbhc_btn_defs[0], mbhc_btn_defs[1],
			mbhc_btn_defs[2], mbhc_btn_defs[3], mbhc_btn_defs[4]);
		pr_info("%s btn_high %d, %d, %d, %d, %d.\n", __func__, mbhc_btn_defs[5], mbhc_btn_defs[6],
			mbhc_btn_defs[7], mbhc_btn_defs[8], mbhc_btn_defs[9]);
	}
	return;
}
