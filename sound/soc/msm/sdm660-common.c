...
#include "../vivo/max98927.h"
#include <linux/qdsp6v2/apr.h>

#ifdef BBK_FEATURE_TFA98XX
#if BBK_FEATURE_TFA98XX == 9894
#include "../vivo/tfa9894/tfa98xx.h"
#define TFA_VERSION 9894
#endif
#else
#include "../vivo/config.h"
#define TFA_VERSION 9891
#endif
...
#endif
#include <sound/vivo-audio-test.h>
...
static u32 mi2s_bits_trans(u32 bit_format)
{
	u32 bit_per_sample;
...
static int vivo_clk_mi2s_id;//declaration

static uint32_t rates[] = {384000, 192000, 96000, 48000, 24000, 16000, 8000, 352800, 176400, 88200, 44100};
static uint32_t bits[] = {16, 24};
static uint32_t channels[] = {8, 7, 6, 5, 4, 3, 2, 1};

int search_bit(uint32_t arrary[], int size, int val)
{
	int i = 0;
	for (i = 0; i < size && arrary[i] != val; i++)
		;
	return (i < size)? i : 0;
}

int convert2num(uint32_t rate, uint32_t bit, uint32_t channel)
{
	return search_bit(channels, ARRAY_SIZE(channels), channel) +
	       (search_bit(bits, ARRAY_SIZE(bits), bit) << 4) +
	       (search_bit(rates, ARRAY_SIZE(rates), rate) << 6) + 1;
}

int smart_pa_clk_check(void)
{
...
exit:
	if (ret) {
		int val = convert2num(cfg.sample_rate, cfg.bit_width, cfg.channels);
		pr_err("error code -%d\n", val);
	} else {
		pr_info("%s normal\n", __func__);
	}
...
static bool msm_swap_gnd_mic(struct snd_soc_codec *codec, int value);
...
static struct dev_config mi2s_tx_cfg[] = {
	...
	[SEC_MI2S]  = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 1},
...
static char const *mi2s_rate_text[] = {"KHZ_44P1", "KHZ_48",
...
static char const *mi2s_mode_text[] = {"Master-Fixed", "Master", "Slave", "Undefine"};
...
static SOC_ENUM_SINGLE_EXT_DECL(prim_mi2s_sample_rate, mi2s_rate_text);
static SOC_ENUM_SINGLE_EXT_DECL(sec_mi2s_sample_rate, mi2s_rate_text);
static SOC_ENUM_SINGLE_EXT_DECL(tert_mi2s_sample_rate, mi2s_rate_text);
static SOC_ENUM_SINGLE_EXT_DECL(quat_mi2s_sample_rate, mi2s_rate_text);
static SOC_ENUM_SINGLE_EXT_DECL(mi2s_bit_format, mi2s_format_text);
...
static int mi2s_sample_rate_get(struct snd_kcontrol *kcontrol,
                                   struct snd_ctl_elem_value *ucontrol)
{
	int idx = mi2s_get_port_idx(kcontrol);
...
static int msm_mi2s_bit_format_get(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
	int idx = mi2s_get_port_idx(kcontrol);
	...
	pr_info("%s: idx[%d]_bit_format = %d, item = %d\n", __func__,
...
const struct snd_kcontrol_new msm_common_snd_controls[] = {
	...
	SOC_ENUM_EXT("QUAT_AUX_PCM_TX SampleRate", quat_aux_pcm_tx_sample_rate,
			aux_pcm_tx_sample_rate_get,
			aux_pcm_tx_sample_rate_put),
	SOC_ENUM_EXT("PRIM_MI2S SampleRate", prim_mi2s_sample_rate,
				mi2s_sample_rate_get,
				NULL),
	SOC_ENUM_EXT("SEC_MI2S SampleRate", sec_mi2s_sample_rate,
				mi2s_sample_rate_get,
				NULL),
	SOC_ENUM_EXT("TERT_MI2S SampleRate", tert_mi2s_sample_rate,
				mi2s_sample_rate_get,
				NULL),
	SOC_ENUM_EXT("QUAT_MI2S SampleRate", quat_mi2s_sample_rate,
				mi2s_sample_rate_get,
				NULL),
	SOC_ENUM_EXT("PRIM_MI2S Format", mi2s_bit_format,
				msm_mi2s_bit_format_get,
				NULL),
	SOC_ENUM_EXT("SEC_MI2S Format", mi2s_bit_format,
				msm_mi2s_bit_format_get,
				NULL),
	SOC_ENUM_EXT("TERT_MI2S Format", mi2s_bit_format,
				msm_mi2s_bit_format_get,
				NULL),
	SOC_ENUM_EXT("QUAT_MI2S Format", mi2s_bit_format,
				msm_mi2s_bit_format_get,
				NULL),
	...
	SOC_ENUM_EXT("QUAT_TDM_TX_0 Channels", tdm_tx_chs,
			tdm_tx_ch_get,
			tdm_tx_ch_put),
	/* just for test the mi2s slave mode. */
				  ...
				  struct snd_pcm_hw_params *params)
{
	...
	unsigned int dai_fmt = SND_SOC_DAIFMT_CBS_CFS;
	int rc = 0;
	int idx;
	unsigned int mi2s_rate = 0;

	pr_debug("%s: format = %d, rate = %d, be_id %d\n",
	...
	case MSM_BACKEND_DAI_QUATERNARY_MI2S_TX:
		...
		if (!(test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
			...
			rc = snd_soc_runtime_set_dai_fmt(rtd, dai_fmt);
			if (IS_ERR_VALUE(rc)) {
...
void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (!(test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
	      test_bit(MI2S_TX_STARTED, mi2s_intf_conf[index].status_mask) ||
	      test_bit(MI2S_CAL_STARTED, mi2s_intf_conf[index].status_mask))) {
		// for lpi gpio, pinctrl state need to set only when adsp is not sleep.
		if (index == TERT_MI2S) {
		...
		//backup clock config
		mi2s_config.enable = 0;
		mi2s_config.port_id = -EINVAL;
	} else {
...
/**
 * msm_mi2s_snd_prepare - prepare ops of mi2s.
 *
 * @substream: PCM stream pointer of associated backend dailink
 *
 * Returns 0 on success or -EINVAL on error.
 */
int msm_mi2s_snd_prepare(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
	...
	if (!(test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
		...
		//backup clock config
		mi2s_config.enable = 1;
		mi2s_config.port_id = port_id;
		mi2s_config.sample_rate = mi2s_rx_cfg[index].sample_rate;
		mi2s_config.bit_width = mi2s_bits_trans(mi2s_rx_cfg[index].bit_format);
		mi2s_config.channels = mi2s_rx_cfg[index].channels;
...
EXPORT_SYMBOL(msm_mi2s_snd_prepare);

/* extern int is_atboot; undefined @zgbing */
static int is_atboot;
struct msm_asoc_mach_data *vivo_pdata = NULL;
static struct tfa98xx_calib *tfa98xx_calib_ptr = NULL;
...
static int msm_mi2s_enable(bool enable)
{
	...
	port_id = msm_get_port_id(MSM_BACKEND_DAI_SECONDARY_MI2S_RX);
	index = (port_id & 0xf) >> 1;
...
static bool msm_swap_gnd_mic(struct snd_soc_codec *codec,  int value)
{
	...
	if (pdata->us_euro_gpio_p) {
		current_value = msm_cdc_pinctrl_get_state(pdata->us_euro_gpio_p);
		if (!value) {
			msm_cdc_pinctrl_select_sleep_state(
							pdata->us_euro_gpio_p);
			pr_debug("%s: msm_cdc_pinctrl_select_sleep_state \n", __func__);
		} else {
			msm_cdc_pinctrl_select_active_state(
							pdata->us_euro_gpio_p);
			pr_debug("%s: msm_cdc_pinctrl_select_active_state \n", __func__);
		}
	} else if (pdata->us_euro_gpio >= 0) {
		current_value = gpio_get_value_cansleep(pdata->us_euro_gpio);
		gpio_direction_output(pdata->us_euro_gpio, value);
	}
	pr_debug("%s: swap select switch %d to %d\n", __func__, current_value, value);
...
#if 0
static int msm_i2s_clk_check(int sample_rate)
{
	struct afe_clk_get clk_cfg;
	int port_id = msm_get_port_id(vivo_clk_mi2s_id);
	int ret = 0;

	if (!afe_get_lpass_clk_cfg(port_id, &clk_cfg)) {
		ret = (clk_cfg.enable != 0 && clk_cfg.sample_rate == sample_rate);
		pr_info("%s enable %d sample rate %d bit width %d \n", __func__,
		        clk_cfg.enable, clk_cfg.sample_rate, clk_cfg.bit_width);
	} else {
		pr_err("%s get error \n", __func__);
		ret = 0;
	}
...
static unsigned int msm_mbhc_get_impedance(void)
{
	unsigned int value = mbhc_cfg.hp_impedance ? *mbhc_cfg.hp_impedance : 32;

	pr_debug("%s: impedance %u ohm\n", __func__, value);

	return value;
}

static void vivo_imped_adjust_ref(const struct device_node *np)
{
	if (of_find_property(np, "vivo,mbhc-imped-adjust-ref", NULL)) {
		of_property_read_u32(np, "vivo,mbhc-imped-adjust-ref",
				&mbhc_cfg.imped_adjust_ref);
		 pr_debug("%s: imped adjust ref %d", __func__, mbhc_cfg.imped_adjust_ref);
	}
}

int vivo_hifi_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_codec *codec = rtd->codec;
	struct snd_soc_dapm_context *dapm = &codec->component.dapm;
	struct vivo_codec_function *fun = snd_soc_codec_get_drvdata(codec);
	...
	mbhc_cfg.get_hp_switch_state = fun->get_hp_switch_state;
	mbhc_cfg.get_hifi_dac_state = fun->get_hifi_dac_state;
	fun->mbhc_get_hp_impedance = msm_mbhc_get_impedance;
...
EXPORT_SYMBOL(vivo_hifi_init);

int vivo_maxim_rx_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_codec *codec = rtd->codec;
	struct snd_soc_dapm_context *dapm = &codec->component.dapm;
	struct max98927_priv *max98927 = snd_soc_codec_get_drvdata(codec);

	pr_debug("Smart pa init\n");

	max98927->mi2s_rx_port_id = msm_get_port_id(rtd->dai_link->be_id);

	snd_soc_dapm_ignore_suspend(dapm, "Speaker");

	return 0;
}
EXPORT_SYMBOL(vivo_maxim_rx_init);

int vivo_maxim_tx_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_codec *codec = rtd->codec;
	struct max98927_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	vivo_clk_mi2s_id = rtd->dai_link->be_id;

	pr_debug("%s \n", __func__);

	max98927->mi2s_tx_port_id = msm_get_port_id(rtd->dai_link->be_id);
	pr_info("%s mi2s_tx_port_id %d \n", __func__, max98927->mi2s_tx_port_id);

	return 0;
}
EXPORT_SYMBOL(vivo_maxim_tx_init);

int vivo_hifi_maximspk_rx_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_codec *hifi_rx = rtd->codec_dais[0]->codec;
	struct snd_soc_codec *spk_rx = rtd->codec_dais[1]->codec;
	struct snd_soc_dapm_context *dapm_hifi = &hifi_rx->component.dapm;
	struct vivo_codec_function *fun = snd_soc_codec_get_drvdata(hifi_rx);
	struct max98927_priv *max98927 = snd_soc_codec_get_drvdata(spk_rx);
	vivo_clk_mi2s_id = rtd->dai_link->be_id;

	pr_debug("Hifi and Smart pa init \n");

	if (!fun) {
		pr_err("vivo_codec_function is NULL\n");
	} else {
		mbhc_cfg.get_hp_switch_state = fun->get_hp_switch_state;
		mbhc_cfg.get_hifi_dac_state = fun->get_hifi_dac_state;
		fun->mbhc_get_hp_impedance = msm_mbhc_get_impedance;

		snd_soc_dapm_ignore_suspend(dapm_hifi, "HP");
		snd_soc_dapm_ignore_suspend(dapm_hifi, "HiFi HPOUT");
	}

	if (!max98927) {
		pr_err("Smart pa is NULL\n");
	} else {
		max98927->mi2s_rx_port_id = msm_get_port_id(rtd->dai_link->be_id);
	}
...
EXPORT_SYMBOL(vivo_hifi_maximspk_rx_init);

int vivo_tfa98xx_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_codec *codec = rtd->codec;
	struct snd_soc_dapm_context *dapm = &codec->component.dapm;
	struct tfa98xx *tfa = snd_soc_codec_get_drvdata(rtd->codec);
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;

	pr_debug("Smart pa init \n");
	tfa98xx_calib_ptr = tfa->calib;
	msm_i2s_dev = cpu_dai ? cpu_dai->dev : NULL;
	vivo_clk_mi2s_id = rtd->dai_link->be_id;

	if (tfa98xx_calib_ptr)
		tfa98xx_calib_ptr->mi2s_check = NULL;//msm_i2s_clk_check;

	snd_soc_dapm_ignore_suspend(dapm, "Speaker");

	return 0;
}
EXPORT_SYMBOL(vivo_tfa98xx_init);

#ifdef CONFIG_BBK_DRIVER_INFO
static void vivo_parse_snd_info(const struct device_node *np)
{
	unsigned int vivo_snd_info = 0, vivo_fm_pcm_info = 0;
	unsigned int mic_num, spa_driver_type, spa_num, spa_manufacturer,
	...
	if (of_find_property(np, "vivo,hifi-always-on", NULL)) {
		...
		mbhc_cfg.imped_detect_en = false;
	}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	vivo_pdata = pdata;
	if (tfa98xx_calib_ptr && (is_atboot == 0)) {
		ret = msm_mi2s_enable(true);
		if (ret) {
			dev_err(&pdev->dev, "Smart pa mi2s clock failed\n");
			tfa98xx_calib_ptr->mi2s_ready = 1;
		} else {
			dev_info(&pdev->dev, "Smart pa calibration start\n");
			tfa98xx_calib_ptr->mi2s_ready = 0;
			msleep(50);
			tfa98xx_calib_ptr->calibrate(false);
		}
		msm_mi2s_enable(false);
		dev_info(&pdev->dev, "Smart pa calibration done\n");
	}
#ifdef CONFIG_BBK_DRIVER_INFO
...
#endif
	/* we can adjust detected imped for diff projects @zgbing */
	vivo_imped_adjust_ref(pdev->dev.of_node);
