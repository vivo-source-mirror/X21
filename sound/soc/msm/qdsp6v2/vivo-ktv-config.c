...
void ktv_params_callback(uint32_t *payload, u32 payload_size)
{
	...
	length = (payload[1] - sizeof(struct adm_ktv_common_param))/sizeof(uint32_t);//payload[1]/4 - 2
	for (i = 0; i < length; i++) {
		pr_info("%s: payload[%d] = %d\n", __func__, i + 3, payload[i+3]);
	}
	if (payload_size > (3 * sizeof(uint32_t))) {
		int params_base = sizeof(struct adm_ktv_common_param)/sizeof(uint32_t);//2
...
int vivo_adm_send_params(u8 *params,u32 param_id, u16 param_size)
{
	int ret = 0, sz = 0 ;
	char *send_params;
	struct adm_param_data_v5 *param_ptr;

	sz = sizeof(struct adm_param_data_v5) + param_size;
	send_params = (char *)kzalloc(sz, GFP_KERNEL);
	if (!send_params) {
		pr_err("%s alloc for sned_params failed.\n", __func__);
		return -ENOMEM;
	}

	param_ptr = (struct adm_param_data_v5 *)send_params;
	param_ptr->module_id = CAPI_V2_MODULE_KTV;
	param_ptr->param_id = param_id;
	param_ptr->param_size = param_size;

	memcpy(send_params + sizeof(struct adm_param_data_v5), (char *)params, param_size);

	//send params
	ret = adm_send_params_v5(this_ktv.port_id, this_ktv.copp_idx, send_params, sz);
	kfree(send_params);
	send_params = NULL;
	return ret;
}
                              ...
                              int ktv_param_id)
{
	...
	rc = get_ktv_apr_err();
get_param_return:
	if (ktv_param_id == KTV_TYPE_STATUS) {
                                     ...
                                     struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, kparam, size_in_bytes / sizeof(int));
	kfree(kparam);
	kparam = NULL;
	return 0;
}
                                    ...
                                    struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: music_gain(0x%x), mix_rate(0x%x), mixed(0x%x),ears_back(0x%x)\n",
	        __func__, kparam->music_gain, kparam->mix_rate, kparam->mixed, kparam->vivo_ktv_ctl);
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)kparam, size_in_bytes / sizeof(int));
	kfree(kparam);
	kparam = NULL;
	return ret;
}
                                    ...
                                    struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&kparam, ucontrol->value.integer.value, num_params);
                                 ...
                                 struct snd_ctl_elem_value *ucontrol)
{
	...
	eq = (struct equalizer_params *) kzalloc(size_in_bytes, GFP_KERNEL);
	...
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_EQ_PARAMS,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)eq);
	} else {
	...
	pr_info("%s: enable %d, p0(%d), p1(%d), p2(%d), p3(%d), p4(%d), p5(%d), p6(%d), p7(%d), p8(%d), p9(%d)\n", __func__,
	        eq->enable, eq->params[0], eq->params[1], eq->params[2], eq->params[3], eq->params[4], eq->params[5], eq->params[6], eq->params[7], eq->params[8], eq->params[9]);
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)eq, size_in_bytes / sizeof(int));
	kfree(eq);
	eq = NULL;
	return ret;
}
                                 ...
                                 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params( (u8 *)&eq, PARAM_ID_KTV_EQ_PARAMS, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
                                     ...
                                     struct snd_ctl_elem_value *ucontrol)
{
	...
	reverb = (struct reverb_params_vivo *) kzalloc(size_in_bytes, GFP_KERNEL);
	if (!reverb) {
		pr_err("%s alloc memory failed.\n", __func__);
		return 0;
	}
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_REVERB_PARAMS,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)reverb);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		kfree(reverb);
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)reverb, num_params);
	for (i = 0; i < num_params; i++) {
		pr_info("%s: num[%d] = 0x%lx",__func__,i, ucontrol->value.integer.value[i]);
	}
	kfree(reverb);
	reverb = NULL;
	return ret;
                                     ...
                                     struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&reverb, ucontrol->value.integer.value, num_params);
	reverb.enable = !!reverb.enable;
	pr_info("%s: enable %d, roomsize(0x%x), damp(0x%x), wet(0x%x), dry(0x%x), width(0x%x), gain(0x%x)\n",
	        __func__, reverb.enable, reverb.roomsize, reverb.damp, reverb.wet,
	        reverb.dry, reverb.width, reverb.gain);

	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params((u8 *)&reverb, PARAM_ID_KTV_REVERB_PARAMS, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
                                   ...
                                   struct snd_ctl_elem_value *ucontrol)
{
	...
	int num_params = size_in_bytes / sizeof(int);
	echo = (struct iirecho_params *) kzalloc(size_in_bytes, GFP_KERNEL);
	if (!echo) {
		pr_err("%s alloc memory failed.\n", __func__);
		return 0;
	}
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_ECHO_PARAMS,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)echo);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		kfree(echo);
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)echo, num_params);
	for (i = 0; i < num_params; i++) {
		pr_info("%s: num[%d] = 0x%lx", __func__,i,ucontrol->value.integer.value[i]);
	}
	kfree(echo);
                                   ...
                                   struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&echo, ucontrol->value.integer.value, num_params);
	...
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params( (u8 *)&echo, PARAM_ID_KTV_ECHO_PARAMS, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
                                 ...
                                 struct snd_ctl_elem_value *ucontrol)
{
	...
	nc = (struct noise_cancel_params *) kzalloc(size_in_bytes, GFP_KERNEL);
	if (!nc) {
		pr_err("%s alloc memory failed.\n", __func__);
		return 0;
	}
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_NOISECANCEL_PARAMS,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)nc);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		kfree(nc);
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)nc, num_params);
	for (i = 0; i < num_params; i++) {
		pr_info("%s: num[%d] = %ld\n", __func__, i, ucontrol->value.integer.value[i]);
	}
	kfree(nc);
	nc = NULL;
	return ret;
                                 ...
                                 struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&nc, ucontrol->value.integer.value, num_params);
	nc.enable = !!nc.enable;
	pr_info("%s: enable %d\n", __func__, nc.enable);

	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params( (u8 *)&nc, PARAM_ID_KTV_NOISECANCEL_PARAMS, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	gramophone = (struct gramophone_params *) kzalloc(size_in_bytes, GFP_KERNEL);
	if (!gramophone) {
		pr_err("%s alloc memory failed.\n", __func__);
		return 0;
	}
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_GRAMOPHONE_PARAMS,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)gramophone);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		kfree(gramophone);
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)gramophone, num_params);
	for (i = 0; i < num_params; i++) {
		pr_info("%s: num[%d] = %ld\n", __func__, i, ucontrol->value.integer.value[i]);
	}
	kfree(gramophone);
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&gramophone, ucontrol->value.integer.value, num_params);
	gramophone.enable = !!gramophone.enable;
	pr_info("%s: enable %d\n", __func__, gramophone.enable);

	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params( (u8 *)&gramophone, PARAM_ID_KTV_GRAMOPHONE_PARAMS, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	voice = (struct voice_params *) kzalloc(size_in_bytes, GFP_KERNEL);
	if (!voice) {
		pr_err("%s alloc memory failed.\n", __func__);
		return 0;
	}
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_VOICE_GAIN,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)voice);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		kfree(voice);
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)voice, num_params);
	for (i = 0; i < num_params; i++) {
		pr_info("%s: num[%d] = %ld\n", __func__, i, ucontrol->value.integer.value[i]);
	}
	kfree(voice);
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&voice, ucontrol->value.integer.value, num_params);
	pr_info("%s: voice_gain %d\n", __func__, voice.voice_gain);

	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params( (u8 *)&voice, PARAM_ID_KTV_VOICE_GAIN, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
                                   ...
                                   struct snd_ctl_elem_value *ucontrol)
{
	...
	int num_params = size_in_bytes / sizeof(int);
	comp = (struct compressor_params *) kzalloc(size_in_bytes, GFP_KERNEL);
	if (!comp) {
		pr_err("%s alloc memory failed.\n", __func__);
		return 0;
	}
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_COMPRESSOR_PARAMS,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)comp);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		kfree(comp);
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)comp, num_params);
	for (i = 0; i < num_params; i++) {
		pr_info("%s: num[%d] = %ld\n",__func__, i, ucontrol->value.integer.value[i]);
	}
	kfree(comp);
                                   ...
                                   struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&comp, ucontrol->value.integer.value, num_params);
	comp.enable = !!comp.enable;
	pr_info("%s: enable %d\n", __func__, comp.enable);

	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params( (u8 *)&comp, PARAM_ID_KTV_COMPRESSOR_PARAMS, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
                                 ...
                                 struct snd_ctl_elem_value *ucontrol)
{
	...
	int num_params = size_in_bytes / sizeof(int);
	hc = (struct howling_cancel_params *) kzalloc(size_in_bytes, GFP_KERNEL);
	if (!hc) {
		pr_err("%s alloc memory failed.\n", __func__);
		return 0;
	}
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_HC_PARAMS,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)hc);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		kfree(hc);
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)hc, num_params);
	for (i = 0; i < num_params; i++) {
		pr_info("%s: num[%d] = %ld\n", __func__, i, ucontrol->value.integer.value[i]);
	}
	kfree(hc);
                                 ...
                                 struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&hc, ucontrol->value.integer.value, num_params);
	hc.enable = !!hc.enable;
	pr_info("%s: enable %d\n", __func__, hc.enable);

	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params( (u8 *)&hc, PARAM_ID_KTV_HC_PARAMS, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
                                      ...
                                      struct snd_ctl_elem_value *ucontrol)
{
	...
	int num_params = size_in_bytes / sizeof(int);
	kf = (struct ktv_func_enable *) kzalloc(size_in_bytes, GFP_KERNEL);
	if (!kf) {
		pr_err("%s alloc memory failed.\n", __func__);
		return 0;
	}
	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = adm_get_params(this_ktv.port_id, this_ktv.copp_idx, CAPI_V2_MODULE_KTV, PARAM_ID_KTV_ENABLE,
		                     size_in_bytes + sizeof(struct adm_param_data_v5), (char *)kf);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		kfree(kf);
	...
	copy_params_to_ucontrol(ucontrol->value.integer.value, (int *)kf, num_params);
	for (i = 0; i < num_params; i++) {
		pr_info("%s: num[%d] = %ld\n",__func__,i, ucontrol->value.integer.value[i]);
	}
	kfree(kf);
                                      ...
                                      struct snd_ctl_elem_value *ucontrol)
{
	...
	copy_params_from_ucontrol((int *)&kf, ucontrol->value.integer.value, num_params);
	kf.enable = !!kf.enable;
	pr_info("%s: enable %d\n", __func__, kf.enable);

	if (get_vivo_ktv_topo_state() == AUDIO_TX_KTV_TOPOLOGY) {
		set_vivo_ktv_cmd_state(1);
		ret = vivo_adm_send_params( (u8 *)&kf, PARAM_ID_KTV_ENABLE, size_in_bytes);
	} else {
		pr_err("%s() KTV Not Running ,Topology is 0x%x.\n", __func__,
		       this_ktv.topology);
		ret = -EPERM;
...
static const char *const vivo_audio_ktv_enable_text[] = {"off", "on"};
...
static const struct snd_kcontrol_new vivo_audio_ktv_controls[] = {
	SOC_ENUM_EXT("VIVO AUDIO KTV ENABLE", vivo_audio_ktv_enum[0],
	             vivo_audio_ktv_enable_get, vivo_audio_ktv_enable_put),
	SOC_SINGLE_EXT("KTV INFO", SND_SOC_NOPM, 0, 1,
	               0, vivo_audio_ktv_info_get, vivo_audio_ktv_info_put),

	SOC_SINGLE_MULTI_EXT("KTV EQ PARAMS", SND_SOC_NOPM, 0, 15,
	                     0, 11, vivo_ktv_eq_param_get, vivo_ktv_eq_param_put),
	SOC_SINGLE_MULTI_EXT("KTV Reverb PARAMS", SND_SOC_NOPM, 0, 0x2000,
	                     0, 7, vivo_ktv_reverb_param_get, vivo_ktv_reverb_param_put),
	SOC_SINGLE_MULTI_EXT("KTV ECHO PARAMS", SND_SOC_NOPM, 0, 0x2000,
	                     0, 5, vivo_ktv_echo_param_get, vivo_ktv_echo_param_put),
	SOC_SINGLE_EXT("KTV Noise Cancel PARAMS", SND_SOC_NOPM, 0, 1,
	               0, vivo_ktv_nc_param_get, vivo_ktv_nc_param_put),
	SOC_SINGLE_EXT("KTV Gramophone PARAMS", SND_SOC_NOPM, 0, 1,
	               0, vivo_ktv_gramophone_param_get, vivo_ktv_gramophone_param_put),
	SOC_SINGLE_EXT("KTV VOICE GAIN", SND_SOC_NOPM, 0, 0x4000,
	               0, vivo_audio_ktv_voice_gain_get, vivo_audio_ktv_voice_gain_put),
	SOC_SINGLE_EXT("KTV Compressor PARAMS", SND_SOC_NOPM, 0, 1,
	               0, vivo_ktv_comp_param_get, vivo_ktv_comp_param_put),
	SOC_SINGLE_EXT("KTV Howling Cancel PARAMS", SND_SOC_NOPM, 0, 1,
	               0, vivo_ktv_hc_param_get, vivo_ktv_hc_param_put),
	SOC_SINGLE_EXT("KTV Module Enable", SND_SOC_NOPM, 0, 1,
	               0, vivo_ktv_module_enable_get, vivo_ktv_module_enable_put),
	SOC_SINGLE_MULTI_EXT("KTV PARAMS OUT Of Module", SND_SOC_NOPM, 0, 0x7fffffff,
	                     0, 4, vivo_audio_ktv_param_get, vivo_audio_ktv_param_put),
};

void vivo_ktv_add_controls(struct snd_soc_platform *platform)
{
	snd_soc_add_platform_controls(platform, vivo_audio_ktv_controls,
