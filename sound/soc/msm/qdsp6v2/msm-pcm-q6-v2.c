					...
					void *priv_data)
{
	...
	switch (event) {
	/*shijianxing add for flush buffer operation when change devices*/
...
static int msm_pcm_playback_prepare(struct snd_pcm_substream *substream)
{
	...
	if (prtd->compress_enable) {
		...
		//shijianxing M for Qualcomm,device change caused sound problem
		event.event_func = msm_pcm_route_event_handler;
		event.priv_data = (void *) prtd;
		ret = msm_pcm_routing_reg_phy_stream_v2(soc_prtd->dai_link->be_id,
			prtd->audio_client->perf_mode,
