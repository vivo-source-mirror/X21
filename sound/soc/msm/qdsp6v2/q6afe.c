...
#define CONFIG_SND_SOC_MAX98927
...
struct afe_ctl {
	...
	struct afe_dsm_get_resp dsm_get_resp;
#endif
};
...
*/
	...
	case AFE_PARAM_ID_CALIB:
		...
		data_dest = (u32 *) &this_afe.dsm_get_resp;
		break;
#endif
	default:
		pr_err("%s: Unrecognized param ID %d\n", __func__,
...
#ifdef CONFIG_SND_SOC_MAX98927
int afe_dsm_set_params(uint8_t *payload, int size, uint32_t dst_port, uint32_t mod_id, uint32_t param_id)
{
	struct param_hdr_v3 param_info = {0};
	u16 port_id;
	...
	param_info.module_id = mod_id;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = param_id;
	param_info.param_size = size;
	
	port_id = q6audio_get_port_id(dst_port);
	ret = q6afe_pack_and_set_param_in_band(port_id, q6audio_get_port_index(dst_port), param_info, payload);
	if (ret) {
		pr_err("%s:afe_dsm_set_params for port 0x%x failed ret %d", __func__, port_id, ret);	
	}
fail_cmd:
	pr_debug("%s:afe_dsm_set_params for port 0x%x ret %d", __func__, port_id, ret);	
	return ret;
}

int afe_dsm_get_params(uint8_t *payload, int size, uint32_t dst_port, uint32_t mod_id, uint32_t param_id)
{
	struct param_hdr_v3 param_hdr = {0};
	u16 port_id;
	int index = 0, ret = -EINVAL;
	...
	pr_debug("%s: get param port 0x%x param id[0x%x]  %d\n",
			__func__, port_id, param_hdr.param_id, ret);
	memcpy(payload, &this_afe.dsm_get_resp.payload,
...
int afe_dsm_setget_params(uint8_t *payload, int size, int dir, uint32_t dst_port, uint32_t mod_id, uint32_t param_id)
{
	...
	if (dir){
		...
		get->param.apr_hdr.hdr_field		= APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
				APR_HDR_LEN(APR_HDR_SIZE), APR_PKT_VER);
		get->param.apr_hdr.pkt_size		= size + sizeof(*get);
		get->param.apr_hdr.token			= index;
		get->param.apr_hdr.opcode		    = AFE_PORT_CMD_GET_PARAM_V2;
...
fail_cmd:
	pr_debug("%s: status %d 0x%x port_id:%d\n",__func__, ret, dst_port,q6audio_get_port_id(dst_port));
...
int afe_dsm_ramp_dn_cfg(uint8_t *payload, uint32_t port_rx, int delay_in_ms)
{
	...
	*(params + 7)	= 1;
	...
	afe_dsm_set_params(payload, 8*sizeof(uint32_t), port_rx, AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
	usleep_range(delay_in_ms*1000, delay_in_ms*1000 + 10);
    return 0;
}

int afe_dsm_rx_set_params(uint8_t *payload, uint32_t port_rx, int size)
{
	return afe_dsm_set_params(payload, size,  port_rx, AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
}

int afe_dsm_rx_get_params(uint8_t *payload, uint32_t port_rx, int size)
{
	return afe_dsm_get_params(payload, size,  port_rx, AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
}

int afe_dsm_set_calib(uint8_t* payload, uint32_t port_tx)
{
	return afe_dsm_set_params(payload, sizeof(uint32_t)*3, port_tx, AFE_MODULE_DSM_TX, AFE_PARAM_ID_CALIB);
}

int afe_dsm_get_calib(uint8_t* payload, uint32_t port_rx, uint32_t port_tx)
{
	uint32_t *params = (uint32_t *)payload;
	*(params)		= 0;
	*(params + 1)	= 1;               //count
	...
	*(params + 3)	= 4;              // mode 0: disable, 1: enable, 2: bypass and pilot tone, 4: pilot tone only
	afe_dsm_set_params(payload, 4*sizeof(uint32_t),  port_rx, AFE_MODULE_DSM_RX, AFE_PARAM_ID_DSM_CFG);
	usleep_range(1000*1000, 1000*1000 + 10);

	return afe_dsm_get_params(payload, sizeof(uint32_t)*14,  port_tx, AFE_MODULE_DSM_TX, AFE_PARAM_ID_CALIB);
}

int afe_dsm_f0_start(uint8_t *payload, uint32_t port_rx, int delay_in_ms)
{
	uint32_t *params = (uint32_t *)payload;
	*(params)		= 0;
	*(params + 1)	= 1;               /* count */
	*(params + 2)	= 0x0300000F;      /* enable flag */
	*(params + 3)	= 1;
...
int afe_dsm_get_f0(uint8_t *payload, uint32_t port_tx, uint32_t port_rx)
{
	static uint32_t pre_fc;
	uint32_t f0 = 0;

	afe_dsm_get_params(payload, sizeof(uint32_t)*14,  port_tx, AFE_MODULE_DSM_TX, AFE_PARAM_ID_CALIB);
	f0 = *((uint32_t *)payload + 2);

	if (pre_fc == f0) {
		afe_dsm_f0_start((uint8_t *)payload, port_rx, 25);
	} else {
		pre_fc = f0;
	}
...
/* int afe_dsm_get_libary_info(uint32_t* payload, uint32_t port_tx, int size) */
/* { */
	/* return afe_dsm_setget_params(payload, size, 1, port_tx, AFE_MODULE_DSM_TX, AFE_PARAM_ID_DSM_INFO); */
/* } */
