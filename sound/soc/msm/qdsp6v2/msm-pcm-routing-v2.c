...
static struct ec_ref_conf msm_ec_ext_ref[MAX_EC] = {
	{AFE_PORT_INVALID, EXT_EC_REF_OFF, false},
	{AFE_PORT_INVALID, EXT_EC_REF_OFF, false}
};
...
static void match_suitale_sample_rate_for_adm_voc_topo(u32 topology,
					u32 be_rate, u32 *target_rate, u16 *bits_per_sample)
{
	switch(topology){
		case VPM_TX_SM_FVSAM_COPP_TOPOLOGY:
		case VPM_TX_DM_FVSAM_COPP_TOPOLOGY:
		case VPM_TX_TM_FVSAM_COPP_TOPOLOGY:
		case VPM_TX_DADCS_FVSAM_COPP_TOPOLOGY:
		case VPM_RX_FVSAM_COPP_TOPOLOGY:
	...
	for (i = 0; i < MSM_BACKEND_DAI_MAX; i++) {
		...
		if (!is_be_dai_extproc(i) &&
			...
			fe_dai_app_type_cfg[fe_id][session_type][i].sample_rate;
				...
				fe_dai_app_type_cfg[fe_id][session_type][i].sample_rate = sample_rate;
			...
			topology = msm_routing_get_adm_topology(fe_id,
								session_type,
								i);
			match_suitale_sample_rate_for_adm_voc_topo(topology,
...
#else
				sample_rate = msm_bedais[i].sample_rate;
				fe_dai_app_type_cfg[fedai_id][session_type][i].sample_rate = sample_rate;
	...
	if (set) {
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			if (app_type && is_lsm) {
				...
				fe_dai_app_type_cfg[val][session_type][reg].sample_rate = sample_rate;
				app_type_cfg[app_type_idx].bit_width = bits_per_sample;
#endif
				copp_token =
				fe_dai_app_type_cfg[val][session_type][reg]
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			topology = adm_get_topology_for_port_copp_idx(port_id,
								      idx);
			/*shijianxing add for flush buffer when change devices before close*/
			if (session_type == SESSION_TYPE_RX &&
				fdai->event_info.event_func)
				fdai->event_info.event_func(
						MSM_PCM_RT_EVT_DEVSWITCH,
...
static const char *const ec_ref_rx[] = { "None", "SLIM_RX", "I2S_RX",
	...
	"INT0_MI2S_RX", "INT4_MI2S_RX", "INT3_MI2S_TX", "PRI_MI2S_RX", "SEC_MI2S_RX"};
...
static int msm_routing_ext_ec_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s:ext_ec_ref_rx  = %x, voice ec_ref %x, voip ec_ref %x\n",
		__func__, msm_route_ext_ec_ref, msm_ec_ext_ref[VOC_EC].ext_ec_ref,
		msm_ec_ext_ref[VOIP_EC].ext_ec_ref);
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	mutex_lock(&routing_lock);
	ext_ec_ref = ucontrol->value.integer.value[0];

	switch (ext_ec_ref) {
	case EXT_EC_REF_PRI_MI2S_TX:
		ref_port_id = AFE_PORT_ID_PRIMARY_MI2S_TX;
		break;
	case EXT_EC_REF_SEC_MI2S_TX:
		ref_port_id = AFE_PORT_ID_SECONDARY_MI2S_TX;
		break;
	case EXT_EC_REF_SEC_MI2S_RX:
		ref_port_id = AFE_PORT_ID_SECONDARY_MI2S_RX;
		break;
	case EXT_EC_REF_TERT_MI2S_TX:
		ref_port_id = AFE_PORT_ID_TERTIARY_MI2S_TX;
		break;
	case EXT_EC_REF_QUAT_MI2S_TX:
		ref_port_id = AFE_PORT_ID_QUATERNARY_MI2S_TX;
		break;
	case EXT_EC_REF_QUIN_MI2S_TX:
		ref_port_id = AFE_PORT_ID_QUINARY_MI2S_TX;
		break;
	case EXT_EC_REF_SLIM_1_TX:
		ref_port_id = SLIMBUS_1_TX;
		break;
	case EXT_EC_REF_NONE:
	default:
		ref_port_id = AFE_PORT_INVALID;
		state = false;
		break;
	}

	pr_info("%s:previous ext_ec_ref val = %d ref_port_id = 0x%0x state = %d\n",
		 __func__, ext_ec_ref, ref_port_id, state);

	/* 1. save ec ref to respording configs */
	if (widget->name) {
		const char *voice_dapm = "VOC_EXT_EC MUX";
		int len = min(strlen(voice_dapm), strlen(widget->name));
		if (!strncmp(widget->name, voice_dapm, len))
			kctl_id = VOC_EC;
	}

	msm_ec_ext_ref[kctl_id].ref_port_id = ref_port_id;
	msm_ec_ext_ref[kctl_id].ext_ec_ref = ext_ec_ref;
	msm_ec_ext_ref[kctl_id].state = state;

	/* 2. voice EC ref prior to voip EC ref. EXT_EC_REF_OFF implies
		the mixer is closed, we can only make voip ec work when voice
		ec is closed.
	*/
	ext_ec_ref_port_id = msm_ec_ext_ref[VOC_EC].ref_port_id;
	msm_route_ext_ec_ref = msm_ec_ext_ref[VOC_EC].ext_ec_ref;
	state = msm_ec_ext_ref[VOC_EC].state;

	/* voip's operation when voice ec ref is closed */
	if (kctl_id == VOIP_EC && msm_ec_ext_ref[VOC_EC].ext_ec_ref == EXT_EC_REF_OFF) {
		ext_ec_ref_port_id = msm_ec_ext_ref[VOIP_EC].ref_port_id;
		msm_route_ext_ec_ref = msm_ec_ext_ref[VOIP_EC].ext_ec_ref;
		state = msm_ec_ext_ref[VOIP_EC].state;
	}
	/*  keep voip alive when closing voice ec ref */
	if (kctl_id == VOC_EC && msm_ec_ext_ref[VOC_EC].ext_ec_ref == EXT_EC_REF_OFF &&
		msm_ec_ext_ref[VOIP_EC].ext_ec_ref != EXT_EC_REF_OFF) {
		ext_ec_ref_port_id = msm_ec_ext_ref[VOIP_EC].ref_port_id;
		msm_route_ext_ec_ref = msm_ec_ext_ref[VOIP_EC].ext_ec_ref;
		state = msm_ec_ext_ref[VOIP_EC].state;
	}

	pr_info("%s:finally msm_route_ext_ec_ref %d, ext_ec_ref_port_id 0x%0x,"
		"state %d, id %d, voice ec_ext_ref %d, voip ec_ext_ref %d!\n",
		 __func__, msm_route_ext_ec_ref, ext_ec_ref_port_id, state, kctl_id,
		 msm_ec_ext_ref[VOC_EC].ext_ec_ref, msm_ec_ext_ref[VOIP_EC].ext_ec_ref);
...
static const char * const ext_ec_ref_rx[] = {"NONE", "PRI_MI2S_TX",
					"SEC_MI2S_TX", "SEC_MI2S_RX", "TERT_MI2S_TX",
...
static const struct snd_soc_dapm_route intercon[] = {
	...
	{"VOIP_EXT_EC MUX", "SEC_MI2S_RX", "SEC_MI2S_RX"},
	...
	/*
	* QC: connect to INT4_MI2S_DL_HL since same pcm_id.
	* vivo: connect to INT0_MI2S_DL_HL since same pcm_id.
	* Reason: FM link to adsp by Slimbus port in sdm660 platform,
	* Spk and Headphnoe must use the same pcm id due to
	* Slimbus protocol specificity.@fanyongxiang.
	*/
	...
	{"SEC_MI2S_RX_DL_HL", "Switch", "INT0_MI2S_DL_HL"},
	{"SEC_MI2S_RX", NULL, "SEC_MI2S_RX_DL_HL"},
	{"TERT_MI2S_RX_DL_HL", "Switch", "TERT_MI2S_DL_HL"},
	{"TERT_MI2S_RX_DL_HL", "Switch", "INT0_MI2S_DL_HL"},
	...
	for_each_set_bit(i, &bedai->fe_sessions[0], MSM_FRONTEND_DAI_MAX) {
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			if (app_type && is_lsm) {
				...
				fe_dai_app_type_cfg[i][session_type][be_id].sample_rate = sample_rate;
				app_type_cfg[app_type_idx].bit_width = bits_per_sample;
#endif
				copp_token =
					fe_dai_app_type_cfg[i][session_type]
...
static int msm_routing_probe(struct snd_soc_platform *platform)
{
	...
	vivo_ktv_add_controls(platform);
